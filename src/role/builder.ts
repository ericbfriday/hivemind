...
declare global {
  ...
  export interface BuilderCreepHeapMemory extends CreepHeapMemory {
  }
...
export class BuilderRole extends Role {
    ...
    const repairableStructureIds = cache.inHeap(`repairStructures:${creep.room.name}`, 5, () => {
      const repairableStructures = _.filter(creep.room.structures, (structure: Structure<BuildableStructureConstant>) => {
        ...
        if (structure.structureType === STRUCTURE_ROAD) {
          ...
          if (!isPlannedRoad && !isOperationRoad) {
            return false;
          }
        }
  ...
  repairNearby(creep: BuilderCreep) {
    if (creep.store[RESOURCE_ENERGY] < creep.store.getCapacity() * 0.7 && creep.store[RESOURCE_ENERGY] > creep.store.getCapacity() * 0.3) {
      return;
    }
    if (throttle(creep.heapMemory._tO)) {
      return;
    }
    ...
    const needsRepair = _.filter(this.getAvailableRepairTargets(creep), (structure) => {
      if (creep.pos.getRangeTo(structure.pos) > 3) {
        return false;
      }
      return true;
    });
