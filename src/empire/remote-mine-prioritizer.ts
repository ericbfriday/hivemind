...
export default class RemoteMinePrioritizer {
  ...
  getRemoteMiningSourceRooms(): Record<string, SourceRoomAvailability> {
    ...
    for (const room of Game.myRooms) {
      ...
      if (powerCreep) {
        const operateSpawnLevel =
          (powerCreep.powers[PWR_OPERATE_SPAWN] || {}).level || 0;
        if (operateSpawnLevel > 0)
          spawnCapacity /=
            POWER_INFO[PWR_OPERATE_SPAWN].effect[operateSpawnLevel - 1];
      }
  ...
  getRemoteHarvestRooms(
    sourceRooms: Record<string, SourceRoomAvailability>,
  ): HarvestRoomInfo[] {
