...
export class RoomDefenseProcess extends Process {
    ...
    manageTowers() {
        ...
        for (const tower of towers) {
            if (this.performTowerAttack(tower, hostileCreeps)) {
                continue;
            }
            if (this.performTowerHeal(tower, enemyStrength)) {
                continue;
            }
            if (this.performTowerRampartRepair(tower, hostileCreeps, enemyStrength)) {
                continue;
            }
        }
    }
    ...
    performTowerHeal(tower: StructureTower, enemyStrength: EnemyStrength): boolean {
        ...
        const bestTarget = _.minBy(damagedTargets, creep => creep.hits / creep.hitsMax / (1 + tower.getPowerAtRange(tower.pos.getRangeTo(creep.pos))));
        if (!bestTarget) {
            return false;
        }
        if (tower.heal(bestTarget) === OK) {
            return true;
        }
    ...
    // Repair ramparts during a strong attack.
        ...
        const availableRamparts = cache.inObject(tower.room, 'endangeredRamparts', 1, () => {
            const availableRamparts = new Set<StructureRampart>();
        ...
        if (availableRamparts.size === 0) {
            return false;
        }

        const prioritizedRampart = _.minBy([...availableRamparts], rampart => rampart.hits / (1 + tower.getPowerAtRange(tower.pos.getRangeTo(rampart.pos))));
        if (!prioritizedRampart) {
            return false;
        }

        if (tower.repair(prioritizedRampart) === OK) {
            return true;
        }
    ...
    manageSafeMode() {
        ...
        if (this.room.defense.getEnemyStrength() === ENEMY_STRENGTH_NONE) {
            return;
        }
        if (this.room.defense.getEnemyStrength() < ENEMY_STRENGTH_NORMAL && Game.myRooms.length > 1) {
            return;
        }
        if (this.room.defense.isWallIntact()) {
            return;
        }
        if ((this.room.myStructuresByType[STRUCTURE_SPAWN] || []).length === 0) {
            return;
        }

        this.room.visual.text('I might safemode soon!', 25, 25);
        if (!this.isEnemyCloseToImportantStructures()) {
            return;
        }
        ...
        for (const structure of this.getImportantStructures()) {
            for (const userName in this.room.enemyCreeps) {
                ...
                for (const creep of this.room.enemyCreeps[userName]) {
                    ...
                    if (structure instanceof Structure && !creep.isDangerous()) {
                        continue;
                    }
                    ...
                    if (structure.pos.getRangeTo(creep.pos) <= maxRange) {
                        return true;
                    }
                }
            }
    ...
    getImportantStructures() {
        ...
        for (const constructionSite of this.room.find(FIND_MY_CONSTRUCTION_SITES, {
            filter: site => ([
                STRUCTURE_FACTORY,
                STRUCTURE_LAB,
                STRUCTURE_NUKER,
            ...
            if (constructionSite.progress < 10_000) {
                continue;
            }
    ...
    abandonSafemodedRoomIfNecessary() {
        ...
        for (const room of Game.myRooms) {
            ...
            if (room.controller.level > maxLevelToAbandon) {
                continue;
            }
    ...
    manageDefenseRequests() {
        ...
        if (!Memory.requests.defense) {
            Memory.requests.defense = {};
        }
