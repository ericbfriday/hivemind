...
export  class RemoteHarvesterRole extends Role {
  ...
  performRemoteHarvest(creep: RemoteHarvesterCreep) {
    ...
    if (this.repairNearbyContainer(creep)) {
      return;
    }
  ...
  mayHarvest(creep: RemoteHarvesterCreep, source: Source): boolean {
    ...
    if (creep.store.getFreeCapacity() >= harvestPower) {
      return true;
    }
    ...
    if (container.store.getFreeCapacity() >= harvestPower) {
      return true;
    }
    ...
    if (source.ticksToRegeneration <= ticksToHarvestFully) {
      return true;
    }
