...
declare global {
  ...
  export interface PowerHarvesterCreepMemory extends CreepMemory {
    role: 'harvester.power'
    isHealer: boolean
    targetRoom: string
  }

  export interface PowerHarvesterCreepHeapMemory extends CreepHeapMemory {
  }
}

export class PowerHarvesterRole extends Role {
  constructor() {
    super();
  ...
  attackNearby(creep: PowerHarvesterCreep) {
    ...
    const highestValue = _.maxBy(targets, c => (c.getDamageCapacity(1) + (c.getHealCapacity(1) * 2)));
    if (!highestValue) {
      return;
    }
  ...
  chaseNearbyRanged(creep: PowerHarvesterCreep) {
    ...
    const targets = creep.pos.findInRange(FIND_HOSTILE_CREEPS, 3, {
      filter: c => !hivemind.relations.isAlly(c.owner.username) && _.some(c.body, p => p.type === RANGED_ATTACK),
    });
    if (targets.length === 0) {
      return;
    }
    ...
    const highestValue = _.maxBy(targets, (c: Creep) => (c.getDamageCapacity(1) + (c.getHealCapacity(1) * 2) - (c.pos.getRangeTo(creep.pos) * 5)));
    if (!highestValue) {
      return;
    }
...
export default PowerHarvesterRole;
