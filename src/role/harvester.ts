...
export default class HarvesterRole extends Role {
  ...
  run(creep: HarvesterCreep) {
    ...
    if (this.hasFinishedDelivering(creep)) {
      ...
      this.hasFinishedHarvesting(creep) &&
      !this.isStationaryHarvester(creep)
    ) {
  ...
  performHarvest(creep: HarvesterCreep) {
    ...
    if (creep.memory.harvestPos) {
      const harvestPosition = deserializePosition(
    ...
    const harvestPower =
      source instanceof Source ? HARVEST_POWER : HARVEST_MINERAL_POWER;
    if (
      targetContainer &&
      targetContainer.pos.getRangeTo(targetPos) > 1 &&
      creep.store.getFreeCapacity() <
        creep.getActiveBodyparts(WORK) * harvestPower
    ) {
  ...
  depositInBay(creep: HarvesterCreep): boolean {
    ...
    if (
      ...
      creep.store.getCapacity() * (bay.needsRefill() ? 0.6 : 1)
    )
  ...
  pickupEnergy(creep: HarvesterCreep) {
    ...
    if (
      container &&
      ((container as StructureContainer).store.energy || 0) > 0
    ) {
  ...
  depositResources(creep: HarvesterCreep, source: Source | Mineral) {
    ...
    if (
      ...
      creep.store.energy >
        Math.max(
          creep.getActiveBodyparts(WORK) * BUILD_POWER,
    ...
    if (creep.room.controller.level < 6 && creep.store.energy > 0) {
      const nearbyCreeps = creep.pos.findInRange(FIND_MY_CREEPS, 1, {
        filter: (c) => {
          if (
            ["transporter", "upgrader", "builder", "builder.remote"].includes(
              c.memory.role,
            ) &&
      ...
      if (nearbyCreeps.length > 0) {
        ...
        creep.transfer(
          targetCreep,
          RESOURCE_ENERGY,
          Math.min(creep.store.energy, targetCreep.store.getFreeCapacity()),
        );
      }
    }
  }
