...
declare global {
  ...
  export interface HelperCreepMemory extends CreepMemory {
    role: 'helper'
    delivering?: boolean
  }

  export interface HelperCreepHeapMemory extends CreepHeapMemory {
  }
}

export default HelperRole;
export class HelperRole extends Role {
  /**
  ...
  performHelperDeliver(creep: HelperCreep) {
    ...
    if (this.performHelperLabDeliver(creep)) {
      return;
    }
  ...
  performHelperGather(creep: HelperCreep) {
    ...
    if (this.performHelperLabGather(creep)) {
      return;
    }
    ...
    const totalNeededEnergy = _.sumBy(labs, lab => Math.min(lab.store.getFreeCapacity(RESOURCE_ENERGY), boostManager.getRequiredEnergyAmount(lab.id) - lab.store.getUsedCapacity(RESOURCE_ENERGY)));
