...
export default TradeRouteManager;
export class TradeRouteManager {
    ...
    public getAvailableTransportRoutes(): TransportRouteOption[] {
        ...
        _.each(rooms, (roomState: RoomResourceState, roomName: string) => {
            ...
            for (const resourceType of getResourcesIn(roomState.state)) {
                if (!roomState.totalResources[resourceType]) {
                    continue;
                }
                ...
                if (needsTerminalSpace && (roomState.totalResources[resourceType] || 0) < 100) {
                    continue;
                }
                if (resourceType === RESOURCE_ENERGY && (roomState.totalResources[resourceType] || 0) < 10_000) {
                    continue;
                }
                ...
                _.each(rooms, (roomState2: any, roomName2: string) => {
                    ...
                    if (room2.terminal.store.getFreeCapacity() < 5000) {
                        return;
                    }

                    if (resourceType === RESOURCE_ENERGY && this.roomNeedsTerminalSpace(room2) && !this.isEvacuatingRoomWithLowEnergy(room2)) {
                        return;
                    }
                    ...
                    if (!needsTerminalSpace && !needsStorageSpace && !shouldReceiveResources) {
                        return;
                    }
                    ...
                    if (room.terminal.store.energy < neededEnergy) {
                        return;
                    }
    ...
    // Fullfill allies trade requests.
        ...
        for (const roomName2 in requests) {
            ...
            if (room.terminal.store.energy < Game.market.calcTransactionCost(5000, room.name, roomName2)) {
                continue;
            }
