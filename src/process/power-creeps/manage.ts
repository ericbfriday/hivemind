...
export default ManagePowerCreepsProcess;
export class ManagePowerCreepsProcess extends Process {
  /**
   * Upgrade power creeps depending on our needs.
   */
  run() {
    if (!hivemind.settings.get('automaticallyUpgradePowerCreeps')) {
      return;
    }

    const usedGpl = _.sumBy(_.map(Game.powerCreeps, creep => creep.level + 1));
    if (usedGpl >= Game.gpl.level) {
      return;
    }

    hivemind.log('creeps').info('Unused power creep levels:', Game.gpl.level - usedGpl);

    const creepToUpgrade = _.minBy(this.getUpgradeablePowerCreeps(), creep => creep.level);
  ...
  getUpgradeablePowerCreeps(): PowerCreep[] {
    return _.filter(Game.powerCreeps, (creepToUpgrade: PowerCreep) => {
      ...
      if (creepToUpgrade.level >= POWER_CREEP_MAX_LEVEL) {
        return false;
      }
  ...
  upgradePowerCreep(creep: PowerCreep) {
    for (const powerOption of hivemind.settings.get('powerPriorities')) {
      ...
      if (currentLevel >= info.level.length) {
        continue;
      }
