...
export default class SpawnManager {
  ...
  manageSpawns(room: Room, spawns: StructureSpawn[]) {
    ...
    const options: SpawnOption[] = filter(
      ...
      (option: SpawnOption) => {
        ...
        const closestSpawn = min(spawns, (spawn) =>
          spawn.pos.getRangeTo(option.preferClosestSpawn),
        );
        // Only spawn once preferred spawn is ready.

        if (
          closestSpawn.pos.getRangeTo(option.preferClosestSpawn) < 3 &&
          !availableSpawns.includes(closestSpawn)
        )
          return false;
    ...
    if (!this.trySpawnCreep(room, spawn, option)) {
      each(availableSpawns, (s) => {
        ...
        room.visual.text(
          option.role + "@" + option.priority.toPrecision(1),
          spawn.pos.x + 0.05,
          spawn.pos.y + 0.65,
          {
            font: 0.5,
  ...
  trySpawnCreep(room: Room, spawn: StructureSpawn, option): boolean {
    ...
    if (boosts && room.boostManager) {
      ...
      for (const partType of body) {
        ...
        boostResources[boosts[partType]] =
          (boostResources[boosts[partType]] || 0) + 1;
        found = true;
      }
  ...
  spawnRevoveryCreep(room: Room, spawn: StructureSpawn) {
    ...
    if (result !== OK) {
      hivemind
        .log("creeps", room.name)
        .error(
          "Trying to spawn recovery creep",
          creepName,
          "failed with error code",
          result,
        );
  ...
  generateCreepName(roleId: string): string {
    ...
    if (
      ...
      Memory.creepCounter[roleId] >= 36 * 36
    ) {
