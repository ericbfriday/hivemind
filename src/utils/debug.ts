...
export default class Logger {
  ...
  constructor(channel: string, roomName?: string) {
    ...
    if (channels[this.channel]) {
      this.channelName = ("          " + channels[this.channel].name).slice(
        -10,
      );
    ...
    if (
      ...
      Memory.logger.channelSettings[this.channel].disabled
    ) {
      this.active = false;
  ...
  getOutputPrefix(): string {
    let prefix =
      '[<font color="' + this.color + '">' + this.channelName + "</font>";
    prefix += "]";
    if (this.roomName) {
      ...
      prefix +=
        '[<font color="#' + roomColor + '">' + this.roomName + "</font>]";
    } else {
      prefix += "        ";
    }
