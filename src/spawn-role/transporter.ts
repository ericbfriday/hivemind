...
export default class TransporterSpawnRole extends SpawnRole {
    ...
    return this.cacheEmptySpawnOptionsFor(room, 10, () => {
      ...
      if (transporterCount < maxTransporters) {
        ...
        if (transporterCount >= maxTransporters / 2) {
          ...
          const minSpawnDistance = min(
            map(spawns, (spawn) =>
              min(map(sources, (source) => spawn.pos.getRangeTo(source.pos))),
            ),
          );
  ...
  getTransporterAmount(room: Room, transporterSize: number): number {
    ...
    maxTransporters = Math.max(
      maxTransporters,
      room.controller.level > 6 ? 2 : 3,
    );
    ...
    if (
      ...
      room.terminal.store.getUsedCapacity() >
        room.terminal.store.getCapacity() * 0.01
    ) {
      maxTransporters *= 1.5;
    }
    ...
    if (
      ...
      room.storage.store.getUsedCapacity() >
        room.storage.store.getCapacity() * 0.01
    ) {
      maxTransporters *= 1.5;
    }
    ...
    if (room.controller.level < 4) {
      ...
      for (const source of room.sources) {
        ...
        if (
          container &&
          container.store.getFreeCapacity() < container.store.getCapacity() / 4
        ) {
          maxTransporters++;
        }
      }
  ...
  getTransporterBaseAmount(room: Room): number {
    ...
    if (room.memory.controllerLink) {
      maxTransporters -=
        1 +
        sum(room.sources, (source: Source) => (source.getNearbyLink() ? 1 : 0));
    }
  ...
  getExtraUpgraderTransporters(room: Room): number {
    ...
    const upgraderWorkParts = sum(room.creepsByRole.upgrader, (creep) =>
      creep.getActiveBodyparts(WORK),
    );
    const refillPathLength = cache.inHeap(
      "ccRefillPathLength:" + room.name,
      5000,
      () => {
        const container = Game.getObjectById<StructureContainer>(
          room.memory.controllerContainer,
        );
    ...
    const usedEnergyPerTrip =
      upgraderWorkParts * UPGRADE_CONTROLLER_POWER * refillPathLength * 1.8;
    return Math.floor(
      usedEnergyPerTrip / CARRY_CAPACITY / this.getTransporterSize(room),
    );
  }
  ...
  getTransporterSize(room: Room): number {
    const fullBayCapacity =
      Math.max(
        SPAWN_ENERGY_CAPACITY,
        EXTENSION_ENERGY_CAPACITY[room.controller.level],
      ) +
      6 * EXTENSION_ENERGY_CAPACITY[room.controller.level];
    ...
    return new BodyBuilder()
      ...
      .setEnergyLimit(
        Math.min(
          ...
          Math.max(
            option.force ? 250 : room.energyCapacityAvailable * 0.9,
