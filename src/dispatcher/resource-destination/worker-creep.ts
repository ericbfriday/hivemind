...
export default WorkerCreepDestination;
export class WorkerCreepDestination extends TaskProvider<WorkerCreepDestinationTask, ResourceDestinationContext> {
    ...
    getTasks(context: ResourceDestinationContext) {
        ...
        return this.cacheEmptyTaskListFor('', 25, () => {
            ...
            for (const role in targetRoleWeights) {
                if (role !== 'builder' && this.room.defense.getEnemyStrength() > ENEMY_STRENGTH_NORMAL) {
                    continue;
                }
    ...
    private addRoleTasks(options: WorkerCreepDestinationTask[], role: string, weight: number, context: ResourceDestinationContext) {
        for (const creep of _.values<Creep>(this.room.creepsByRole[role])) {
            ...
            if (creep.store.getFreeCapacity(RESOURCE_ENERGY) < creep.store.getCapacity(RESOURCE_ENERGY) / 3) {
                continue;
            }
    ...
    isValid(task: WorkerCreepDestinationTask, context: ResourceDestinationContext) {
        ...
        if (target.store.getFreeCapacity(task.resourceType) < target.store.getCapacity(RESOURCE_ENERGY) / 5) {
            return false;
        }
        if (target.room.name !== context.creep.room.name) {
            return false;
        }
