...
Room.prototype.assertMilitarySituation = function (this: Room) {
  ...
  for (const creep of creeps) {
    if (creep.my) {
      ...
      creep.isDangerous() &&
      !hivemind.relations.isAlly(creep.owner.username)
    ) {
  ...
  for (const powerCreep of powerCreeps) {
    if (powerCreep.my) {
      ...
      powerCreep.isDangerous() &&
      !hivemind.relations.isAlly(powerCreep.owner.username)
    ) {
 ...
 *   The creep to asses.
 */
Room.prototype.assertMilitaryCreepPower = function (
  this: Room,
  ...
  for (const part of creep.body) {
    ...
    if (part.boost) {
      if (part.type === ATTACK && BOOSTS[ATTACK][part.boost].attack) {
        ...
        BOOSTS[RANGED_ATTACK][part.boost].rangedAttack
      ) {
  ...
  const assertAllTargets = (
    targets: Array<Creep | PowerCreep>,
    range: number,
    amount: number,
    type: string,
  ) => {
  ...
  assertAllTargets(
    targets,
    1,
    ATTACK_POWER * totalParts[ATTACK],
    hostile ? "damage" : "myDamage",
  );
  ...
  assertAllTargets(
    targets,
    3,
    RANGED_ATTACK_POWER * totalParts[RANGED_ATTACK],
    hostile ? "damage" : "myDamage",
  );
  ...
  assertAllTargets(
    allies,
    3,
    RANGED_HEAL_POWER * totalParts[HEAL],
  ...
  assertAllTargets(
    allies,
    1,
    (HEAL_POWER - RANGED_HEAL_POWER) * totalParts[HEAL],
    hostile ? "healing" : "myHealing",
  );
};
 ...
 */
Room.prototype.assertMilitaryStructurePower = function (
  this: Room,
  ...
  if (structure.my && structure.store[RESOURCE_ENERGY] < TOWER_ENERGY_COST * 10)
    return;
 ...
 */
Room.prototype.addMilitaryAssertion = function (
  this: Room,
  x: number,
  y: number,
  amount: number,
 ...
 */
Room.prototype.getMilitaryAssertion = function (
  this: Room,
  x: number,
  y: number,
  type: string,
...
Room.prototype.assertTargetPriorities = function (this: Room) {
  ...
  for (const creep of this.militaryObjects.creeps) {
    const potentialDamage = this.getMilitaryAssertion(
      creep.pos.x,
      creep.pos.y,
      "myDamage",
    );
    const potentialHealing = this.getMilitaryAssertion(
      creep.pos.x,
      creep.pos.y,
      "healing",
    );
    ...
    if (effectiveDamage > potentialHealing * neededDamageFactor) {
      ...
      creep.militaryPriority =
        (creep.getMilitaryValue() *
          (effectiveDamage - potentialHealing) *
          (creep.hitsMax / creep.hits) *
          creep.ticksToLive) /
        CREEP_LIFE_TIME;
      visual.text(
        creep.militaryPriority.toPrecision(2),
        creep.pos.x + 1,
        creep.pos.y + 0.2,
        { font: 0.5, color: "yellow" },
      );
    }
  }
};
...
Room.prototype.getTowerTarget = function (this: Room) {
  return cache.inObject(this, "towerTarget", 1, () => {
    ...
    if (max)
      this.visual.circle(max.pos.x, max.pos.y, {
        radius: 1,
        fill: "red",
        opacity: 0.5,
      });
