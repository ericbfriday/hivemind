...
export default class HarvesterSpawnRole extends SpawnRole {
  ...
  getSpawnOptions(room: Room): HarvesterSpawnOption[] {
    return this.cacheEmptySpawnOptionsFor(room, 10, () => {
      ...
      if (room.isFullOnEnergy() && !this.isSmallHarvesterNeeded(room))
        return [];
  ...
  addInitialHarvester(source: Source, options: HarvesterSpawnOption[]) {
    ...
    const minSpawnDistance = min(
      map(spawns, (spawn) => spawn.pos.getRangeTo(source.pos)),
    );
    const activeHarvesters = filter(
      source.harvesters,
      (creep) =>
        creep.spawning ||
        creep.ticksToLive >
          creep.body.length * CREEP_SPAWN_TIME + minSpawnDistance,
    );
  ...
  addAdditionalHarvesters(source: Source, options: HarvesterSpawnOption[]) {
    ...
    if (totalWorkParts < maxParts) {
      options.push({
        ...
        weight:
          1 - totalWorkParts / maxParts / 2 - (50 - minSpawnDistance) / 100,
        source: source.id,
        preferClosestSpawn: source.pos,
        size: maxParts,
        force: false,
  ...
  isSmallHarvesterNeeded(room: Room): boolean {
    ...
    if (size(room.creepsByRole.harvester) > 0) return false;
  ...
  getMaxWorkParts(source: Source): number {
    ...
    const sizeFactor =
      source.room.controller.level === 8
        ? 2
        : source.room.controller.level === 7
          ? 1.8
          : source.room.controller.level === 6
            ? 1.5
            : 1.2;
  ...
  getCreepBody(room: Room, option: HarvesterSpawnOption): BodyPartConstant[] {
    ...
    const hasSpawnAtSource = some(
      room.myStructuresByType[STRUCTURE_SPAWN],
      (s) => source.pos.getRangeTo(s.pos) <= 2 && s.isOperational(),
    );
    const hasFewExtensions =
      room.energyCapacityAvailable < SPAWN_ENERGY_CAPACITY * 2;

    return new BodyBuilder()
      .setWeights({ [WORK]: 4, [CARRY]: 1 })
      .setPartLimit(WORK, option.size)
      .setMovementMode(
        hasSpawnAtSource || hasFewExtensions
          ? MOVEMENT_MODE_MINIMAL
          : MOVEMENT_MODE_ROAD,
      )
      ...
      .setEnergyLimit(
        Math.min(
          ...
          Math.max(
            option.force ? SPAWN_ENERGY_CAPACITY : room.energyCapacityAvailable,
