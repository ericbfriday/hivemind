...
export default class PowerHaulerRole extends Role {
  ...
  run(creep: PowerHaulerCreep) {
    if (
      !creep.memory.isReturning &&
      (creep.store.getFreeCapacity() === 0 ||
        (creep.store.power || 0) > creep.store.getCapacity() / 10)
    ) {
      // Return home.
    ...
    if (powerBanks.length > 0) {
      ...
      if (powerBank.hits < ATTACK_POWER * MAX_CREEP_SIZE * 5) {
        ...
        // power is depleted.
        if (
          creep.store.getUsedCapacity() > (creep.store[RESOURCE_POWER] || 0)
        ) {
  ...
  returnHome(creep: PowerHaulerCreep) {
    ...
    if (
      !isInTargetRoom &&
      creep.store.getFreeCapacity() > 0 &&
      this.pickupResources(creep, RESOURCE_POWER)
    )
      return;
    ...
    if (creep.memory.pickupTarget) {
      ...
      if (!target) {
        ...
        (target.store.getUsedCapacity() === 0 ||
          (resourceType && (target.store[resourceType] || 0) === 0))
      ) {
