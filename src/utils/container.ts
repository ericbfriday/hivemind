...
type ContainerConstructors = {
  [key in keyof DependencyInjectionContainer]: (
    c: Container,
  ) => DependencyInjectionContainer[key];
};
...
export class Container {
  ...
  get<T extends keyof DependencyInjectionContainer>(
    key: T,
  ): DependencyInjectionContainer[T] {
    if (this.services[key]) return this.services[key];
    if (!this.constructors[key])
      throw 'Invalid service "' + key + '" requested';

    const instance = this.constructors[key](
      this,
    ) as DependencyInjectionContainer[T];
    this.services[key] = instance;
    return instance;
  }

  set<T extends keyof DependencyInjectionContainer>(
    key: T,
    generator: (c: Container) => DependencyInjectionContainer[T],
  ) {
