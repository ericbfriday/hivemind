...
export class PlacementManager {
  ...
  discouragePosition(x: number, y: number) {
    if (this.buildingMatrix.get(x, y) >= this.DISCOURAGED_POSITION || this.buildingMatrix.get(x, y) === this.ROAD_POSITION) {
      return;
    }
  ...
  isBuildableTile(x: number, y: number, allowRoads?: boolean, allowNearExit?: boolean): boolean {
    ...
    if (matrixValue === this.DISCOURAGED_POSITION && this.wallDistanceMatrix.get(x, y) <= 2) {
      return true;
    }
    ...
    if (matrixValue === this.ROAD_POSITION && !allowRoads) {
      return false;
    }
  ...
  startBuildingPlacement(origin?: RoomPosition, originEntrances?: RoomPosition[]) {
    ...
    if (originEntrances) {
      this.originEntrances = originEntrances;
    }
  ...
  getNextAvailableBuildSpot(): RoomPosition {
    while (_.size(this.openList) > 0) {
      ...
      handleMapArea(nextPos.x, nextPos.y, (x, y) => {
        ...
        if (this.openList[location] || this.closedList[location]) {
          return;
        }
      ...
      if (nextPos.getRangeTo(this.origin) < 3) {
        continue;
      }

      // Don't build on roads.
      if (!this.isBuildableTile(nextPos.x, nextPos.y)) {
        continue;
      }
  ...
  isPositionAccessible(pos: RoomPosition, noTunnels?: boolean) {
    ...
    if (noTunnels) {
      for (const position of result.path) {
        if (this.terrain.get(position.x, position.y) === TERRAIN_MASK_WALL && !this.roomPlan.hasPosition('road', position)) {
          return false;
        }
      }
    }
