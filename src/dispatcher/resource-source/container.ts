...
export class ContainerSource extends StructureSource<ContainerSourceTask> {
    ...
    private emptyEnergyFromContainers(options: ContainerSourceTask[], context: ResourceSourceContext) {
        ...
        for (const container of containers) {
            ...
            for (const source of container.room.sources) {
                if (source.getNearbyContainer()?.id !== container.id) {
                    continue;
                }
                ...
                // busy filling extensions.
                    && (this.room.energyAvailable >= this.room.energyCapacityAvailable || !this.room.storage || creep.memory.role !== 'transporter')) {
                    option.priority += 2;
                }
            ...
            for (const bay of container.room.bays) {
                if (bay.pos.getRangeTo(container.pos) > 0) {
                    continue;
                }
                if (!container.room.roomPlanner) {
                    continue;
                }
                if (!container.room.roomPlanner.isPlannedLocation(container.pos, 'harvester')) {
                    continue;
                }
    ...
    private getAssignedResourceType(container: StructureContainer): ResourceConstant | null {
        for (const mineral of this.room.minerals) {
            if (container.id !== mineral.getNearbyContainer()?.id) {
                continue;
            }
