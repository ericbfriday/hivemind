...
export default class OperatorRole extends Role {
  ...
  moveToTargetRoom() {
    ...
    if (
      this.creep.ticksToLive < POWER_CREEP_LIFE_TIME * 0.8 &&
      this.creep.room.powerSpawn
    ) {
      this.performRenew();
    ...
    const targetPosition = new RoomPosition(
      25,
      25,
      this.creep.memory.newTargetRoom,
    );
  ...
  interruptOrderIfNecessary() {
    cache.inHeap("interruptOrderIfNecessary:" + this.creep.name, 10, () => {
      ...
      if (bestOrder.canInterrupt && this.mayCurrentOrderBeInterrupted())
  ...
  mayCurrentOrderBeInterrupted(): boolean {
    ...
    if (
      targetId &&
      (Game.getObjectById(targetId)?.pos?.getRangeTo(this.creep) || 100) <= 5
    ) {
      return false;
    }
  ...
  addEnableRoomOptions(options: OperatorOrder[]) {
    if (
      ...
      this.creep.room.controller.isPowerEnabled
    )
      return;
  ...
  addRegenSourcesOptions(options: OperatorOrder[]) {
    ...
    each(this.creep.room.sources, (source: Source) => {
      ...
      options.push({
        ...
        priority: 3,
        weight:
          2 - (10 * ticksRemaining) / POWER_INFO[PWR_REGEN_SOURCE].duration,
      });
    });
  }
  ...
  addRegenMineralOptions(options: OperatorOrder[]) {
    ...
    for (const mineral of this.creep.room.minerals) {
      ...
      if (
        ticksRemaining >
        maxCooldown + POWER_INFO[PWR_REGEN_MINERAL].duration / 5
      )
        return;
      ...
      options.push({
        ...
        priority: 3,
        weight:
          1 - (5 * ticksRemaining) / POWER_INFO[PWR_REGEN_MINERAL].duration,
        canInterrupt: false,
      });
    }
  }
  ...
  addOperateStorageOptions(options: OperatorOrder[]) {
    ...
    if (
      (this.creep.store[RESOURCE_OPS] || 0) <
      POWER_INFO[PWR_OPERATE_STORAGE].ops
    )
      return;
    ...
    options.push({
      ...
      priority: 4,
      weight:
        1 - (5 * ticksRemaining) / POWER_INFO[PWR_OPERATE_STORAGE].duration,
    });
  }
  ...
  addOperateSpawnOptions(options: OperatorOrder[]) {
    ...
    if (
      (this.creep.store[RESOURCE_OPS] || 0) < POWER_INFO[PWR_OPERATE_SPAWN].ops
    )
      return;
    ...
    const spawn = find(
      ...
      (spawn) => {
        ...
        if ((memory.options || 0) < 2 && spawningTicks / totalTicks < 0.8)
          return false;
  ...
  addOperateFactoryOptions(options: OperatorOrder[]) {
    ...
    if (
      (this.creep.store[RESOURCE_OPS] || 0) <
      POWER_INFO[PWR_OPERATE_FACTORY].ops
    )
      return;
    ...
    options.push({
      ...
      priority: 4,
      weight:
        1 - (5 * ticksRemaining) / POWER_INFO[PWR_OPERATE_FACTORY].duration,
    });
  }
  ...
  addOperateExtensionOptions(options: OperatorOrder[]) {
    ...
    if (
      (this.creep.store[RESOURCE_OPS] || 0) <
      POWER_INFO[PWR_OPERATE_EXTENSION].ops
    )
      return;
    ...
    if (
      ...
      this.creep.room.energyCapacityAvailable * 0.8
    )
      return;
  ...
  addOperateTowerOptions(options: OperatorOrder[]) {
    ...
    if (
      (this.creep.store[RESOURCE_OPS] || 0) < POWER_INFO[PWR_OPERATE_TOWER].ops
    )
      return;
    if (this.creep.room.defense.getEnemyStrength() === ENEMY_STRENGTH_NONE)
      return;

    const towers: StructureTower[] =
      this.creep.room.myStructuresByType[STRUCTURE_TOWER];
    ...
    for (const tower of towers) {
      ...
      options.push({
        ...
        priority: 5,
        weight:
          1 - (5 * ticksRemaining) / POWER_INFO[PWR_OPERATE_TOWER].duration,
      });
    }
  }
  ...
  addDepositOpsOptions(options: OperatorOrder[]) {
    ...
    if (
      ...
      this.creep.store.getCapacity() * 0.9
    )
      return;

    const storage = this.creep.room.getBestStorageTarget(
      this.creep.store[RESOURCE_OPS],
      RESOURCE_OPS,
    );
    if (!storage) return;
  ...
  addRetrieveOpsOptions(options: OperatorOrder[]) {
    if (
      (this.creep.store[RESOURCE_OPS] || 0) >
      this.creep.store.getCapacity() * 0.1
    )
      return;
  ...
  performEnablePowers() {
    this.creep.whenInRange(1, this.creep.room.controller, () => {
      if (this.creep.enableRoom(this.creep.room.controller) === OK)
        this.orderFinished();
    });
  }
  ...
  usePower() {
    ...
    const target =
      order.target && Game.getObjectById<RoomObject & _HasId>(order.target);
  ...
  depositOps() {
    ...
    const amount = Math.min(
      Math.floor(this.creep.store.getCapacity() / 2),
      this.creep.store[RESOURCE_OPS] || 0,
      storage.store.getFreeCapacity(RESOURCE_OPS),
    );
    if (!amount) {
      this.orderFinished();
      return;
    ...
    this.creep.whenInRange(1, storage, () => {
      if (this.creep.transfer(storage, RESOURCE_OPS, amount) === OK)
        this.orderFinished();
    });
  }
  ...
  retrieveOps() {
    ...
    this.creep.whenInRange(1, storage, () => {
      if (this.creep.withdraw(storage, RESOURCE_OPS, amount) === OK)
        this.orderFinished();
    });
  }
  ...
  generateOps() {
    ...
    if (
      this.creep.room.isMine() &&
      this.creep.room.getCurrentResourceAmount(RESOURCE_OPS) > 15_000
    )
      return;
