...
export default class MineBuilderRole extends Role {
  ...
  run(creep: MineBuilderCreep) {
    ...
    if (!creep.memory.source) {
      if (creep.pos.roomName !== creep.memory.sourceRoom) {
        creep.interRoomTravel(
          new RoomPosition(25, 25, creep.memory.sourceRoom),
        );
      } else {
        creep.whenInRange(
          3,
  ...
  scoreHarvestPosition(creep: MineBuilderCreep, position: RoomPosition) {
    ...
    if (!operation || operation.isUnderAttack())
      return { position, work: -1000 };
  ...
  performReturnHome(creep: MineBuilderCreep) {
    ...
    if (
      container &&
      container.pos.roomName === creep.pos.roomName &&
  ...
  pickupNearbyEnergy(creep: MineBuilderCreep) {
    ...
    if (!resource) {
      ...
      const resources = creep.pos.findInRange(FIND_DROPPED_RESOURCES, 3, {
        filter: (resource) =>
          resource.resourceType === RESOURCE_ENERGY && resource.amount >= 100,
      });
    ...
    for (let i = pos - 2; i <= pos + 2; i++) {
      ...
      for (const structure of structures) {
        ...
        if (structure.hits < structure.hitsMax - workParts * REPAIR_POWER) {
          ...
          if (creep.repair(structure) === OK) {
            creep.operation.addResourceCost(
              workParts * REPAIR_COST * REPAIR_POWER,
              RESOURCE_ENERGY,
            );
          ...
          if (
            structure.hits <
            structure.hitsMax - workParts * 2 * REPAIR_POWER
          ) {
            return true;
          }
      ...
      if (
        ...
        if (
          sites.length === 0 &&
          numberSites < 5 &&
          position.createConstructionSite(STRUCTURE_ROAD) === OK
        ) {
  ...
  ensureRemoteHarvestContainerIsBuilt(creep: MineBuilderCreep) {
    ...
    if (creep.operation.hasContainer(creep.memory.source)) {
      ...
      if (container) {
        if (
          creep.pos.getRangeTo(container) > 3 ||
          container.hits > container.hitsMax - workParts * 100
        )
          return false;
    ...
    const containerPosition: RoomPosition =
      creep.operation.getContainerPosition(creep.memory.source);
  ...
  buildNearby(creep: MineBuilderCreep): boolean {
    const workParts = creep.getActiveBodyparts(WORK);
    const needsBuilding = creep.pos.findClosestByRange(
    ...
    if (needsBuilding && creep.pos.getRangeTo(needsBuilding) <= 3) {
      ...
      if (creep.build(needsBuilding) === OK) {
        const buildCost = Math.min(
          ...
          workParts * 5,
          needsBuilding.progressTotal - needsBuilding.progress,
        );
