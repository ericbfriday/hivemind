...
export class RoomPlanner {
  ...
  runLogic() {
    ...
    if (this.memory.lastRun && !hivemind.hasIntervalPassed(100, this.memory.lastRun)) {
      return;
    }
  ...
  initializeRoomPlanGenerationIfNecessary() {
    if (this.activeRoomPlan && this.activeRoomPlanVersion === this.roomPlannerVersion) {
      return;
    }
    if (this.generator) {
      return;
  ...
  continueRoomPlanGeneration() {
    ...
    if (cpuStart >= Game.cpu.tickLimit / 2) {
      return;
    }
