...
class Hivemind {
	 ...
	 */
	runProcess<P extends ProcessParameters>(
		id: string,
		ProcessConstructor: new (parameters: P) => ProcessInterface,
		options: P,
	) {
		...
		if (Game.cpu.getUsed() > Game.cpu.tickLimit * 0.85) {
			if (!this.emergencyBrakeProcessId) {
				...
				this.log("cpu").error(
					...
					"/",
					Game.cpu.tickLimit.toPrecision(3),
					"cpu used!",
				);
			}
	...
	timeProcess(id: string, stats: ProcessMemory, callback: () => void) {
		...
		this.memory.process[id].cpu =
			(this.memory.process[id].cpu || cpuUsage) * 0.99 + cpuUsage * 0.01;
	...
	isProcessAllowedToRun(
		...
		const throttleAt =
			options.throttleAt || priorityEffects[priority].throttleAt || 0;
	...
	getThrottleMultiplier(stopAt?: number, throttleAt?: number): number {
		...
		if (Game.cpu.getUsed() > minThrottle) {
			throttling /=
				1 - (Game.cpu.getUsed() - minThrottle) / (maxThrottle - minThrottle);
		}
	...
	migrateData(): boolean {
		...
		if (!this.memory.intelMigrated) {
			...
			each<[string, OutdatedRoomMemory]>(Object.entries(Memory.rooms as any as OutdatedRoomMemory), ([roomName, memory]) => {
		...
		const drawProcesses = function (parentId, indent) {
			each(processData[parentId], (data) => {
				...
				if (data.lastRun !== Game.time) {
					visual.text(Game.time - data.lastRun + " ago", 2, lineNumber, {
						align: "right",
						color: "#808080",
					});
				}
