...
export default DismantlerSpawnRole;
export class DismantlerSpawnRole extends SpawnRole {
  /**
  ...
  addRoomPlannerDismantlers(room: Room, options: DismantlerSpawnOption[]) {
    ...
    if (!room.roomManager.needsDismantling()) {
      return;
    }
  ...
  addDismantlerForOperation(room: Room, options: DismantlerSpawnOption[], operation: RemoteMiningOperation, sourceLocation: string) {
    if (!operation.needsDismantler(sourceLocation)) {
      return;
    }
