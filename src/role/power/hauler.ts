...
declare global {
  ...
  export interface PowerHaulerCreepMemory extends CreepMemory {
    role: 'hauler.power'
    delivering?: boolean
    isReturning?: boolean
    sourceRoom: string
  ...
  export interface PowerHaulerCreepHeapMemory extends CreepHeapMemory {
  }
}

export default PowerHaulerRole;
export class PowerHaulerRole extends Role {
  /**
  ...
  run(creep: PowerHaulerCreep) {
    ...
    if (!isInTargetRoom && this.pickupResources(creep, RESOURCE_POWER)) {
      return;
    }
  ...
  returnHome(creep: PowerHaulerCreep) {
    ...
    // Pick up dropped power in rooms we pass.
    if (!isInTargetRoom && creep.store.getFreeCapacity() > 0 && this.pickupResources(creep, RESOURCE_POWER)) {
      return;
    }
