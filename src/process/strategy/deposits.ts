...
export default class DepositMiningProcess extends Process {
  ...
  run() {
    ...
    _.each(memory.rooms, (info, roomName) => {
      ...
      if (
        ...
        (Memory.rooms[roomName].enemies.parts[WORK] || 0) > 0
      )
        return;
  ...
  getPotentialSpawns(
    targetRoom: string,
  ): Array<{ room: string; distance: number }> {
    ...
    for (const room of Game.myRooms) {
      ...
      if (
        room.getEffectiveAvailableEnergy() <
        hivemind.settings.get("minEnergyForDepositMining")
      )
        continue;
      if (
        room.controller.level < hivemind.settings.get("minRclForDepositMining")
      )
        continue;
      if (
        Game.map.getRoomLinearDistance(targetRoom, room.name) >
        hivemind.settings.get("maxRangeForDepositMining")
      )
        continue;

      // @todo Use actual position of power cache.
      const roomRoute = this.mesh.findPath(
        new RoomPosition(25, 25, room.name),
        new RoomPosition(25, 25, targetRoom),
      );
      if (
        roomRoute.incomplete ||
        roomRoute.path.length >
          3 * hivemind.settings.get("maxRangeForDepositMining")
      )
        continue;
