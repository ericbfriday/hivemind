...
export default class RemoteBuilderRole extends Role {
  ...
  run(creep: RemoteBuilderCreep) {
    ...
    if (creep.memory.targetRoom) {
      if (
        creep.interRoomTravel(new RoomPosition(25, 25, creep.memory.targetRoom))
      )
        return;
  ...
  performRemoteBuild() {
    ...
    if (spawns && spawns.length > 0) {
      const maySwitchToRefill =
        (!creep.memory.repairTarget && !creep.memory.buildTarget) ||
        creep.pos.getRangeTo(spawns[0].pos) < 5;
      if (maySwitchToRefill) {
        creep.whenInRange(1, spawns[0], () =>
    ...
    if (
      ...
      this.saveExpiringRamparts(10_000)
    )
      return;
    ...
    if (!creep.memory.buildTarget) {
      ...
      if (!creep.memory.buildTarget && !creep.memory.repairTarget) {
        if (
          this.creep.room.needsReclaiming() &&
          this.saveExpiringRamparts(hivemind.settings.get("minWallIntegrity"))
        )
          return;
  ...
  supplyTowers() {
    ...
    if (towers && towers.length > 0) {
      this.creep.whenInRange(1, towers[0], () =>
  ...
  saveExpiringRamparts(minHits: number): boolean {
    ...
    if (this.creep.memory.repairTarget) {
      let maxRampartHits = Math.max(
        minHits * 1.1,
        this.creep.room.controller.level < 6
          ? 15_000
          : hivemind.settings.get("minWallIntegrity") * 1.1,
        (this.creep.heapMemory.repairMinHits || 0) * 1.1,
      );
      if ((this.creep.room.controller.safeMode || 0) > 5000)
        maxRampartHits = 15_000;

      const target = Game.getObjectById<Structure>(
        this.creep.memory.repairTarget,
      );
      if (
        !target ||
        (target.structureType === STRUCTURE_RAMPART &&
          target.hits > maxRampartHits)
      ) {
        delete this.creep.memory.repairTarget;
        delete this.creep.heapMemory.repairMinHits;
      }
  ...
  determineBuildTarget() {
    ...
    if (this.creep.room.controller.level >= 6) {
      ...
      if (lowRamparts.length > 0) {
        this.creep.memory.repairTarget = minBy(lowRamparts, "hits").id;
      }
    }
  }
  ...
  performControllerUpgrade() {
    if (
      this.creep.room.controller.level === 0 ||
      !this.creep.room.isMine() ||
      this.creep.room.controller.upgradeBlocked
    ) {
      this.creep.memory.upgrading = false;
      return;
    }

    this.creep.whenInRange(3, this.creep.room.controller, () =>
  ...
  performGetRemoteBuilderEnergy() {
    ...
    const dropped = creep.pos.findClosestByPath(FIND_DROPPED_RESOURCES, {
      filter: (drop) =>
        ...
        (drop.amount > creep.store.getCapacity() * 0.3 ||
          (creep.pos.getRangeTo(drop) <= 1 && drop.amount > 20)),
    });
    if (dropped) {
    ...
    if (
      ...
      creep.whenInRange(1, creep.room.storage, () =>
    ...
    if (!creep.memory.resourceTarget) {
      ...
      const container = creep.pos.findClosestByPath(FIND_STRUCTURES, {
        filter: (structure) =>
          ...
          (structure.store.energy || 0) > 500,
      });
      if (
        container &&
        (creep.room.isMine() || !creep.room.controller.safeMode)
      ) {
  ...
  setExtraEnergyTarget(creep: RemoteBuilderCreep) {
    ...
    if (
      !creep.memory.extraEnergyTarget &&
      creep.store.getFreeCapacity() < creep.store.getCapacity() / 3
    ) {
      this.setBuilderState(true);
      return;
    }
  ...
  collectExtraEnergy() {
    ...
    const dropped = this.creep.pos.findClosestByPath(FIND_DROPPED_RESOURCES, {
      filter: (drop) =>
        drop.resourceType === RESOURCE_ENERGY &&
        (drop.amount > this.creep.store.getCapacity() * 0.3 ||
          (this.creep.pos.getRangeTo(drop) <= 1 && drop.amount > 20)),
    });
    if (dropped) {
    ...
    if (
      container &&
      (this.creep.room.isMine() || !this.creep.room.controller.safeMode)
    ) {
      this.creep.whenInRange(1, container, () =>
