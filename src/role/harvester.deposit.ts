...
export default class DepositHarvesterRole extends Role {
  ...
  run(creep: DepositHarvesterCreep) {
    ...
    if (creep.memory.delivering && creep.store.getUsedCapacity() === 0) {
      ...
      creep.store.getUsedCapacity() > creep.store.getCapacity() * 0.95
    ) {
      this.setDelivering(creep, true);
    } else if (
      !creep.memory.delivering &&
      creep.pos.roomName == targetPosition.roomName &&
      creep.pos.getRangeTo(targetPosition) < 3 &&
      creep.ticksToLive <= 1.1 * this.getReturnTravelTime(creep)
    ) {
      this.setDelivering(creep, true);
    }
  ...
  getReturnTravelTime(creep: DepositHarvesterCreep): number {
    ...
    creep.heapMemory.returnTravelTime = cache.inHeap(
      "returnTravel:" + creep.memory.targetPos + ":" + creep.memory.origin,
      10_000,
      () => {
        ...
        const path = mesh.findPath(
          targetPosition,
          new RoomPosition(25, 25, creep.memory.origin),
        );
        if (path.incomplete) {
          creep.heapMemory.returnTravelTime =
