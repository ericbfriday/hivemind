...
export default class OwnedRoomProcess extends Process {
  ...
  run() {
    ...
    const totalTime = timeCall("operation:" + operationName, () => {
      ...
      const prioritizeRoomManager =
        this.room.roomManager.shouldRunImmediately();
      hivemind.runSubProcess("rooms_manager", () => {
        hivemind.runProcess(this.room.name + "_manager", RoomManagerProcess, {
          ...
          room: this.room,
          priority: prioritizeRoomManager
            ? PROCESS_PRIORITY_ALWAYS
            : PROCESS_PRIORITY_DEFAULT,
        });
      });
      ...
      hivemind.runSubProcess("rooms_power", () => {
        ...
        if (
          ...
          powerSpawn.my &&
          powerSpawn.power > 0 &&
          powerSpawn.energy >= POWER_SPAWN_ENERGY_RATIO &&
          powerSpawn.processPower() === OK
        )
      ...
      hivemind.runSubProcess("rooms_observers", () => {
        ...
        if (
          this.room.observer &&
          this.room.memory.observeTargets &&
          this.room.memory.observeTargets.length > 0
        ) {
  ...
  gatherStats() {
    ...
    if (
      !memory.tower &&
      (this.room.myStructuresByType[STRUCTURE_TOWER] || []).length > 0
    ) {
