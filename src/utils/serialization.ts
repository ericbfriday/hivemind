...
function serializePosition(position: RoomPosition, fixedRoom?: string) {
  return serializeCoords(
    position.x,
    position.y,
    position.roomName === fixedRoom ? null : position.roomName,
  );
}

function deserializePosition(
  coords: string | number | [number, string],
  fixedRoom?: string,
) {
  // Fallback for old string positions.
  ...
  return map(path, (location) => {
    pos =
      typeof location === "string"
        ...
        : new RoomPosition(
            pos.x + xOffsets[location],
            pos.y + yOffsets[location],
            pos.roomName,
          );
