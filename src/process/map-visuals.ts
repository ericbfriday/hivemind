...
export default class MapVisualsProcess extends Process {
  ...
  drawIntelStatus(roomName: string) {
    ...
    Game.map.visual.text("•", new RoomPosition(3, 3, roomName), {
      color,
      fontSize: 10,
    });
  }
  ...
  drawRoomStatus(roomName: string) {
    ...
    if (drawIntelStatus && scoutPriority > 0) {
      Game.map.visual.text(
        scoutPriority.toPrecision(2),
        new RoomPosition(3, 23, roomName),
        { fontSize: 5 },
      );
      Game.map.visual.line(
        new RoomPosition(25, 25, roomName),
        new RoomPosition(25, 25, this.roomStatus.getOrigin(roomName)),
        { opacity: 0.5, width: 1 },
      );
    }
    ...
    Game.map.visual.text(
      expansionScore.toPrecision(3),
      new RoomPosition(8, 4, roomName),
  ...
  drawExpansionStatus() {
    ...
    for (const attempt of Memory.strategy.expand.failedExpansions || []) {
      Game.map.visual.circle(new RoomPosition(25, 25, attempt.roomName), {
        fill: "#ff0000",
        opacity: 0.2,
        radius: 25,
      });
    ...
    if (Memory.strategy.expand.currentTarget) {
      ...
      for (const roomName of targetMemory.supportingRooms || []) {
        Game.map.visual.line(
          new RoomPosition(25, 25, roomName),
          targetPosition,
          { opacity: 0.35, lineStyle: "dashed" },
        );
      }

      Game.map.visual.line(
        new RoomPosition(25, 25, targetMemory.spawnRoom),
        targetPosition,
        { opacity: 0.5 },
      );
  ...
  drawRemoteMinePaths(roomName: string) {
    ...
    Game.map.visual.text(
      harvestPriority.toPrecision(3),
      new RoomPosition(7, 3, roomName),
  ...
  drawNavMesh() {
    ...
    _.each(Memory.nav.rooms, (navInfo, roomName) => {
      ...
      for (const portal of navInfo.portals || []) {
        Game.map.visual.line(
          new RoomPosition(25, 25, portal.room),
          new RoomPosition(25, 25, roomName),
          { ...style, color: "#8080ff", width: 1 },
        );
      }
      ...
      if (!navInfo.regions) {
        ...
        for (const exit of navInfo.exits) {
          Game.map.visual.line(
            new RoomPosition(25, 25, roomName),
            deserializePosition(exit.center, roomName),
            style,
          );
        }
