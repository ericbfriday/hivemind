...
export interface AvailableBoosts extends Partial<Record<ResourceConstant, {
  effect: number
  available: number
}>> {};
...
export class BoostManager {
  ...
  public getRequiredBoostAmount(labId: Id<StructureLab>): number {
    if (!this.isLabUsedForBoosting(labId)) {
      return 0;
    }
  ...
  public overrideCreepLogic(creep: Creep): boolean {
    if (!this.creepNeedsBoosting(creep)) {
      return false;
    }

    // @todo If the creep has claim parts, it cannot be boosted.
    if ((creep.ticksToLive || 0) < CREEP_LIFE_TIME * 0.7) {
  ...
  public getAvailableBoosts(type: string): AvailableBoosts {
    const availableBoosts = cache.inObject(
      ...
      () => {
        ...
        if ((storage.store[RESOURCE_ENERGY] || 0) + (terminal.store[RESOURCE_ENERGY] || 0) < 2500) {
          return boosts;
        }
  ...
  private boostCreepAtLab(creep: Creep, lab: StructureLab) {
    ...
    const amount = this.memory.creeps[creep.name][resourceType] as number;

    creep.whenInRange(1, lab, () => {
      if (lab.mineralType !== resourceType) {
        return;
      }
      if (lab.store.getUsedCapacity(resourceType) < amount * LAB_BOOST_MINERAL) {
        return;
      }
      if (lab.store.getUsedCapacity(RESOURCE_ENERGY) < amount * LAB_BOOST_ENERGY) {
        return;
      }
      if (lab.hasBoostedThisTick) {
        return;
      }
  ...
  private ensureLabIsStillNeededForBoosting(lab: StructureLab) {
    const resourceType = this.memory.labs[lab.id];
    const needsThisBoost = _.find(Object.entries(this.memory.creeps), ([creepName, boosts]) => boosts[resourceType] && Game.creeps[creepName]?.pos?.roomName === this.room.name);

    if (!needsThisBoost) {
      delete this.memory.labs[lab.id];
    }
  }
  ...
  private getNumberOfPartsToBoost(labId: Id<StructureLab>): number {
    ...
    return _.sumBy(Object.entries(this.memory.creeps), (it) => {
      const [creepName, rawBoosts] = it;
      const boosts = Object.values(rawBoosts);
      if (Game.creeps[creepName]?.room?.name !== this.room.name) {
        return 0;
      }
  ...
  private creepNeedsBoosting(creep: Creep) {
    if (this.memory.creeps[creep.name]) {
      return true;
    }
  ...
  private getMostPreparedLab(boosts: Partial<Record<ResourceConstant, number>>): StructureLab {
    ...
    let best: StructureLab | undefined;
    let bestScore = 0;
    for (const lab of labs) {
      const resourceType = this.memory.labs[lab.id];
      const energyScore = Math.min(lab.store.energy / LAB_BOOST_ENERGY / boosts[resourceType]!, 1);
      const mineralScore = 3 * Math.min(lab.store.getUsedCapacity(resourceType) as number / LAB_BOOST_MINERAL / boosts[resourceType]!, 1);
  ...
  private ensureBoostsHaveLabAssigned(boosts: Partial<Record<ResourceConstant, number>>) {
    ...
    for (const resourceType in boosts) {
      ...
      for (const lab of roomLabs) {
        if (lab.id === this.room.memory.labs?.source1) {
          continue;
        }
        if (lab.id === this.room.memory.labs?.source2) {
          continue;
        }
