...
export interface FactorySourceTask extends StructureSourceTask {
  type: 'factory'
  target: Id<AnyStoreStructure>
}

export default FactorySource;
export class FactorySource extends StructureSource<FactorySourceTask> {
  ...
  addEmptyFactoryTasks(options: FactorySourceTask[], context: ResourceSourceContext) {
    ...
    for (const resourceType of getResourcesIn(this.room.factory.store)) {
      ...
      if (neededResources[resourceType] && this.room.factory.store.getUsedCapacity(resourceType) <= neededResources[resourceType] * 1.5) {
        continue;
      }
