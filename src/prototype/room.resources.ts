...
declare global {
    ...
    export interface RoomResourceState {
    ...
    export interface StructureKeeperLair {
        isDangerous: () => boolean
    }
}
...
Room.prototype.isFullOn = function (this: Room, resourceType: ResourceConstant) {
    if (resourceType === RESOURCE_ENERGY) {
        return this.isFullOnEnergy();
    }
    if (resourceType === RESOURCE_POWER) {
        return this.isFullOnPower();
    }
...
Room.prototype.getStorageLocation = function (this: Room) {
    ...
    if (this.roomPlanner) {
        return this.roomPlanner.getRoomCenter();
    }
    ...
    return cache.inHeap(`remoteSourcePositions:${this.name}`, 500, () => {
        ...
        _.each(Game.operationsByType.mining, (operation) => {
            ...
            _.each(locations[this.name], (location) => {
                if (!operation.getPaths()[location]?.path) {
                    return;
                }
...
function getRemoteHarvestSourceEvaluation(operation: RemoteMiningOperation, location: string): SourceEvaluation {
    ...
    return {
        ...
        averageDistance: _.sumBy(filteredPaths, path => path.path.length) / _.size(filteredPaths),
    };
}
...
StructureKeeperLair.prototype.isDangerous = function (this: StructureKeeperLair) {
    if (_.some(this.room.enemyCreeps['Source Keeper'], c => c.pos.getRangeTo(this) <= 5)) {
        return true;
    }
 ...
 */
function isDangerous(this: Source | Mineral): boolean {
