...
export default class BrawlerSpawnRole extends SpawnRole {
  ...
  getRemoteDefenseSpawnOptions(room: Room, options: BrawlerSpawnOption[]) {
    ...
    const harvestPositions: RoomPosition[] =
      room.getRemoteHarvestSourcePositions();
    for (const pos of harvestPositions) {
      ...
      if (
        room.memory.recentBrawler &&
        Game.time - (room.memory.recentBrawler[targetPos] || -10_000) <
          defenseTimeDiff
      )
        continue;
  ...
  getPowerHarvestDefenseSpawnOptions(
    room: Room,
    options: BrawlerSpawnOption[],
  ) {
  ...
  getDefenseCreepSize(room: Room, enemyData: EnemyData): number {
    ...
    const smallDefenderPower =
      defaultAttack * ATTACK_POWER +
      defaultHeal * HEAL_POWER * meleeWithHealValue;
    const smallBlinkyPower =
      defaultBlinkyAttack * RANGED_ATTACK_POWER +
      defaultBlinkyHeal * HEAL_POWER * healValue;
    ...
    if (
      ...
      numberBlinkyRanged * RANGED_ATTACK_POWER +
        numberBlinkyHeal * HEAL_POWER * healValue
    ) {
    ...
    if (
      !isRangedEnemy &&
      enemyPower <
        numberTrainAttack * ATTACK_POWER +
          numberBlinkyRanged * RANGED_ATTACK_POWER +
          numberBlinkyHeal * HEAL_POWER * healValue
    ) {
    ...
    if (
      enemyPower <
      (numberTrainRanged + numberBlinkyRanged) * RANGED_ATTACK_POWER +
        numberBlinkyHeal * HEAL_POWER * healValue
    ) {
    ...
    if (
      enemyPower <
      2 * numberBlinkyRanged * RANGED_ATTACK_POWER +
        2 * numberBlinkyHeal * HEAL_POWER * healValue
    ) {
    ...
    if (
      enemyPower <
      numberTrainRanged * RANGED_ATTACK_POWER +
        numberTrainHeal * HEAL_POWER * healValue
    ) {
    ...
    if (
      enemyPower <
      numberBlinkyRanged * RANGED_ATTACK_POWER +
        (numberTrainHeal + numberBlinkyHeal) * HEAL_POWER * healValue
    ) {
  ...
  canReclaimRoom(targetRoom: Room, room: Room): boolean {
    ...
    const remoteDefense = filter(
      ...
      (creep: Creep) =>
        creep.memory.target ===
        encodePosition(targetRoom.roomPlanner.getRoomCenter()),
    ).length;
    if (remoteDefense > 3) return false;

    const route = cache.inHeap(
      "reclaimPath:" + targetRoom.name + "." + room.name,
      100,
      () => {
        if (!this.navMesh) this.navMesh = new NavMesh();
        return this.navMesh.findPath(
          room.roomPlanner.getRoomCenter(),
          targetRoom.roomPlanner.getRoomCenter(),
          { maxPathLength: 700 },
        );
      },
    );
    ...
    if (option.responseType) {
      switch (option.responseType) {
        ...
        case RESPONSE_FULL_BRAWLER:
          return this.getBrawlerCreepBody(
            room,
            option.responseType === RESPONSE_MINI_BRAWLER ? 5 : null,
          );
        ...
        case RESPONSE_BLINKY_HEAL_TRAIN:
          return this.getBlinkyCreepBody(
            room,
            option.responseType === RESPONSE_MINI_BLINKY ? 6 : null,
          );
  ...
  onSpawn(
    ...
    if (option.trainStarter) {
      ...
      creep.memory.train.partsToSpawn =
        creep.memory.train.partsToSpawn.slice(1);
    }
    ...
    hivemind
      ...
      .info("Spawning new brawler", name, "to defend", position);
  }
}
