...
export default RoomVariationBuilderBase;
export class RoomVariationBuilderBase {
  ...
  placeBayStructures(bayPosition: RoomPosition, options: { spawn?: boolean, source?: boolean, id?: number } = {}) {
    ...
    if (this.roomPlan.canPlaceMore('spawn') && options.spawn) {
      handleMapArea(bayPosition.x, bayPosition.y, (x, y) => {
        ...
        if (x === bayPosition.x && y === bayPosition.y) {
          return true;
        }
        ...
        handleMapArea(x, y, (x2, y2) => {
          if (x2 == bayPosition.x && y2 == bayPosition.y) {
            return true;
          }
          if (!this.roomPlan.hasPosition('road', new RoomPosition(x2, y2, this.roomName))) {
            return true;
          }
