...
export default class TerminalDestination extends StructureDestination<TerminalDestinationTask> {
  ...
  addMinimumEnergyDestination(
    options: TerminalDestinationTask[],
  ...
  addTransferResourceDestination(
    options: TerminalDestinationTask[],
    context: ResourceDestinationContext,
  ) {
    ...
    const missingAmount =
      targetAmount - terminal.store.getUsedCapacity(resourceType);
  ...
  addResourcesForSaleDestination(
    options: TerminalDestinationTask[],
    context: ResourceDestinationContext,
  ) {
    const terminal = this.room.terminal;
    ...
    for (const order of roomSellOrders) {
      ...
      if (
        this.room.getCurrentResourceAmount(resourceType) < order.remainingAmount
      )
        continue;
      if (
        terminal.store.getFreeCapacity(resourceType) <
        order.remainingAmount - terminal.store[resourceType]
      )
        continue;
      ...
      options.push({
        ...
        weight:
          ...
          terminal.store[resourceType] / order.remainingAmount,
        type: this.getType(),
        target: terminal.id,
