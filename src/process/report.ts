...
export default class ReportProcess extends Process {
  ...
  initMemory(baseTimestamp: number) {
    Memory.strategy.reports = {
      nextReportTime: this.normalizeDate(
        new Date(baseTimestamp + 24 * 60 * 60 * 1000),
      ).getTime(),
      data: {
        time: Game.time,
        gcl: Game.gcl,
        ...
        remoteHarvestCount:
          Memory.strategy?.remoteHarvesting?.currentCount || 0,
        cpu: {},
      },
    };
  ...
  generateLevelReport(variable: string, label: string) {
    ...
    reportText +=
      "\nProgress: " +
      (
        (100 * currentValues.progress) /
        currentValues.progressTotal
      ).toPrecision(3) +
      "% (+" +
  ...
  generatePowerReport() {
    ...
    reportText +=
      ...
      this.getStoredPower() +
      " (+" +
      (this.getStoredPower() - (this.memory.data.storedPower || 0)) +
      ")";
  ...
  generateRemoteMiningReport() {
    ...
    if (
      Memory.strategy.remoteHarvesting.currentCount >
      this.memory.data.remoteHarvestCount
    ) {
      reportText +=
        " (+" +
        (Memory.strategy.remoteHarvesting.currentCount -
  ...
  generateMiningOperationsReport() {
    ...
    reportText += "<pre>";
    reportText += this.formatSignificantEntries(
      operationScores,
      (o, index) => index + 1 + ". " + o.name + " - " + o.score.toPrecision(3),
    ).join("\n");
    reportText += "</pre>";
