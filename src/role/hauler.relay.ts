...
declare global {
  ...
  interface RelayHaulerCreepHeapMemory extends CreepHeapMemory {
    ...
    pickupTarget?: Id<
      Resource | Tombstone | Ruin | StructureContainer | ScoreContainer
    >;
  }
}
...
export default class RelayHaulerRole extends Role {
  ...
  determineTargetSource(creep: RelayHaulerCreep) {
    const harvestPositions =
      Game.rooms[creep.memory.sourceRoom].getRemoteHarvestSourcePositions();
    ...
    const bestPosition = maxBy(scoredPositions, "energy");
  ...
  scoreHarvestPosition(creep: RelayHaulerCreep, position: RoomPosition) {
    ...
    const maxHarvesterLifetime = maxBy(
      filter(
    ...
    const projectedIncomeDuration = Math.min(
      maxHarvesterLifetime,
      path.travelTime,
    );
    const sourceMaxEnergy = operation.canReserveFrom(creep.memory.sourceRoom)
      ? SOURCE_ENERGY_CAPACITY
      : SOURCE_ENERGY_NEUTRAL_CAPACITY;
    const projectedIncome = operation.hasContainer(targetPos)
      ? (projectedIncomeDuration * sourceMaxEnergy) / ENERGY_REGEN_TIME
      : 0;

    const queuedHaulerCapacity = sumBy(
      filter(
    ...
    const queuedBuilderCapacity = sumBy(
      filter(
  ...
  getPath(creep: RelayHaulerCreep): RoomPosition[] | null {
    ...
    if (!paths[creep.memory.source] || !paths[creep.memory.source].accessible)
      return null;
  ...
  performDeliver(creep: RelayHaulerCreep) {
    ...
    if (this.combatManager.needsToFlee(creep)) {
      this.combatManager.performFleeTowards(
        creep,
        room.getStorageLocation(),
        5,
      );
      return;
    }
    ...
    if (creep.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
      const creeps = creep.pos.findInRange(FIND_MY_CREEPS, 1, {
        filter: (creep) =>
          ["builder", "builder.remote", "builder.mines", "upgrader"].includes(
            creep.memory.role,
          ) &&
  ...
  getDeliveryTarget(creep: RelayHaulerCreep) {
    ...
    const target = Game.rooms[creep.memory.sourceRoom].getBestStorageTarget(
  ...
  storeResources(creep: RelayHaulerCreep, target?: AnyStoreStructure) {
    ...
    if (
      ...
      if (
        !creep.heapMemory.order ||
        !creep.room.destinationDispatcher.validateTask(creep.heapMemory.order, {
          creep,
        })
      ) {
      ...
      if (creep.heapMemory.order) {
        creep.room.destinationDispatcher.executeTask(creep.heapMemory.order, {
          creep,
        });
        return;
      }
    ...
    if (
      !target ||
      creep.store.getUsedCapacity() > target.store.getFreeCapacity()
    ) {
    ...
    creep.whenInRange(1, target, () => {
      for (const resourceType of getResourcesIn(creep.store)) {
        if (creep.transfer(target, resourceType) === OK) {
          creep.operation?.addResourceGain(
  ...
  performPickup(creep: RelayHaulerCreep) {
    ...
    if (container) {
      ...
      if (
        !hasActiveHarvester &&
        container.store.getUsedCapacity(RESOURCE_ENERGY) < 20
      ) {
        this.startDelivering(creep);
        return;
      }
  ...
  getSource(creep: RelayHaulerCreep): Source {
    ...
    return creep.room.find(FIND_SOURCES, {
      filter: (source) =>
        source.pos.x === sourcePosition.x && source.pos.y === sourcePosition.y,
    })[0];
  }
    ...
    if (creep.heapMemory.pickupTarget) {
      ...
      const container = creep.pos.findInRange(FIND_STRUCTURES, 1, {
        filter: (structure) =>
          ...
          structure.store.getCapacity() * 0.1 &&
