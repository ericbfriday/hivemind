...
export default class ExpandProcess extends Process {
  ...
  getCpuStats() {
    const harvestRooms = Memory.strategy.remoteHarvesting
      ? Memory.strategy.remoteHarvesting.currentCount
      : 0;
    ...
    const longTermCpuUsage = stats.getStat("cpu_total", 10_000)
      ? stats.getStat("cpu_total", 10_000) / Game.cpu.limit
      : shortTermCpuUsage;
  ...
  mayExpand(): boolean {
    ...
    const mayHaveMoreRooms =
      !maxRooms || (shardMemory.info && shardMemory.info.ownedRooms < maxRooms);

    const cpuStats = this.getCpuStats();

    return (
      hasFreeControlLevels &&
      mayHaveMoreRooms &&
  ...
  chooseNewExpansionTarget() {
    ...
    if (expansionTargetScoringProgress) {
      ...
      if (bestTarget)
        modifiedBestExpansionScore = this.getModifiedExpansionScore(
          bestTarget.roomName,
          bestTarget.expansionScore,
        );
    } else {
    ...
    for (const roomName of this.roomStatus.getPotentialExpansionTargets()) {
      ...
      if (
        ...
        settings.get("maxExpansionCpuPerTick")
      ) {
        ...
        hivemind
          ...
          .debug(
            "Current best target:",
            bestTarget ? bestTarget.roomName : "N/A",
            "@",
            bestTarget ? modifiedBestExpansionScore : "N/A",
          );
        return;
      }
      ...
      if (expansionScore === 0) continue;

      const modifiedExpansionScore = this.getModifiedExpansionScore(
        roomName,
        expansionScore,
      );
  ...
  startExpansion(roomInfo) {
    ...
    hivemind
      .log("strategy")
      .notify("🏴 Started expanding to " + roomInfo.roomName);
  }
    ...
    if (Game.rooms[info.roomName]) {
      ...
      if (room.controller.my) {
        ...
        if (
          ...
          room.controller.reservation.username !== getUsername() &&
          room.controller.reservation.ticksToEnd > 100
        ) {
          squad.setUnitCount("singleClaim", 0);
          squad.setUnitCount(
            "claimer",
            Math.ceil(room.controller.reservation.ticksToEnd / 2000),
          );
        } else {
          squad.setUnitCount("singleClaim", 1);
          squad.setUnitCount("claimer", 0);
  ...
  hasExpansionFailed(): boolean {
    ...
    if (
      !this.memory.claimed &&
      Game.time - this.memory.started > 5 * CREEP_LIFE_TIME
    )
      return true;
    ...
    if (
      this.memory.claimed &&
      Game.time - this.memory.claimed > 20 * CREEP_LIFE_TIME
    )
      return true;
  ...
  manageExpansionSupport() {
    ...
    for (const room of Game.myRooms) {
      ...
      const path = cache.inHeap(
        "spawnAssistPath:" + info.roomName + ":" + room.name,
        2000,
        () =>
  ...
  checkClaimPath() {
    ...
    for (const creep of creeps) {
      ...
      const tunnelPath = PathFinder.search(
        creep.pos,
        [{ pos: room.controller.pos, range: 1 }],
        {
          maxRooms: 1,
  ...
  checkAccessPath() {
    ...
    if (
      !originRoom ||
      (this.memory.pathBlocked &&
        Game.time - this.memory.pathBlocked > 5 * CREEP_LIFE_TIME)
    ) {
  ...
  findClosestSpawn(targetRoom: string): string {
    ...
    for (const room of Game.myRooms) {
      ...
      const path = cache.inHeap(
        "spawnPath:" + targetRoom + ":" + room.name,
        1000,
        () =>
          this.navMesh.findPath(
            new RoomPosition(25, 25, room.name),
  ...
  abandonWeakRooms() {
    ...
    const hasBetterExpansion =
      shardMemory.info.rooms.bestExpansion.score -
        shardMemory.info.rooms.worstRoom.score >=
      0.5;
    const maxRooms = settings.get("maxOwnedRooms");
  ...
  abandonStripmines() {
    ...
    for (const room of Game.myRooms) {
      ...
      hivemind
        .log("strategy")
        .notify(
          "💀 Evacuating " + room.name + " to mine another room instead.",
        );
      return;
    }
  }
  ...
  manageEvacuation() {
    ...
    if (
      ...
      room.terminal.store.getFreeCapacity() >
        room.nuker.store.getUsedCapacity(RESOURCE_ENERGY)
    ) {
      room.nuker.destroy();
      return;
    }
    ...
    const filledRuins = room.find(FIND_RUINS, {
      filter: (ruin) => ruin.store.getUsedCapacity() > 100,
    });
