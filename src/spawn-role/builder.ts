...
export default class BuilderSpawnRole extends SpawnRole {
  ...
  getSpawnOptions(room: Room): BuilderSpawnOption[] {
    return this.cacheEmptySpawnOptionsFor(room, 50, () => {
      ...
      const needsStrongerRamparts =
        room.terminal &&
        this.getLowestRampartValue(room) < 3_000_000 &&
        availableEnergy > 10_000;
  ...
  getNeededWorkParts(room: Room): number {
    const numberConstructionSites = room.find(
    ...
    if (
      numberConstructionSites === 0 &&
      room.memory.noBuilderNeeded &&
      Game.time - room.memory.noBuilderNeeded < 1500 &&
      hasStorage
    ) {
      return 0;
    }
  ...
  getLowestRampartValue(room: Room): number {
    return cache.inHeap("lowestRampart:" + room.name, 100, () => {
      ...
      return minBy(ramparts, "hits").hits;
    });
  }
