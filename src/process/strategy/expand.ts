...
export interface ExpandProcessMemory {
  started?: number
  claimed?: number
  currentTarget: {
    roomName: string
...
export default ExpandProcess;
export class ExpandProcess extends Process {
  ...
  run() {
    ...
    if (!stats.getStat('cpu_total', 1000)) {
      return;
    }
  ...
  chooseNewExpansionTarget() {
    ...
    if (expansionTargetScoringProgress) {
      ...
      if (bestTarget) {
        modifiedBestExpansionScore = this.getModifiedExpansionScore(bestTarget.roomName, bestTarget.expansionScore);
      }
    }
    else {
    ...
    for (const roomName of this.roomStatus.getPotentialExpansionTargets()) {
      ...
      if (Game.cpu.getUsed() - startTime >= settings.get('maxExpansionCpuPerTick')) {
        ...
        // a target each tick.
        hivemind.log('strategy').debug('Suspended trying to find expansion target.', _.size(expansionTargetScoringProgress.rooms), '/', this.roomStatus.getPotentialExpansionTargets().length, 'rooms checked so far.');
      ...
      if (expansionTargetScoringProgress.rooms[roomName]) {
        continue;
      }
      ...
      expansionTargetScoringProgress.rooms[roomName] = true;
      const expansionScore = this.roomStatus.getExpansionScore(roomName) || 0;
      if (expansionScore === 0) {
        continue;
      }

      const modifiedExpansionScore = this.getModifiedExpansionScore(roomName, expansionScore);
      if (bestTarget && modifiedBestExpansionScore >= modifiedExpansionScore) {
        continue;
      }
      ...
      bestTarget = { spawnRoom: bestSpawn, roomName, expansionScore };
   ...
   * the target room.
   */
  getModifiedExpansionScore(roomName: string, expansionScore: number): number {
    let score = expansionScore;
    ...
    for (const failedAttempt of this.memory.failedExpansions || []) {
      ...
      if (elapsedTicks > 1_000_000) {
        continue;
      }
      if (elapsedTicks > 100_000) {
        multiplier = 1 - ((elapsedTicks - 100_000) / 900_000);
      }
  ...
  recordFailedExpansion() {
    ...
    if (!this.memory.failedExpansions) {
      this.memory.failedExpansions = [];
    }
  ...
  manageExpansionSupport() {
    ...
    for (const room of Game.myRooms) {
      ...
      if ((room.structuresByType[STRUCTURE_SPAWN] || []).length === 0) {
        continue;
      }
      if (room.name === info.spawnRoom || room.name === info.roomName) {
        continue;
      }
      if (room.getEffectiveAvailableEnergy() < 50_000) {
        continue;
      }
  ...
  abandonWeakRooms() {
    ...
    if (this.memory.evacuatingRoom) {
      return;
    }

    // Only give up a room if we need more CPU.
    const cpuStats = this.getCpuStats();
    if (cpuStats.shortTermCpuUsage < cpuStats.cpuLimit) {
      return;
    }
    if (cpuStats.longTermCpuUsage < cpuStats.cpuLimit) {
      return;
    }
    ...
    if (shardMemory.info.ownedRooms && shardMemory.info.ownedRooms < 2) {
      return;
    }
    if (!shardMemory.info.rooms) {
      return;
    }
    if (!shardMemory.info.rooms.bestExpansion) {
      return;
    }
    if (!shardMemory.info.rooms.worstRoom) {
      return;
    }
    ...
    if (!hasBetterExpansion && !hasTooManyRooms) {
      return;
    }
  ...
  manageEvacuation() {
    ...
    if (!room || !room.isMine()) {
      ...
      if (Game.time < this.memory.evacuatingRoom.cooldown) {
        return;
      }
    ...
    if (room.terminal && room.terminal.store.getUsedCapacity() > 10_000) {
      return;
    }
    if (room.terminal && room.terminal.store.getUsedCapacity() > 0 && room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) / room.terminal.store.getUsedCapacity() < 0.8) {
      return;
    }

    const filledRuins = room.find(FIND_RUINS, { filter: ruin => ruin.store.getUsedCapacity() > 100 });
    if (filledRuins?.length > 0) {
      return;
    }
  ...
  private cleanupMemory() {
    if (!hivemind.hasIntervalPassed(5000, lastCleanup)) {
      return;
    }
    ...
    for (let i = this.memory.failedExpansions.length - 1; i >= 0; i--) {
      if (Game.time - this.memory.failedExpansions[i].time < 1_000_000) {
        continue;
      }
...
globalThis.ExpandProcess = ExpandProcess;
