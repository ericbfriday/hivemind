...
import {
  getRoomPlanFor,
  setRoomPlanFor,
} from "room/planner/room-plan-management";
...
export default class RoomPlanner {
  ...
  constructor(roomName: string) {
    ...
    if (
      this.activeRoomPlan &&
      (hivemind.settings.get("visualizeRoomPlan") ||
        (this.memory.drawDebug || 0) > 0)
    ) {
  ...
  runLogic() {
    ...
    if (
      this.memory.lastRun &&
      !hivemind.hasIntervalPassed(100, this.memory.lastRun)
    )
      return;
  ...
  initializeRoomPlanGenerationIfNecessary() {
    if (
      this.activeRoomPlan &&
      this.activeRoomPlanVersion === this.roomPlannerVersion
    )
      return;
    if (this.generator) return;
  ...
  startRoomPlanGeneration() {
    this.generator = new RoomPlanGenerator(
      this.roomName,
      this.roomPlannerVersion,
    );
  ...
  continueRoomPlanGeneration() {
    ...
    const cpuUsage =
      stats.getStat("cpu_total", 1000) || stats.getStat("cpu_total", 10) || 0;
    const cpuLimit = Math.min(
      Game.cpu.tickLimit / 2,
      0.8 * Math.max(Game.cpu.limit - cpuUsage, Game.cpu.limit / 10),
    );

    while (
      !this.isPlanningFinished() &&
      Game.cpu.getUsed() - cpuStart < cpuLimit
    ) {
  ...
  applyGeneratedRoomPlan() {
    ...
    hivemind
      ...
      .info("Stored room plan for room " + this.roomName);
  ...
  checkAdjacentRooms() {
    ...
    for (const dir in newStatus.directions) {
      if (newStatus.directions[dir] !== this.memory.adjacentSafe[dir]) {
        ...
        hivemind
          .log("room plan", this.roomName)
          .debug("changed adjacent room status!");
        hivemind
          .log("room plan", this.roomName)
          .notify(
            "🛡 Exit safety has changed!\n\n" +
              "N: " +
              (this.memory.adjacentSafe.N ? "safe" : "not safe") +
              " -> " +
              (newStatus.directions.N ? "safe" : "not safe") +
              "\n" +
              "E: " +
              (this.memory.adjacentSafe.E ? "safe" : "not safe") +
              " -> " +
              (newStatus.directions.E ? "safe" : "not safe") +
              "\n" +
              "S: " +
              (this.memory.adjacentSafe.S ? "safe" : "not safe") +
              " -> " +
              (newStatus.directions.S ? "safe" : "not safe") +
              "\n" +
              "W: " +
              (this.memory.adjacentSafe.W ? "safe" : "not safe") +
              " -> " +
              (newStatus.directions.W ? "safe" : "not safe") +
              "\n",
          );
  ...
  getNavigationMatrix(): CostMatrix {
    ...
    return cache.inHeap("plannerCostMatrix:" + this.roomName, 500, () =>
      this.activeRoomPlan.createNavigationMatrix(),
    );
  }
}
