...
export default class RemoteHarvesterRole extends Role {
  ...
  travelToSource(creep: RemoteHarvesterCreep) {
    ...
    if (
      creep.pos.roomName !== creep.operation.getRoom() &&
      !creep.hasCachedPath()
    ) {
      ...
      if (!paths[creep.memory.source] || !paths[creep.memory.source].accessible)
        return false;
      creep.setCachedPath(
        serializePositionPath(paths[creep.memory.source].path),
        true,
        1,
      );
    }
  ...
  performRemoteHarvest(creep: RemoteHarvesterCreep) {
    ...
    const workParts = creep.getActiveBodyparts(CARRY)
      ? creep.getActiveBodyparts(WORK)
      : 0;
    const needsBuild = creep.pos.findClosestByRange(
      FIND_MY_CONSTRUCTION_SITES,
      {
    ...
    if (needsBuild && creep.pos.getRangeTo(needsBuild) <= 3) {
      if (
        creep.store.energy >=
          Math.min(workParts * 5, creep.store.getCapacity()) &&
        workParts > 0
      ) {
    ...
    if (creep.operation instanceof RemoteMiningOperation) {
      ...
      const creepsOnContainer =
        container && container.pos.lookFor(LOOK_CREEPS).length > 0;
    ...
    creep.whenInRange(moveRange, moveTarget, () => {
      ...
      if (!creep.operation.hasContainer(creep.memory.source)) {
        ...
        const containerPosition = creep.operation.getContainerPosition(
          creep.memory.source,
        );
  ...
  repairNearbyContainer(creep: RemoteHarvesterCreep): boolean {
    ...
    const needsRepair = _.filter(
      ...
      (structure) =>
        ...
        creep.operation
          .getContainerPosition(creep.memory.source)
          ?.isEqualTo(structure.pos),
    );
