...
declare global {
  export interface MuleCreep extends Creep {
    role: 'mule'
    memory: MuleCreepMemory
    heapMemory: MuleCreepHeapMemory
  }

  export interface MuleCreepMemory extends CreepMemory {
    delivering?: boolean
    route: string
    roomPath?: string[]
    pathIndex?: number
  ...
  export interface MuleCreepHeapMemory extends CreepHeapMemory {
 ...
 */
export default MuleRole;
export class MuleRole extends Role {
  ...
  reportEarlyDemise(creep: MuleCreep) {
    ...
    if (creep.heapMemory.demiseReported) {
      return;
    }
    creep.heapMemory.demiseReported = true;

    const storageContents: unknown[] = [];
    ...
    Game.notify(`${creep.name} containing ${storageContents.join(', ')} is about to expire in ${creep.pos.roomName} on tick ${(Game.time + Number(creep.ticksToLive)) || 'undefined'}`);
  }

  setDelivering(creep: MuleCreep, deliver: boolean) {
    creep.memory.delivering = deliver;
    const previousIndex = creep.memory.pathIndex;
    this.setRoomPath(creep, deliver ? this.tradeRoute.getPath() as string[] : this.tradeRoute.getReversePath() as string[]);
    if (previousIndex) {
      creep.memory.pathIndex = ((creep.memory.roomPath?.length) || 0) - previousIndex - 1;
    }
  }
  ...
  pickupResources(creep: MuleCreep) {
    ...
    // @todo Cache it.
    const resourceType: ResourceConstant = this.tradeRoute.getResourceType() as ResourceConstant;
  ...
  deliverResources(creep: MuleCreep) {
    ...
    if (creep.store.getUsedCapacity() === 0) {
      ...
      if (travelLength && (creep.ticksToLive || 0) < 2.1 * travelLength) {
        this.performRecycle(creep);
      }
      return;
    }
    ...
    const amount = creep.store[resourceType as ResourceConstant] || 0;

    const target = creep.room.getBestStorageTarget(amount, resourceType as ResourceConstant);
    if (!target) {
      return;
    }
    ...
    Game.notify(`${creep.memory.route}: Transferred ${creep.store[resourceType as ResourceConstant] || 0} ${resourceType} to ${creep.room.name}`);
