...
export default class HelperRole extends Role {
  ...
  performHelperCleanup(creep: HelperCreep): boolean {
    ...
    for (const lab of boostLabs) {
      if (
        lab.mineralType &&
        lab.mineralType !== boostManager.getRequiredBoostType(lab.id)
      ) {
        if (creep.memory.delivering) {
          // Put everything away.
  ...
  performHelperLabDeliver(creep: HelperCreep): boolean {
    ...
    for (const lab of boostLabs) {
      ...
      if (creep.store.getUsedCapacity(resourceType) > 0) {
        const diff =
          boostManager.getRequiredBoostAmount(lab.id) -
  ...
  performHelperGather(creep: HelperCreep) {
    ...
    if (target) {
      const amount = Math.min(
        totalNeededEnergy - creep.store.getUsedCapacity(RESOURCE_ENERGY),
