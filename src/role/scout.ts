...
export default class ScoutRole extends Role {
  ...
  getScoutableRoomsForCreep(creep: ScoutCreep): ScoutTarget[] {
    if (creep.pos.roomName === creep.memory.origin) {
      return this.filterScoutableRooms(
        creep,
        this.getScoutableRoomsByOrigin(creep.memory.origin),
      );
    }

    return this.filterScoutableRooms(
      creep,
      this.getScoutableRoomsInRange(creep.pos.roomName, 5),
    );
  }
  ...
  filterScoutableRooms(creep: ScoutCreep, rooms: ScoutTarget[]): ScoutTarget[] {
    return _.filter(rooms, (info: ScoutTarget) => {
      ...
      if (
        creep.memory.invalidScoutTargets &&
        creep.memory.invalidScoutTargets.includes(info.roomName)
      )
        return false;
      if (Game.map.getRoomLinearDistance(creep.pos.roomName, info.roomName) > 5)
        return false;
  ...
  hasRoomPath(creep: Creep, destination: string): boolean {
    return cache.inObject(
      accessibilityCache,
      creep.pos.roomName + "/" + destination,
      5000,
      () => {
        const path = container
          .get("NavMesh")
          .findPath(creep.pos, new RoomPosition(25, 25, destination));
  ...
  isOscillating(creep: ScoutCreep) {
    ...
    if (
      history.length === 0 ||
      history[history.length - 1] !== creep.pos.roomName
    )
