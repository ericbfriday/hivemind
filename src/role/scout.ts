...
declare global {
  ...
  export interface ScoutCreepMemory extends CreepMemory {
    role: 'scout'
    scoutTarget?: string
    portalTarget?: string
  ...
  export interface ScoutCreepHeapMemory extends CreepHeapMemory {
...
export class ScoutRole extends Role {
  ...
  performScout(creep: ScoutCreep) {
    ...
    if (this.isOscillating(creep) || this.isStuck(creep)) {
  ...
  chooseScoutTarget(creep: ScoutCreep, invalidateOldTarget?: boolean) {
    if (creep.heapMemory.pauseUntil) {
      ...
      if (Game.time >= creep.heapMemory.pauseUntil) {
        delete creep.heapMemory.pauseUntil;
      }
  ...
  filterScoutableRooms(creep: ScoutCreep, rooms: ScoutTarget[]): ScoutTarget[] {
    return _.filter(rooms, (info: ScoutTarget) => {
      if (info.roomName === creep.pos.roomName) {
        return false;
      }
      if (creep.memory.invalidScoutTargets && creep.memory.invalidScoutTargets.includes(info.roomName)) {
        return false;
      }
      if (container.get('NavMesh').getRoomDistance(creep.pos.roomName, info.roomName) > 5) {
        return false;
      }
  ...
  getScoutableRoomsInRange(roomName: string, range: number): ScoutTarget[] {
    return cache.inHeap(`scoutableRooms:${roomName}:${range}`, 200, () => _.filter(this.getScoutableRooms(), (info: ScoutTarget) => {
      if (container.get('NavMesh').getRoomDistance(roomName, info.roomName) > range) {
        return false;
      }
  ...
  isOscillating(creep: ScoutCreep) {
    if (!creep.heapMemory.roomHistory) {
      creep.heapMemory.roomHistory = [];
    }
    const history = creep.heapMemory.roomHistory;

    if (history.length === 0 || history[history.length - 1] !== creep.pos.roomName) {
      history.push(creep.pos.roomName);
    }
    if (history.length > 20) {
      creep.heapMemory.roomHistory = history.slice(-10);
    }
  ...
  isTileOscillating(creep: ScoutCreep) {
    if (!creep.heapMemory.posHistory) {
      creep.heapMemory.posHistory = [];
    }
    ...
    if (history.length > 30) {
      creep.heapMemory.posHistory = history.slice(-20);
    }
