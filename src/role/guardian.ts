...
const filterEnemyCreeps = (c: Creep) =>
  !hivemind.relations.isAlly(c.owner.username) && c.isDangerous();
...
export default class GuardianRole extends Role {
  ...
  setAlternatePositions(creep: GuardianCreep) {
    container
      .get("TrafficManager")
      .setAlternatePositions(creep, this.getAdjacentRampartPositions(creep));
  }
  ...
  getBestRampartToCover(creep: GuardianCreep): StructureRampart {
    ...
    return min(
      ...
      (s: StructureRampart) =>
        ...
        s.pos.getRangeTo(
          s.pos.findClosestByRange(FIND_HOSTILE_CREEPS, {
            filter: filterEnemyCreeps,
          }),
        ),
    );
