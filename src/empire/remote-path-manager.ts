...
export default RemotePathManager;
export class RemotePathManager {
  getPathFor(sourcePosition: RoomPosition): RoomPosition[] | null {
    ...
    for (const room of sortedByDist) {
      ...
      if (minPathLength < hivemind.settings.get('maxRemoteMinePathLength') && cannotFindShorterPath) {
        continue;
      }
      if (!room.roomPlanner) {
        continue;
