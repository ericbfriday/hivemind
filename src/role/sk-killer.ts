...
declare global {
  ...
  export interface SkKillerCreepMemory extends CreepMemory {
    role: 'skKiller'
    targetRoom: string
  }

  export interface SkKillerCreepHeapMemory extends CreepHeapMemory {
...
export class RemoteHarvesterRole extends Role {
  ...
  run(creep: SkKillerCreep) {
    ...
    if (this.travelToTargetRoom(creep)) {
      return;
    }
  ...
  travelToTargetRoom(creep: SkKillerCreep) {
    if (creep.pos.roomName === creep.operation.getRoom()) {
      return false;
    }

    if (!creep.hasCachedPath()) {
      const paths = creep.operation.getPaths();
      const path = _.minBy(_.filter(paths, path => path.accessible), path => path.travelTime ?? 500);

      if (path) {
        creep.setCachedPath(serializePositionPath(path.path), true, 1);
      }
    }
  ...
  performSkSlaughter(creep: SkKillerCreep) {
    ...
    for (const owner in room.enemyCreeps) {
      ...
      for (const enemyCreep of room.enemyCreeps[owner]) {
        if (!enemyCreep.isDangerous()) {
          continue;
        }

        if (
          enemyCreep.owner.username === 'Source Keeper'
          && _.minBy(_.map(room.structuresByType[STRUCTURE_KEEPER_LAIR], (s: StructureKeeperLair) => s.pos.getRangeTo(enemyCreep.pos))) <= 5
        ) {
          const closestLair = _.minBy(room.structuresByType[STRUCTURE_KEEPER_LAIR], (s: StructureKeeperLair) => s.pos.getRangeTo(enemyCreep.pos));
    ...
    const nextLair = _.minBy(room.structuresByType[STRUCTURE_KEEPER_LAIR], (lair: StructureKeeperLair) => {
      const closestResource: Source | Mineral = _.minBy([...room.sources, ...room.minerals], (source: Source | Mineral) => source.pos.getRangeTo(lair.pos));
  ...
  healNearbyCreeps(creep: SkKillerCreep): boolean {
    ...
    const target = _.minBy(injuredCreeps, c => c.pos.getRangeTo(creep.pos));
    if (creep.pos.getRangeTo(target) > 1) {
      if (creep.rangedHeal(target) === OK) {
        return true;
      }
    }
    else if (creep.heal(target) === OK) {
      return true;
    }
