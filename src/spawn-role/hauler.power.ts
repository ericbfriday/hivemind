...
export class PowerHaulerSpawnRole extends SpawnRole {
  /**
  ...
  getSpawnOptions(room: Room): PowerHaulerSpawnOption[] {
    ...
    return this.cacheEmptySpawnOptionsFor(room, 100, () => {
      ...
      _.each(Memory.strategy.power.rooms, (info, roomName) => {
        ...
        if (timeToKill > (CREEP_SPAWN_TIME * MAX_CREEP_SIZE) + Math.max(CREEP_LIFE_TIME / 3, travelTime)) {
          return;
        }
...
export default PowerHaulerSpawnRole;
