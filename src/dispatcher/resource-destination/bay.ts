...
export default class BayDestination extends TaskProvider<
  BayDestinationTask,
    ...
    return this.cacheEmptyTaskListFor("", 5, () => {
      ...
      for (const bay of this.room.bays) {
        ...
        const deliveryAmount = Math.min(
          context.creep.store[RESOURCE_ENERGY],
          bay.energyCapacity - bay.energy,
        );
        option.weight +=
          deliveryAmount / context.creep.store.getCapacity() +
          1 -
          context.creep.pos.getRangeTo(bay) / 100;
        option.priority -=
          this.room.getCreepsWithOrder(this.getType(), bay.name).length * 3;
