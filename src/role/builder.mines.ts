...
declare global {
  ...
  export interface MineBuilderCreepMemory extends CreepMemory {
    role: 'builder.mines'
    returning: boolean
    source: string
  }

  export interface MineBuilderCreepHeapMemory extends CreepHeapMemory {
    energyPickupTarget: Id<Resource>
  }
}

export default MineBuilderRole;
export class MineBuilderRole extends Role {
  actionTaken: boolean;
  ...
  run(creep: MineBuilderCreep) {
    ...
    if (creep.memory.returning) {
      ...
      if (this.performBuildRoad(creep)) {
        return;
      }
  ...
  determineTargetSource(creep: MineBuilderCreep) {
    ...
    const bestPosition = _.maxBy(_.filter(scoredPositions, p => p.work > 0), 'work');
  ...
  performBuildRoad(creep: MineBuilderCreep) {
    ...
    if (creep.hasCachedPath()) {
      if (this.buildRoadOnCachedPath(creep)) {
        return true;
      }
    }
    else if (this.repairNearby(creep)) {
      return true;
    }

    // Check source container and repair that, too.
    if (this.ensureRemoteHarvestContainerIsBuilt(creep)) {
      return true;
    }

    if (this.buildNearby(creep)) {
      return true;
    }
  ...
  buildRoadOnCachedPath(creep: MineBuilderCreep) {
    ...
    if (creep.room.controller && creep.room.controller.owner && !creep.room.isMine()) {
      return false;
    }
