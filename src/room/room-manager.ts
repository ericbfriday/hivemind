...
export default class RoomManager {
  ...
  initializeMemory() {
    ...
    this.featureFlags = new PersistentFeatureFlag<RoomManagerFeatureFlag>(
      "room:managerFlags:" + this.room.name,
    );
  }
  ...
  runLogic() {
    ...
    if (
      this.room.defense.getEnemyStrength() > ENEMY_STRENGTH_NONE &&
    ...
    if (
      this.room.controller.level !== this.featureFlags.getNumeric("ranAtRcl")
    ) {
      this.featureFlags.reset();
    ...
    if (
      ...
      this.roomConstructionSites.length + this.newStructures === 0
    ) {
  ...
  periodicallyCheckDecayingStructures() {
    ...
    if (!hivemind.hasIntervalPassed(CREEP_LIFE_TIME, this.lastDecayCheck))
      return;
  ...
  recoverRoom(): boolean {
    ...
    if (
      !this.structuresByType[STRUCTURE_SPAWN] ||
      this.structuresByType[STRUCTURE_SPAWN].length === 0
    )
      return true;
    if (
      CONTROLLER_STRUCTURES[STRUCTURE_STORAGE][this.room.controller.level] >
        0 &&
      (!this.structuresByType[STRUCTURE_STORAGE] ||
        this.structuresByType[STRUCTURE_STORAGE].length === 0)
    )
      return true;
  ...
  buildRoomDefenseFirst() {
    for (
      let i = 0;
      i < CONTROLLER_STRUCTURES[STRUCTURE_TOWER][this.room.controller.level];
      i++
    ) {
      // Build ramparts at tower spots.
      this.buildPlannedStructures(`tower.${i}`, STRUCTURE_RAMPART, (pos) =>
        this.roomPlanner.isPlannedLocation(pos, "tower"),
      );
      this.buildPlannedStructures(`tower.${i}`, STRUCTURE_TOWER, (pos) =>
        this.roomPlanner.isPlannedLocation(pos, "tower"),
      );
    }
    ...
    if (this.checkWallIntegrity(10_000)) {
      const creepCost =
        6 * BODYPART_COST[WORK] +
        3 * BODYPART_COST[MOVE] +
        3 * BODYPART_COST[CARRY];
      if (
        this.room.energyCapacityAvailable + SPAWN_ENERGY_CAPACITY <
        creepCost
      ) {
  ...
  cleanExtensions() {
    for (const extension of this.structuresByType[STRUCTURE_EXTENSION] || []) {
      ...
      if (
        this.roomPlanner.isPlannedLocation(extension.pos, "extension") &&
        extension.isOperational()
      )
        continue;
      if (
        !this.roomPlanner.isPlannedLocation(extension.pos, "road") &&
        extension.isOperational()
      )
        continue;
        ...
        for (const sourceLocation of locations[this.room.name]) {
          for (const position of paths[sourceLocation].path) {
            if (position.roomName === this.room.name)
              positions[serializeCoords(position.x, position.y)] = position;
          }
        }
      }
  ...
  manageStructures() {
    ...
    if (this.room.controller.level === 0) {
      ...
      this.buildPlannedStructures(
        "road.source",
        STRUCTURE_ROAD,
        (pos) => terrain.get(pos.x, pos.y) === TERRAIN_MASK_SWAMP,
      );
    ...
    if (
      this.room.storage ||
      CONTROLLER_STRUCTURES[STRUCTURE_STORAGE][this.room.controller.level] === 0
    ) {
    ...
    if (
      CONTROLLER_STRUCTURES[STRUCTURE_EXTRACTOR][this.room.controller.level] > 0
    ) {
  ...
  manageTowers() {
    ...
    const maxTowers =
      CONTROLLER_STRUCTURES[STRUCTURE_TOWER][this.room.controller.level];
  ...
  manageSpawns() {
    ...
    if (
      ...
      this.roomConstructionSites.length === 0
    ) {
      this.removeMisplacedSpawn(roomSpawns as StructureSpawn[]);
    } else if (
      roomSpawns.length + roomSpawnSites.length <
      CONTROLLER_STRUCTURES[STRUCTURE_SPAWN][this.room.controller.level]
    ) {
      for (
        let i = 0;
        i < CONTROLLER_STRUCTURES[STRUCTURE_SPAWN][this.room.controller.level];
        i++
      ) {
        this.buildPlannedStructures(`spawn.${i}`, STRUCTURE_SPAWN, (pos) =>
          this.roomPlanner.isPlannedLocation(pos, "spawn"),
        );
      }
  ...
  manageRamparts() {
    ...
    if (
      ...
      this.buildPlannedStructures(
        "rampart",
        STRUCTURE_RAMPART,
        (pos) => !this.roomPlanner.isPlannedLocation(pos, "rampart.ramp"),
      );
  ...
  manageStorage() {
    if (
      this.hasMisplacedStorage() &&
      this.room.storage.store.getUsedCapacity() < 5000
    ) {
  ...
  manageTerminal() {
    if (
      this.hasMisplacedTerminal() &&
      this.room.terminal.store.getUsedCapacity() < 5000
    ) {
  ...
  manageExtensions() {
    ...
    if (this.room.controller.level >= 3) {
      ...
      this.buildPlannedStructures(
        "extension.harvester",
        STRUCTURE_EXTENSION,
        (pos) => this.roomPlanner.isPlannedLocation(pos, "extension"),
      );
    }
    ...
    this.buildPlannedStructures("extension.bay", STRUCTURE_EXTENSION, (pos) =>
      this.roomPlanner.isPlannedLocation(pos, "extension"),
    );
  ...
  manageExtractors() {
    ...
    if (
      plannedLocations.length <=
      CONTROLLER_STRUCTURES[STRUCTURE_EXTRACTOR][this.room.controller.level]
    ) {
    ...
    for (const mineral of sortedMinerals) {
      ...
      if (mineral.score > 0) {
        if (mineral.hasExtractor) {
          ...
          if (
            currentExtractors >=
            CONTROLLER_STRUCTURES[STRUCTURE_EXTRACTOR][
  ...
  scoreExtractorPositions(
    positions: RoomPosition[],
  ): ScoredExtractorPosition[] {
   ...
   */
  buildPlannedStructures(
    locationType: string,
    structureType: StructureConstant,
    filterCallback?: (pos: RoomPosition) => boolean,
  ): boolean {
  ...
  tryBuild(pos: RoomPosition, structureType) {
    ...
    if (
      ...
      if (
        ...
        pos.findInRange(FIND_NUKES, 2).length > 0
      ) {
        return true;
      }

      const isBlocked =
        OBSTACLE_OBJECT_TYPES.includes(structureType) &&
        (pos.lookFor(LOOK_CREEPS).length > 0 ||
          pos.lookFor(LOOK_POWER_CREEPS).length > 0);
  ...
  hasMisplacedStorage(): boolean {
    ...
    if (this.roomPlanner.isPlannedLocation(this.room.storage.pos, "storage"))
      return false;
  ...
  hasMisplacedTerminal(): boolean {
    ...
    if (!this.roomPlanner) return false;
    if (this.roomPlanner.isPlannedLocation(this.room.terminal.pos, "terminal"))
      return false;
  ...
  checkWallIntegrity(minHits?: number) {
    ...
    return cache.inHeap(
      `wallIntegrity:${this.room.name}:${minHits}`,
      100,
      () => {
  ...
  buildLabs() {
    ...
    if (
      CONTROLLER_STRUCTURES[STRUCTURE_LAB][this.room.controller.level] === 3
    ) {
  ...
  buildFactories() {
    ...
    if (size(this.roomConstructionSites) === 0) {
      ...
      if (
        this.room.factory &&
        (this.room.factory.level || 0) > 0 &&
        this.room.factoryManager &&
        this.room.factoryManager.getFactoryLevel() > 0 &&
...
Structure.prototype.needsDismantling = function (this: Structure): boolean {
  if (!this.room.roomManager || !this.room.roomManager.needsDismantling())
    return false;
