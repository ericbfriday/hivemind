...
export class FunnelManager {
    ...
    return cache.inHeap('funneledRooms', 500, () => {
      ...
      if (hasTooMuchEnergy) {
        ...
        for (const room of [...(roomsAtLevel[6] || []), ...(roomsAtLevel[7] || [])]) {
          if (!funneledRooms.includes(room.name)) {
            funneledRooms.push(room.name);
          }
        }
      ...
      if (this.getGclUpgradeScore() > _.maxBy(funneledRooms.map(roomName => this.getFunnelRoomScore(Game.rooms[roomName])))) {
        return [];
      }
  ...
  protected getAvailableRoomsToFunnel(): Room[] {
    return _.filter(Game.myRooms, (room) => {
      ...
      if (room.isStripmine() && room.controller.level >= 6) {
        return false;
      }
