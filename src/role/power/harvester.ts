...
export default class PowerHarvesterRole extends Role {
  ...
  run(creep: PowerHarvesterCreep) {
    ...
    if (
      ...
      Memory.strategy.power.rooms[creep.pos.roomName]
    ) {
      if (powerBanks.length > 0) {
        Memory.strategy.power.rooms[creep.pos.roomName].hits =
          powerBanks[0].hits;
        Memory.strategy.power.rooms[creep.pos.roomName].decays =
          Game.time + (powerBanks[0].ticksToDecay || POWER_BANK_DECAY);
      } else {
    ...
    if (powerResources.length === 0) {
      ...
      if (
        ...
        Memory.strategy.power &&
        Memory.strategy.power.rooms &&
        Memory.strategy.power.rooms[creep.memory.targetRoom]
      ) {
  ...
  chaseNearbyRanged(creep: PowerHarvesterCreep) {
    ...
    const highestValue = max(
      ...
      (c) =>
        ...
        c.getHealCapacity(1) * 2 -
        c.pos.getRangeTo(creep.pos) * 5,
    );
    if (!highestValue) return;
  ...
  attackPowerBank(creep: PowerHarvesterCreep, powerBank: StructurePowerBank) {
    if (creep.memory.isHealer) {
      const damagedCreep = creep.pos.findClosestByRange(FIND_MY_CREEPS, {
        filter: (otherCreep) =>
          otherCreep.memory.role === "harvester.power" &&
          otherCreep.hits + (otherCreep.incHealing || 0) < otherCreep.hitsMax,
      });
