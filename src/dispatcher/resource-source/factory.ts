...
export default class FactorySource extends StructureSource<FactorySourceTask> {
  ...
  addEmptyFactoryTasks(
    options: FactorySourceTask[],
    ...
    for (const resourceType of getResourcesIn(this.room.factory.store)) {
      ...
      if (
        ...
        this.room.factory.store.getUsedCapacity(resourceType) <=
          neededResources[resourceType] * 1.5
      )
        continue;
      const storedAmount =
        this.room.factory.store.getUsedCapacity(resourceType);
      ...
      options.push({
        type: this.getType(),
        priority:
          (extraAmount > 1000 ? 3 : 2) -
          this.room.getCreepsWithOrder("factory", structure.id).length,
