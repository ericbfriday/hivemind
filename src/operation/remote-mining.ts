...
export default class RemoteMiningOperation extends Operation {
      ...
      for (const sourcePos of positions) {
        ...
        const info = cache.inHeap("rmPath:" + sourceLocation, 100, () => {
          ...
          const generatedEnergy = roomIntel.isClaimable()
            ? this.canReserveFrom(sourceRoom)
              ? SOURCE_ENERGY_CAPACITY
              : SOURCE_ENERGY_NEUTRAL_CAPACITY
            : SOURCE_ENERGY_KEEPER_CAPACITY;
          const requiredWorkParts =
            generatedEnergy / ENERGY_REGEN_TIME / HARVEST_POWER;
          const requiredCarryParts = Math.ceil(
            (2 * travelTime * generatedEnergy) /
              ENERGY_REGEN_TIME /
              CARRY_CAPACITY,
          );
          return {
  ...
  isUnderAttack(): boolean {
    ...
    if (
      roomMemory?.enemies &&
      !roomMemory.enemies.safe &&
      roomMemory.enemies.expires > Game.time
    )
      return true;
  ...
  getTotalEnemyData(): EnemyData {
    ...
    for (const roomName of this.getRoomsOnPath()) {
      ...
      for (const part in roomMemory.enemies.parts || {}) {
        totalEnemyData.parts[part] =
          (totalEnemyData.parts[part] || 0) + roomMemory.enemies.parts[part];
      }
    }
  ...
  needsRepairs(sourceLocation: string) {
    return cache.inHeap("needsRepairs:" + sourceLocation, 50, () => {
      ...
      if (
        this.getNeededWorkForPosition(containerPositon, STRUCTURE_CONTAINER) >
        CONTAINER_HITS / 2
      )
        return true;
      ...
      for (const position of path.path || []) {
        ...
        if (
          this.getNeededWorkForPosition(position, STRUCTURE_ROAD) >
          ROAD_HITS / 3
        )
          return true;
      }
  ...
  getNeededWork(sourceLocation: string) {
    return cache.inHeap("neededRepairs:" + sourceLocation, 50, () => {
      const containerPositon = this.getContainerPosition(sourceLocation);
      let total = this.getNeededWorkForPosition(
        containerPositon,
        STRUCTURE_CONTAINER,
      );
  ...
  getContainer(sourceLocation: string): StructureContainer | null {
    ...
    return Game.getObjectById<StructureContainer>(
      this.memory.status[sourceLocation].containerId,
    );
  }
    ...
    const cached = cache.inHeap("blockedTiles:" + sourceLocation, 100, () => {
      ...
      for (let i = path.length - 1; i > 0; i--) {
        ...
        if (Game.rooms[roomName]) {
          for (const structure of Game.rooms[roomName].structuresByType[
