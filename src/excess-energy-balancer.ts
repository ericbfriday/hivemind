...
const balancer = {
  ...
  balanceEnergyLevels() {
    ...
    const currentRatio =
      Memory.energyBalace.powerEnergy / Memory.energyBalace.gplEnergy;

    if (currentRatio > ratio) {
      // We spent more energy on power than planned.
      Memory.energyBalace.powerEnergy -=
        (Memory.energyBalace.gplEnergy * ratio) / (1 - ratio);
      Memory.energyBalace.gplEnergy = 0;
    } else {
      // We spent more energy on GPL than planned.
      Memory.energyBalace.gplEnergy -=
        (Memory.energyBalace.powerEnergy * (1 - ratio)) / ratio;
      Memory.energyBalace.powerEnergy = 0;
    }
  },
