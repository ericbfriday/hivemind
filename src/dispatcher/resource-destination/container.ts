...
export default class ContainerDestination extends StructureDestination<ContainerDestinationTask> {
  ...
  addControllerContainerTask(
    ...
    const refillPathLength = cache.inHeap(
      "ccRefillPathLength:" + this.room.name,
      5000,
      () => {
        const storagePosition = this.room.getStorageLocation();
        if (!storagePosition) return 1;
        const path = utilities.getPath(storagePosition, container.pos, false, {
          singleRoom: this.room.name,
        });
    ...
    const usedEnergyUntilArrival =
      upgraderWorkParts * UPGRADE_CONTROLLER_POWER * refillPathLength;
    const otherDeliveringCreeps = this.room.getCreepsWithOrder(
      this.getType(),
      container.id,
    );
  ...
  isValid(task: ContainerDestinationTask, context: ResourceDestinationContext) {
    ...
    if (
      context.creep.memory.singleRoom &&
      structure.pos.roomName !== context.creep.memory.singleRoom
    )
      return false;
    if (
    ...
    if (
      structure.store.getFreeCapacity(task.resourceType) === 0 &&
      context.creep.pos.getRangeTo(structure.pos) === 1
    )
      return false;
