...
import type SegmentedMemory from '@/utils/segmented-memory';
...
export interface OutdatedRoomMemory {
    intel: any
}
...
class Hivemind {
    ...
    emergencyBrakeProcessId: string | null;
    ...
    gatherCpuStats() {
        ...
        if (!Memory.strategy.reports) {
            return;
        }
        if (!Memory.strategy.reports.data) {
            return;
        }
        if (!Memory.strategy.reports.data.cpu) {
            Memory.strategy.reports.data.cpu = {};
        }
    ...
    runProcess<P extends ProcessParameters>(id: string, ProcessConstructor: new (parameters: P) => ProcessInterface, options: P) {
        if (this.pullEmergengyBrake(id)) {
            return;
        }
        if (options && options.requireSegments && !this.segmentMemory.isReady()) {
            return;
        }
     ...
     */
    pullEmergengyBrake(id: string): boolean {
     ...
     */
    initializeProcessStats(id: string): object {
    ...
    isProcessAllowedToRun(stats: ProcessMemory, options: ProcessParameters): boolean {
        ...
        if (Game.cpu.bucket <= stopAt) {
            return false;
        }

        // No need to throttle if no interval is set.
        if (interval === 0 || priority === PROCESS_PRIORITY_ALWAYS) {
            return true;
        }
    ...
    hasIntervalPassed(interval: number, startTime: number, stopAt?: number, throttleAt?: number): boolean {
        ...
        if (Game.time - startTime < interval * this.getThrottleMultiplier(stopAt, throttleAt)) {
            return false;
        }
    ...
    log(channel: string, roomName?: string): Logger {
        ...
        if (!this.loggers[category][channel]) {
            this.loggers[category][channel] = new Logger(channel, roomName);
        }
    ...
    migrateData(): boolean {
        ...
        if (!this.memory.intelMigrated) {
            ...
            _.each(Memory.rooms, (memory: RoomMemory, roomName: string) => {
                if (!memory.intel) {
                    return;
                }
    ...
    drawProcessDebug() {
        ...
        const processData = _.groupBy(_.orderBy(filtered, ['lastRun', 'lastCpu'], ['desc', 'desc']), 'parentId');
