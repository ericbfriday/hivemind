...
export default class BoostManager {
  ...
  private ensureLabIsStillNeededForBoosting(lab: StructureLab) {
    ...
    const needsThisBoost = find(
      ...
      (boosts, creepName) =>
        boosts[resourceType] &&
        Game.creeps[creepName]?.pos?.roomName === this.room.name,
    );
  ...
  private cleanBoostMemory() {
    ...
    for (const id in this.memory.labs) {
      if (
        !Game.getObjectById(id as Id<StructureLab>) ||
        !neededBoosts[this.memory.labs[id]]
      ) {
  ...
  private getNumberOfPartsToBoost(labId: Id<StructureLab>): number {
    ...
    return sumBy(this.memory.creeps, (boosts, creepName) => {
  ...
  private getAllRequestedBoosts(): Partial<Record<ResourceConstant, number>> {
    ...
    for (const creepName in this.memory.creeps) {
      ...
      for (const resourceType in this.memory.creeps[creepName]) {
        boosts[resourceType] =
          (boosts[resourceType] || 0) +
          this.memory.creeps[creepName][resourceType];
      }
    }
  ...
  private getMostPreparedLab(
    ...
    for (const lab of labs) {
      ...
      const energyScore = Math.min(
        lab.store.energy / LAB_BOOST_ENERGY / boosts[resourceType],
        1,
      );
      const mineralScore =
        3 *
  ...
  private ensureBoostsHaveLabAssigned(
    boosts: Partial<Record<ResourceConstant, number>>,
  ) {
