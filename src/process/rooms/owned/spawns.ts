...
export default class ManageSpawnsProcess extends Process {
  ...
  collectSpawnStats(spawns: StructureSpawn[]) {
    for (const spawn of spawns) {
      ...
      if (spawn.heapMemory.ticks >= historyChunkLength) {
        ...
        spawn.heapMemory.history =
          spawn.heapMemory.history.slice(-maxHistoryChunks);

        // Also record to room stats if enabled.
        if (
          settings.get("recordRoomStats") &&
          Memory.roomStats[spawn.room.name]
        ) {
          Memory.roomStats[spawn.room.name][
            "RCL" + spawn.room.controller.level + "SpawnSpawning"
          ] =
            (Memory.roomStats[spawn.room.name][
              "RCL" + spawn.room.controller.level + "SpawnSpawning"
            ] || 0) + spawn.heapMemory.spawning;
          Memory.roomStats[spawn.room.name][
            "RCL" + spawn.room.controller.level + "SpawnWaiting"
          ] =
            (Memory.roomStats[spawn.room.name][
              "RCL" + spawn.room.controller.level + "SpawnWaiting"
            ] || 0) + spawn.heapMemory.waiting;
          Memory.roomStats[spawn.room.name][
            "RCL" + spawn.room.controller.level + "SpawnIdle"
          ] =
            (Memory.roomStats[spawn.room.name][
              "RCL" + spawn.room.controller.level + "SpawnIdle"
            ] || 0) +
          ...
          Memory.roomStats[spawn.room.name][
            "RCL" + spawn.room.controller.level + "SpawnTotalTicks"
          ] =
            (Memory.roomStats[spawn.room.name][
              "RCL" + spawn.room.controller.level + "SpawnTotalTicks"
            ] || 0) + spawn.heapMemory.ticks;
        }
  ...
  visualizeSpawning(spawns: StructureSpawn[]) {
    ...
    for (const spawn of spawns) {
      ...
      this.room.visual.rect(spawn.pos.x - 0.5, spawn.pos.y, 1, 0.3, {
        fill: "#888888",
        opacity: 0.5,
      });
      this.room.visual.rect(
        spawn.pos.x - 0.5,
        spawn.pos.y,
        spawningTicks / totalTicks,
        0.3,
        { fill: "#88ff88" },
      );
      this.room.visual.rect(
        spawn.pos.x - 0.5 + spawningTicks / totalTicks,
        spawn.pos.y,
        waitingTicks / totalTicks,
        0.3,
        { fill: "#ff8888" },
      );
  ...
  visualizeSpawnQueue() {
    ...
    for (const option of queue) {
      tableData.push([
        option.role,
        option.priority + "/" + option.weight.toPrecision(2),
      ]);
    }
