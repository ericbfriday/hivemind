...
export default UpgraderSpawnRole;
export class UpgraderSpawnRole extends SpawnRole {
  /**
  ...
  getBaseUpgraderAmount(room: Room): number {
    ...
    if (room.controller.level >= 6 && room.isStripmine()) {
      return 0;
    }
    ...
    if (isFunnelingElsewhere && availableEnergy < 100_000) {
      return 0;
    }

    if (room.controller.level === 8 && !balancer.maySpendEnergyOnGpl()) {
      return 0;
    }

    // RCL 8 rooms can't make use of more than 1 upgrader.
    if (room.controller.level === 8) {
      if (availableEnergy < hivemind.settings.get('minEnergyToUpgradeAtRCL8')) {
        return 0;
      }
      return 1;
    }
    ...
    if (availableEnergy < (room.controller.level === 7 ? 25_000 : 10_000)) {
      return 0;
    }
    if (availableEnergy < (room.controller.level === 7 ? 75_000 : 50_000)) {
      return 1;
    }
    if (availableEnergy < 100_000) {
      return 2;
    }
    if (availableEnergy < 125_000 && isFunnelingToRoom) {
      return 3;
    }
    if (availableEnergy < 150_000) {
      return isFunnelingToRoom ? 4 : 3;
    }
    if (availableEnergy < 200_000) {
      return isFunnelingToRoom ? 5 : 4;
    }
    if (availableEnergy < 250_000) {
      return isFunnelingToRoom ? 6 : 5;
    }
