...
export default class RemotePathManager {
  getPathFor(sourcePosition: RoomPosition): RoomPosition[] | null {
    ...
    if (
      memory.generated &&
      !hivemind.hasIntervalPassed(5000, memory.generated)
    ) {
    ...
    for (const room of sortedByDist) {
      ...
      const cannotFindShorterPath =
        Game.map.getRoomLinearDistance(sourcePosition.roomName, room.name) >
        Math.ceil(minPathLength / 50);
      if (
        minPathLength < hivemind.settings.get("maxRemoteMinePathLength") &&
        cannotFindShorterPath
      )
        continue;
      ...
      const result = PathFinder.search(
        ...
        { pos: storagePos, range: 1 },
        {
          ...
          roomCallback: (roomName) =>
            this.getCostMatrix(roomName, sourcePosition.roomName === roomName),
        },
      );

      if (!result || result.incomplete || result.path.length >= minPathLength)
        continue;
  ...
  getCostMatrix(roomName: string, isTargetRoom: boolean): CostMatrix | false {
    return cache.inHeap(
      "remotePathManagerCostMatrix:" + roomName + (isTargetRoom ? ":t" : ""),
      1000,
      () => {
        ...
        const matrix = isMyRoom
          ? Game.rooms[roomName].roomPlanner.getNavigationMatrix().clone()
