...
export class CombatManager {
    ...
    public hasEnemyCreepsInFightingRange(creep: Creep, enemyCreeps?: Creep[]): boolean {
        return _.some(enemyCreeps ?? this.getEnemyMilitaryCreeps(creep.room), (c: Creep) => c.pos.getRangeTo(creep) <= (c.owner.username === 'Source Keeper' ? 3 : 5));
    }
    ...
    public couldWinFightAgainst(creep: Creep, otherCreep: Creep): boolean {
        if (creep.room.isMine() && creep.room.controller.safeMode) {
            return true;
        }

        const towerHealPower = this.getTowerHealPower(creep);
        const activeAllies = this.getAlliedCombatCreeps(creep.pos);
        const attackParts = creep.getActiveBodyparts(ATTACK) + _.sumBy(activeAllies, c => c.getActiveBodyparts(ATTACK)) * 0.5;
        const healParts = creep.getActiveBodyparts(HEAL) + _.sumBy(activeAllies, c => c.getActiveBodyparts(HEAL)) * 0.5;
        const rangedAttackParts = creep.getActiveBodyparts(RANGED_ATTACK) + _.sumBy(activeAllies, c => c.getActiveBodyparts(RANGED_ATTACK)) * 0.5;
    ...
    public getAlliedCombatCreeps(position: RoomPosition): Creep[] {
        ...
        for (const creep of Object.values(room.creeps)) {
            ...
            if (creep.pos.getRangeTo(position) > 5) {
                continue;
            }
    ...
    public getMostValuableTarget(creep: Creep, targets?: AttackTarget[]): AttackTarget | null {
        if (!targets) {
            targets = this.getAllTargetsInRoom(creep.room);
        }
    ...
    private healNearbyTargets(creep: Creep) {
        ...
        const target = _.maxBy(targets, (c: Creep) => Math.min(
            c.hitsMax - c.hits,
        ...
        if (target) {
            ...
            else if (!this.hasAttacked && !this.hasRangedAttacked) {
    ...
    private attackNearbyTargets(creep: Creep) {
        ...
        const availableOwnedTargets = _.filter(availableTargets, target => ('owner' in target) && target?.owner?.username) as AttackTarget[];
    ...
    private getNearbyTargets(creep: Creep): AttackTarget[] {
        ...
        for (const enemyName in creep.room.enemyCreeps) {
            if (hivemind.relations.isAlly(enemyName)) {
                continue;
            }

            for (const target of creep.room.enemyCreeps[enemyName]) {
                if (creep.pos.getRangeTo(target) > maxRange) {
                    continue;
                }
        ...
        for (const structure of creep.pos.findInRange(FIND_STRUCTURES, maxRange)) {
            ...
            if ('owner' in structure && hivemind.relations.isAlly(structure.owner?.username)) {
                continue;
            }
            if (!('owner' in structure) && isMyRoom) {
                continue;
            }
    ...
    private getValidNeighboringPositions(position: RoomPosition) {
        ...
        handleMapArea(position.x, position.y, (x, y) => {
            ...
            if (this.isTileWall(newPosition)) {
                return;
            }
    ...
    private addTerrainScore(creep: Creep, enemyCreeps: Creep[], positions: ScoredPosition[]) {
        ...
        for (const position of positions) {
            if (this.isTileSwamp(position.pos)) {
                position.score -= 100;
            }
            ...
            handleMapArea(position.pos.x, position.pos.y, (x, y) => {
                ...
                if (this.isTileWall(pos)) {
                    position.score -= 20;
                }
                if (this.isTileSwamp(pos)) {
                    position.score -= 10;
                }
                if (x === 0 || x === 49 || y === 0 || y === 49) {
                    position.score += isFleeing ? 20 : -20;
                }
            }, 2);
        }
    }
    ...
    private generateTileCache(pos: RoomPosition, encodedPos: string) {
        ...
        if (_.some(structures, s => !s.isWalkable())) {
            this.tileCache[encodedPos] = TILE_WALL;
            return;
        }
        ...
        for (const enemy of enemyCreeps) {
            ...
            for (const position of positions) {
                ...
                if (isRangedCreep) {
                    ...
                    if (distance === 2) {
                        position.score += isEnemyMeleeCreep ? 300 : 1000;
                    }
                    if (distance === 3) {
                        position.score += isEnemyMeleeCreep ? 1000 : 300;
                    }
                }
                else {
