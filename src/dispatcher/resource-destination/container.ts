...
export default ContainerDestination;
export class ContainerDestination extends StructureDestination<ContainerDestinationTask> {
    ...
    addControllerContainerTask(context: ResourceDestinationContext, options: ContainerDestinationTask[]) {
        if (!this.room.creepsByRole.upgrader && !this.room.creepsByRole.builder) {
            return;
        }
        ...
        const upgraderWorkParts = _.sumBy(Object.values(this.room.creepsByRole.upgrader), creep => creep.getActiveBodyparts(WORK));
        ...
        const totalNeededEnergy = container.store.getFreeCapacity() + usedEnergyUntilArrival - _.sumBy(otherDeliveringCreeps, c => c.store.getUsedCapacity(RESOURCE_ENERGY));
    ...
    // We need an adjusted `isValid` check because even if the container is full, the task might still be valid.
        ...
        if (structure.store.getFreeCapacity(task.resourceType) === 0 && context.creep.pos.getRangeTo(structure.pos) === 1) {
            return false;
        }
        ...
        const totalNeededEnergy = structure.store.getFreeCapacity() + usedEnergyUntilArrival - _.sumBy(otherDeliveringCreeps, (c) => {
            if (c.id === context.creep.id) {
                return 0;
            }
            if (c.pos.getRangeTo(structure.pos) > refillPathLength) {
                return 0;
            }

            return c.store.getUsedCapacity(RESOURCE_ENERGY);
        });
        if (totalNeededEnergy <= 0) {
            return false;
        }
