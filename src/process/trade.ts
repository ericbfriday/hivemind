...
export type TradeResource = ResourceConstant | InterShardResourceConstant;
...
export class TradeProcess extends Process {
   ...
   */
  run() {
    if (!hivemind.settings.get('enableTradeManagement')) {
      return;
    }

    // Only trade if we have a terminal to trade with.
    if (_.size(_.filter(Game.myRooms, room => room.terminal)) === 0) {
      return;
    }
  ...
  manageEnergyTradeOrders(resources: ResourceStates) {
    if (this.availableCredits > 0 && hivemind.settings.get('allowBuyingEnergy')) {
      ...
      _.each(resources.rooms, (roomState, roomName) => {
        if (!roomState.canTrade) {
          return;
        }
        if (roomState.isEvacuating) {
          return;
        }
        if ((roomState.totalResources[RESOURCE_ENERGY] || 0) > STORAGE_CAPACITY / 10) {
          return;
        }
  ...
  manageCommodityTradeOrders(resources: ResourceStates) {
    ...
    for (const resourceType of RESOURCES_ALL) {
      if (!resourceManager.isCommodityResource(resourceType) && !resourceManager.isDepositResource(resourceType)) {
        continue;
      }

      // If we can upgrade this commodity, don't sell it.
      if (this.canUpgradeCommodity(resourceType)) {
        continue;
      }
  ...
  getPowerCreepFactoryLevels(): number[] {
    return cache.inHeap('power-creep-factory-levels', 1000, () => {
      ...
      for (const powerCreep of Object.values(Game.powerCreeps)) {
        ...
        if (powerCreep.shard !== Game.shard.name) {
          continue;
        }
        ...
        if (!levels.includes(powerCreep.powers[PWR_OPERATE_FACTORY].level)) {
          levels.push(powerCreep.powers[PWR_OPERATE_FACTORY].level);
        }
      }
    ...
    return cache.inHeap('commodity-upgrade-levels', 100_000, () => {
      ...
      for (const [, commodity] of Object.entries(COMMODITIES)) {
        ...
        for (const component of getResourcesIn(commodity.components)) {
          if (!resourceManager.isCommodityResource(component)) {
            continue;
          }
          if (!levels[component]) {
            levels[component] = [];
          }
          if (!levels[component].includes(commodity.level)) {
            levels[component].push(commodity.level);
          }
        }
      }
  ...
  manageOverflowingTerminals(resources: ResourceStates) {
    ...
    for (const room of Game.myRooms) {
      ...
      if (room.storage && room.storage.store.getFreeCapacity() > room.storage.store.getCapacity() * 0.1) {
        continue;
      }
      if (room.terminal.store.getFreeCapacity() >= room.terminal.store.getCapacity() * 0.05) {
        continue;
      }

      const mostValuableResource = this.getMostValuableResource(room);
      if (!mostValuableResource) {
        continue;
      }
  ...
  instaBuyResources(resourceType: TradeResource, rooms?: Record<string, RoomResourceState>, force?: boolean) {
    ...
    if (!roomName && !isIntershardResource(resourceType)) {
      return;
    }
    ...
    if (bestOrder.price > maxPrice && !force) {
      return;
    }
  ...
  tryBuyResources(resourceType: TradeResource, rooms?: Record<string, RoomResourceState>, ignoreOtherRooms?: boolean) {
    if (!hivemind.settings.get('enableCreatingTradeOrders')) {
      return;
    }
  ...
  getRoomLowestOn(resourceType: ResourceConstant, rooms: Record<string, RoomResourceState>): string {
    ...
    _.each(rooms, (roomState: RoomResourceState, roomName: string) => {
      ...
      if (Game.rooms[roomName]?.isFullOn(resourceType)) {
        return;
      }
  ...
  getMaxOrderAmount(resourceType: TradeResource): number {
    ...
    if (history.total > 10_000) {
      return 10_000;
    }
    if (history.total > 1000) {
      return 1000;
    }
    ...
    return cache.inHeap(`price:${resourceType}`, 5000, () => {
      ...
      if (history.length > 3) {
        ...
        _.each(history, (day) => {
          ...
          if (day.date === minDay.date) {
            return;
          }
          if (day.date === maxDay.date) {
            return;
          }
          if (day.date === maxDev.date) {
            return;
          }
          if (day.resourceType !== resourceType) {
            return;
          }
      ...
      const currentStdDev = Math.sqrt(_.reduce(items, (total, item) => {
        return total + (item.price - currentValue / currentCount) ** 2 * item.amount;
      }, 0) / currentCount);
