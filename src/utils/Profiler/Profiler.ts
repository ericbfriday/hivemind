import type { Profiler, ProfilerMemory } from './typings';
...
export function profile(target: func): void;
export function profile(target: object, key: string | symbol, _descriptor?: TypedPropertyDescriptor<func>): void;
export function profile(
  target: object | func,
  key?: string | symbol,
  _descriptor?: TypedPropertyDescriptor<func>,
): void {
  // if (!globalThis.__PROFILER_ENABLED__) { return; }
...
function outputProfilerData() {
  ...
  const longestName = (_.maxBy(data, d => d.name.length) as OutputData).name.length + 2;
