...
export default class SegmentedMemory {
  ...
  reloadData() {
    ...
    for (let i = this.memory.startSegment; i <= this.memory.endSegment; i++) {
      ...
      if (typeof RawMemory.segments[i] === "undefined") {
        allLoaded = false;
        if (nextActiveSegments.length < maxActiveSegments)
          nextActiveSegments.push(i);
      ...
      try {
        ...
        hivemind
          .log("memory")
          .error("Failed to load segmented memory from segment " + i);
      }
  ...
  saveData() {
    ...
    _.each(this.data, (value, key) => {
      ...
      if (currentLength + partLength > maxSegmentLength) {
        ...
        if (
          typeof RawMemory.segments[this.currentSegment] === "undefined" &&
          this.savedSegments.length >= maxActiveSegments
        ) {
          // Can't save more data this tick.
          RawMemory.setActiveSegments(
            _.range(
      ...
      if (partLength > maxSegmentLength) {
        hivemind
          .log("memory")
          .notify(
            `Data for key "${key}" exceeds the maximum segment length (${(partLength / 1024).toPrecision(3)} kb).`,
          );
        return null;
      }

      stringified +=
        (stringified.length > 0 ? "," : "") + '"' + key + '":' + part;
  ...
  saveToCurrentSegment(data: string) {
    ...
    if (!this.savedSegments.includes(this.currentSegment))
      this.savedSegments.push(this.currentSegment);
  ...
  registerSaveCompletion() {
    ...
    hivemind
      ...
      .debug(
        "Saved",
        (this.totalLength / 1024).toPrecision(3) + "kB of data to",
