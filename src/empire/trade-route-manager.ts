...
export default class TradeRouteManager {
  ...
  public getAvailableTransportRoutes(): TransportRouteOption[] {
    ...
    each(rooms, (roomState: RoomResourceState, roomName: string) => {
      ...
      if (room.memory.fillTerminal && !needsTerminalSpace && !needsStorageSpace)
        return;
      ...
      for (const resourceType of getResourcesIn(roomState.state)) {
        ...
        if (
          needsTerminalSpace &&
          (roomState.totalResources[resourceType] || 0) < 100
        )
          continue;
        if (
          resourceType === RESOURCE_ENERGY &&
          (roomState.totalResources[resourceType] || 0) < 10_000
        )
          continue;
        ...
        each(rooms, (roomState2: any, roomName2: string) => {
          ...
          if (
            resourceType === RESOURCE_ENERGY &&
            this.roomNeedsTerminalSpace(room2) &&
            !this.isEvacuatingRoomWithLowEnergy(room2)
          )
            return;

          const resourceLevel2 =
            roomState2.state[resourceType] ||
            this.resourceLevelManager.determineResourceLevel(
              room2,
              roomState2.totalResources[resourceType] || 0,
              resourceType,
            );
          const isLow =
            resourceLevel2 === "low" ||
            (resourceType === RESOURCE_ENERGY &&
              room2.defense.getEnemyStrength() > ENEMY_STRENGTH_NONE &&
              resourceLevel2 === "medium");
          const isLowEnough = resourceLevel2 === "medium";
          const shouldReceiveResources =
            isLow || (resourceLevel === "excessive" && isLowEnough);

          if (
            !needsTerminalSpace &&
            !needsStorageSpace &&
            !shouldReceiveResources
          )
            return;

          // Make sure source room has enough energy to send resources.
          const neededEnergy =
            Game.market.calcTransactionCost(5000, roomName, roomName2) +
            (resourceType === RESOURCE_ENERGY ? 5000 : 0);
          if (room.terminal.store.energy < neededEnergy) return;

          const option = {
            priority: 3,
            weight:
              ((roomState.totalResources[resourceType] || 0) -
                (roomState2.totalResources[resourceType] || 0)) /
                100_000 -
          ...
          if (needsTerminalSpace && resourceType !== RESOURCE_ENERGY) {
            ...
            if (
              ...
              room.terminal.store[resourceType] >= 5000
            ) {
              option.priority++;
            }
          } else if (!isLow) {
  ...
  private addResourceRequestOptions(
    options: TransportRouteOption[],
    room: Room,
    resourceType: ResourceConstant,
    roomState: RoomResourceState,
  ) {
    ...
    for (const roomName2 in Memory?.requests?.trade?.[resourceType] || {}) {
      ...
      const shouldReceiveResources =
        (roomState.state[resourceType] === "excessive" &&
          info.priority >= 0.05) ||
        (roomState.state[resourceType] === "high" &&
          (info.priority >= 0.5 || tier > 1)) ||
        (roomState.state[resourceType] === "medium" && info.priority >= 0.8) ||
        (roomState.state[resourceType] === "medium" &&
          info.priority >= 0.5 &&
          tier > 1) ||
        (roomState.state[resourceType] === "low" &&
          info.priority >= 0.8 &&
          tier > 1) ||
        (roomState.state[resourceType] === "low" &&
          info.priority >= 0.5 &&
      ...
      if (
        ...
        Game.market.calcTransactionCost(5000, room.name, roomName2)
      )
        continue;
      ...
      const option = {
        priority: 3,
        weight:
          roomState.totalResources[resourceType] / 100_000 -
          Game.map.getRoomLinearDistance(room.name, roomName2),
        resourceType,
        source: room.name,
        target: roomName2,
      };
      ...
      if (
        this.roomNeedsTerminalSpace(room) &&
        resourceType !== RESOURCE_ENERGY
      ) {
        option.priority++;
        if (
      ...
      return (
        ...
        (room.isClearingTerminal() &&
          ...
          room.storage.store.getFreeCapacity() <
            room.storage.store.getCapacity() * 0.3) ||
        (room.isClearingStorage() &&
          room.terminal &&
          room.terminal.store.getFreeCapacity() <
            room.terminal.store.getCapacity() * 0.3)
      );
    });
  }
  ...
  public roomNeedsStorageSpace(room: Room): boolean {
    return cache.inObject(room, "roomNeedsStorageSpace", 1, () => {
      return (
        room.terminal &&
        room.terminal.store.getFreeCapacity() <
          room.terminal.store.getCapacity() * 0.1 &&
        room.storage &&
        room.storage.store.getFreeCapacity() <
