...
export default class RoomsReport {
  ...
  getRoomTableData(): string[][] {
    const roomData: string[][] = [
      ["Room", "RCL", "Energy", "Storage", "Mineral", "Labs"],
    ];
    ...
    for (const data of this.getAllRoomInfo()) {
      ...
      roomData.push([
        ...
        data.storagePercent === null
          ? "-"
          : data.storagePercent.toFixed(1) + "%",
        data.mineralType,
        (data.currentReaction ? data.currentReaction.join("+") : "-") +
          (data.labUsage === null
            ? ""
            : " (" + (data.labUsage * 100).toFixed(1) + "%)"),
      ]);
    }
    ...
    return {
      ...
      storagePercent:
        room.storage || room.terminal
          ...
          : null,
      labUsage:
        (room.memory.labUsage?.total || 0) === 0
          ? null
          : room.memory.labUsage.busy / room.memory.labUsage.total,
    };
  }
