...
export class ManageFactoryProcess extends Process {
   ...
   */
  run() {
    if (!this.room.factory || !this.room.factoryManager) {
      return;
    }
    ...
    for (product in jobs) {
      ...
      if (this.room.factory.cooldown) {
        continue;
      }

      if (this.room.factory.produce(product as CommodityConstant) === OK && settings.get('notifyFactoryProduction')) {
        Game.notify(`Produced ${product} in ${this.room.name}.`);
      }
    }
