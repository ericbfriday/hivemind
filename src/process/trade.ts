...
function isIntershardResource(
  resourceType: TradeResource,
): resourceType is InterShardResourceConstant {
...
export default class TradeProcess extends Process {
  ...
  manageTradeOrders() {
    ...
    if (
      ...
      hivemind.settings.get("allowBuyingPixels")
    ) {
  ...
  manageTier1ResourceTradeOrder(
    ...
    if (
      (total.resources[resourceType] || 0) < lowStorage &&
      this.availableCredits > 0
    ) {
    ...
    if (
      (total.resources[resourceType] || 0) < minStorage &&
      this.availableCredits > 0
    ) {
  ...
  manageCompoundResourceTradeOrder(
    resourceType: ResourceConstant,
    resources: ResourceStates,
  ) {
  ...
  manageEnergyTradeOrders(resources: ResourceStates) {
    if (
      this.availableCredits > 0 &&
      hivemind.settings.get("allowBuyingEnergy")
    ) {
      // Buy energy for rooms under attack so we can hold out longer.
      for (const room of Game.myRooms) {
        if (
          room.getEffectiveAvailableEnergy() < 30_000 &&
          room.defense.getEnemyStrength() >= ENEMY_STRENGTH_NORMAL
        ) {
          if (
            room.factory &&
            room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) > 500
          ) {
            this.instaBuyResources(
              RESOURCE_BATTERY,
              { [room.name]: resources.rooms[room.name] },
              true,
            );
          } else {
            this.instaBuyResources(
      ...
      each(resources.rooms, (roomState, roomName) => {
        ...
        if (
          (roomState.totalResources[RESOURCE_ENERGY] || 0) >
          STORAGE_CAPACITY / 10
        )
          return;
  ...
  manageOverflowingTerminals(resources: ResourceStates) {
    ...
    for (const room of Game.myRooms) {
      ...
      if (
        ...
        room.storage.store.getCapacity() * 0.1
      )
        continue;
      if (
        room.terminal.store.getFreeCapacity() >=
        room.terminal.store.getCapacity() * 0.05
      )
        continue;
      ...
      this.instaSellResources(mostValuableResource, {
        [room.name]: resources.rooms[room.name],
      });
    }
  }
    ...
    for (const room of Game.myRooms) {
      ...
      for (const resourceType of getResourcesIn(roomData.totalResources)) {
        total.resources[resourceType] =
          (total.resources[resourceType] || 0) +
          roomData.totalResources[resourceType];
      }
  ...
  instaSellResources(
    ...
    const amount = Math.min(
      this.getMaxOrderAmount(resourceType),
      bestOrder.amount,
    );
    ...
    if (amount > (room.terminal.store[resourceType] || 0)) {
      if (room.memory.fillTerminal) {
        hivemind
          .log("trade", roomName)
          .info("Busy, can't prepare", amount, resourceType, "for selling.");
      } else {
  ...
  instaBuyResources(
    ...
    const roomName = isIntershardResource(resourceType)
      ? null
      : this.getRoomLowestOn(resourceType, rooms);
    ...
    const maxPrice =
      history.average - Math.min(history.stdDev / 5, history.average * 0.1);
    hivemind
      .log("trade", roomName)
      .debug(
        "Could buy",
        ...
        "for",
        bestOrder.price,
        "- we want to spend at most",
        maxPrice,
      );
    if (bestOrder.price > maxPrice && !force) return;

    let amount = Math.min(
      force ? 10_000 : this.getMaxOrderAmount(resourceType),
      bestOrder.amount,
    );
    if (isIntershardResource(resourceType)) {
      ...
      if (transactionCost > room.terminal.store.energy) {
        ...
        if (force) {
          amount = Math.floor(
            (amount * room.terminal.store.energy) / transactionCost,
          );
  ...
  tryBuyResources(
    ...
    const maxPrice =
      history.average - Math.min(history.stdDev / 2, history.average * 0.2);
    ...
    if (bestBuyOrder) {
      ...
      hivemind
        .log("trade", roomName)
        .info("Nobody else is currently buying", resourceType);
      offerPrice =
        history.average - Math.min(history.stdDev, history.average * 0.8);
    }
    ...
    if (result !== OK) {
      hivemind
        .log("trade", roomName)
        .error("Could not create buy order:", result);
    }
  }
  ...
  trySellResources(
    ...
    if (bestSellOrder) {
      ...
      hivemind
        .log("trade", roomName)
        .info(resourceType, "is currently being sold for", bestSellOrder.price);
      offerPrice = Math.min(
      ...
      hivemind
        .log("trade", roomName)
        .info("Nobody else is currently selling", resourceType);
  ...
  getRoomLowestOn(
    ...
    each(rooms, (roomState: RoomResourceState, roomName: string) => {
      ...
      if (
        !minAmount ||
        (roomState.totalResources[resourceType] || 0) < minAmount
      ) {
  ...
  getRoomHighestOn(
    ...
    each(rooms, (roomState: RoomResourceState, roomName: string) => {
      ...
      if (
        !maxAmount ||
        (roomState.totalResources[resourceType] || 0) > maxAmount
      ) {
   ...
   */
  getPriceData(resourceType: TradeResource): {
    total: number;
    average: number;
    stdDev: number;
  } {
    return cache.inHeap("price:" + resourceType, 5000, () => {
      ...
      const minDay = minBy(history, "avgPrice");
      const maxDay = maxBy(history, "avgPrice");
      const maxDev = maxBy(history, "stddevPrice");
      ...
      const reagentWorth = reduce(
        ...
        (total: number, componentType: ResourceConstant) => {
          ...
          return (
            ...
            Math.max(
              componentWorth,
              componentHistory ? componentHistory.average : 0,
            )
          );
        },
        0,
