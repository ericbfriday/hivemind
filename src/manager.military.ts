...
Room.prototype.assertMilitaryStructurePower = function (this: Room, structure: StructureTower) {
    if (structure.store[RESOURCE_ENERGY] < TOWER_ENERGY_COST) {
        return;
    }
...
Room.prototype.getTowerTarget = function (this: Room) {
    return cache.inObject(this, 'towerTarget', 1, () => {
        ...
        let max = null;
        for (const creep of this.militaryObjects.creeps) {
            if (!creep.militaryPriority) {
                continue;
            }
            if (creep.militaryPriority <= 0) {
                continue;
            }
            if (max && max.militaryPriority > creep.militaryPriority) {
                continue;
            }
        ...
        if (max) {
            this.visual.circle(max.pos.x, max.pos.y, { radius: 1, fill: 'red', opacity: 0.5 });
        }
