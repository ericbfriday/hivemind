...
export default class LabDestination extends StructureDestination<LabDestinationTask> {
  ...
  getTasks(context: ResourceDestinationContext) {
    ...
    return this.cacheEmptyTaskListFor(context.resourceType || "", 25, () => {
      ...
      this.addLabTask(
        this.room.memory.currentReaction[0],
        this.room.memory.labs.source1,
        options,
        context,
      );
      this.addLabTask(
        this.room.memory.currentReaction[1],
        this.room.memory.labs.source2,
        options,
        context,
      );
  ...
  addLabTask(
    ...
    const option: LabDestinationTask = {
      priority:
        3 +
        (lab.mineralType
          ? lab.store[resourceType] > LAB_REACTION_AMOUNT
            ? 0
            : 1
          : 1),
