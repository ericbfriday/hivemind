...
declare global {
  ...
  export interface PowerTargetRoom {
    decays: number
    isActive?: boolean
    amount: number
    hits: number
...
export default PowerMiningProcess;
export class PowerMiningProcess extends Process {
  mesh: NavMesh;
  ...
  run() {
    ...
    _.each(memory.rooms, (info, roomName) => {
      ...
      // Don't harvest new power banks if we have too much power already.
      if (storedPowerLevel > 100_000) {
        return;
      }
      ...
      if (info.amount <= 0 || info.hits <= 0) {
        return;
      }

      // Skip if low amount.
      if (info.amount < hivemind.settings.get('powerBankMinAmount')) {
        return;
      }
  ...
  getPotentialSpawnRoomsForHarvesting(roomName: string, maxDistance: number): Array<{ room: string, distance: number }> {
    ...
    for (const room of Game.myRooms) {
      ...
      if (room.getEffectiveAvailableEnergy() < hivemind.settings.get('minEnergyForPowerHarvesting')) {
        continue;
      }
      if (room.controller.level < hivemind.settings.get('minRclForPowerMining')) {
        continue;
      }
      if (Game.map.getRoomLinearDistance(roomName, room.name) > maxDistance) {
        continue;
      }
      ...
      if (roomRoute.incomplete || roomRoute.path.length > 2 * maxDistance) {
        continue;
      }
  ...
  logHarvestIntent(roomName, info) {
    ...
    if (!Memory.strategy || !Memory.strategy.reports) {
      return;
    }
