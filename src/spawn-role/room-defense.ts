...
declare global {
  export interface RoomDefenseSpawnOption extends SpawnOption {
    responseType?: number
    creepRole: string
  }
}
...
export default RoomDefenseSpawnRole;
export class RoomDefenseSpawnRole extends SpawnRole {
  /**
  ...
  addLowLevelRoomSpawnOptions(room: Room, options: RoomDefenseSpawnOption[]) {
    ...
    if (_.size(room.creepsByRole.brawler) >= 2) {
      return;
    }
  ...
  addRampartDefenderSpawnOptions(room: Room, options: RoomDefenseSpawnOption[]) {
    ...
    if (_.size(room.creepsByRole.guardian) >= 5) {
      return;
    }
  ...
  addEmergencyRepairSpawnOptions(room: Room, options: RoomDefenseSpawnOption[]) {
    ...
    if (_.size(room.creepsByRole.builder) >= 5) {
      return;
    }
  ...
  getCreepBody(room: Room, option: RoomDefenseSpawnOption): BodyPartConstant[] {
    if (option.creepRole === 'builder') {
      return this.getRepairCreepBody(room);
    }
  ...
  private getMaxEnemyBoostLevel(room: Room): number {
    ...
    for (const userName in room.enemyCreeps) {
      ...
      highest = Math.max(highest, _.maxBy(_.map(room.enemyCreeps[userName], creep => _.maxBy(_.map(creep.body, (part) => {
