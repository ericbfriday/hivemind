...
export default class FactoryManager {
  ...
  hasAllComponents(product: FactoryProductConstant): boolean {
    ...
    for (resourceType in recipe.components) {
      if (
        this.room.factory.store.getUsedCapacity(resourceType) <
        recipe.components[resourceType]
      )
        return false;
    }
  ...
  getMissingComponents(): Partial<
    Record<FactoryComponentConstant, number>
  > | null {
    ...
    for (resourceType in requestedResources) {
      if (
        (this.room.factory.store[resourceType] || 0) >
  ...
  getRequestedComponents(): Partial<Record<FactoryComponentConstant, number>> {
    ...
    for (productName in jobs) {
      ...
      const amount = Math.max(
        1,
        factoryFillTime / recipe.cooldown / numberJobs,
      );
      let resourceType: FactoryComponentConstant;
      for (resourceType in recipe.components) {
        neededResources[resourceType] =
          (neededResources[resourceType] || 0) +
          Math.ceil(recipe.components[resourceType] * amount);
      }
    }
  ...
  getJobs(): Partial<Record<FactoryProductConstant, Recipe>> {
    return cache.inHeap("factoryJobs:" + this.roomName, 50, () => {
      ...
      for (resourceType in COMMODITIES) {
        if (this.room.isEvacuating() && resourceType !== RESOURCE_ENERGY)
          continue;

        const recipe: Recipe = COMMODITIES[resourceType];

        if (this.isRecipeAvailable(resourceType, recipe))
          result[resourceType] = recipe;
      }
  ...
  isRecipeAvailable(
    resourceType: FactoryProductConstant,
    recipe: Recipe,
  ): boolean {
    ...
    const storageFull =
      this.room.getFreeStorage() - this.room.factory.store.getUsedCapacity() <
      this.room.getStorageLimit() * 0.05;
    const storageEmpty =
      this.room.getFreeStorage() - this.room.factory.store.getUsedCapacity() >
      this.room.getStorageLimit() * 0.95;
    const minRawMaterialRatio = 0.2;
    const maxRawMaterialRatio = 0.8;

    const storedEnergy =
      this.room.getStoredEnergy() +
      this.room.factory.store.getUsedCapacity(RESOURCE_ENERGY);
    const storedProduct =
      this.room.getCurrentResourceAmount(resourceType) +
      this.room.factory.store.getUsedCapacity(resourceType);
    const storedResource =
      this.room.getCurrentResourceAmount(
        uncompressRecipes[resourceType] || compressRecipes[resourceType],
      ) +
      this.room.factory.store.getUsedCapacity(
        uncompressRecipes[resourceType] || compressRecipes[resourceType],
      );

    if (resourceType === RESOURCE_BATTERY) {
      const rawMaterialRatio =
        storedResource / Math.max(storedProduct + storedResource, 1);
      return (
        (storedProduct < 500 ||
          storageFull ||
    ...
    if (resourceType === RESOURCE_ENERGY) {
      const rawMaterialRatio =
        storedProduct / Math.max(storedProduct + storedResource, 1);
      return (
  ...
  mayCreateCommodities(
    product: FactoryProductConstant,
    recipe: Recipe,
  ): boolean {
    const createdAmount =
      this.room.getCurrentResourceAmount(product) / recipe.amount;
    ...
    if (this.isMadeOnlyFromBasicResources(recipe)) {
      for (const resourceType in recipe.components) {
        ...
        if (
          resourceAmount <
          recipe.components[resourceType] * createdAmount * 5
        )
          return false;
      }
  ...
  isMadeOnlyFromBasicResources(recipe: Recipe): boolean {
    for (const resourceType in recipe.components) {
      if (!compressRecipes[resourceType] && !uncompressRecipes[resourceType])
        return false;
    }
