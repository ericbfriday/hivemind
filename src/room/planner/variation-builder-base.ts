...
export default class RoomVariationBuilderBase {
  ...
  buildNextStep() {
    ...
    if (stepResult === "done") {
      ...
      hivemind
        ...
        .info("Finished building room plan variation", this.variation);
    }
  }
  ...
  placeBayStructures(
    bayPosition: RoomPosition,
    options: { spawn?: boolean; source?: boolean; id?: number } = {},
  ) {
    ...
    if (this.roomPlan.canPlaceMore("spawn") && options.spawn) {
      handleMapArea(bayPosition.x, bayPosition.y, (x, y) => {
        ...
        if (!this.placementManager.isBuildableTile(x, y, false, allowNearExit))
          return true;
        ...
        handleMapArea(x, y, (x2, y2) => {
          ...
          if (
            !this.roomPlan.hasPosition(
              "road",
              new RoomPosition(x2, y2, this.roomName),
            )
          )
            return true;
          ...
          this.placementManager.planLocation(
            ...
            "spawn." + this.roomPlan.getPositions("spawn").length,
          );
          this.placementManager.planLocation(
            new RoomPosition(x, y, this.roomName),
            "spawn",
  ...
  findContainerPosition(sourceRoads: RoomPosition[]): RoomPosition {
    if (
      sourceRoads[0] &&
      this.placementManager.isBuildableTile(
        sourceRoads[0].x,
        sourceRoads[0].y,
        true,
        true,
  ...
  protectPosition(pos: RoomPosition, distance?: number) {
    if (typeof distance === "undefined")
      distance = hivemind.settings.get("minCutRampartDistance");
