...
export default class DropSource extends TaskProvider<
  DropSourceTask,
...
> {
  ...
  private addDroppedResourceOptions(
    options: DropSourceTask[],
    ...
    for (const resource of resources) {
      ...
      const option: DropSourceTask = {
        ...
        weight:
          resource.amount /
          (resource.resourceType === RESOURCE_ENERGY ? 100 : 30), // @todo Also factor in distance.
        type: this.getType(),
        target: resource.id,
      ...
      if (resource.resourceType === RESOURCE_POWER) {
        ...
        if (
          storagePosition &&
          resource.pos.x === storagePosition.x &&
          resource.pos.y === storagePosition.y
        ) {
          option.priority =
          ...
          if (
            ...
            creep.room.getCurrentResourceAmount(RESOURCE_ENERGY) > 5000 &&
            creep.memory.role === "transporter"
          )
            option.priority -= 2;
          ...
          if (
            creep.room.storage &&
            creep.room.getFreeStorage() < resource.amount &&
            creep.room.getEffectiveAvailableEnergy() > 20_000
          ) {
      ...
      if (resource.amount < creep.store.getCapacity() * 2) {
        ...
        option.priority -=
          this.room.getCreepsWithOrder("drop", resource.id).length * 2;
      }
  ...
  isValid(task: DropSourceTask, context: ResourceSourceContext): boolean {
    ...
    if (context.creep.store.getFreeCapacity(resource.resourceType) === 0)
      return false;
    ...
    const terminalNeedsSpaceForEnergy =
      ...
      terminal.store.getFreeCapacity() +
        ...
        5000;
    if (task.resourceType !== RESOURCE_ENERGY && terminalNeedsSpaceForEnergy)
      return false;
