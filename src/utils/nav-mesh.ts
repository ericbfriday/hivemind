import type { ProfilerMemory } from './Profiler/typings';
...
export interface NavMeshPathfindingEntry {
  exitId: number
  pos: number
  roomName: string
...
export class NavMesh {
  ...
  generateForRoom(roomName: string) {
    ...
    for (const region of regions) {
      try {
        const centerX = region.center as number % 50;
        const centerY = Math.floor(region.center as number / 50);
        regionMem.push({
          exits: region.exits,
          center: serializeCoords(centerX, centerY),
        });
        ...
        console.error(`Error serializing region center for room ${roomName}: ${err}`);
      }
    }
  ...
  isAvailableExitDirection(roomName: string, dir: DirectionConstant): boolean {
    const otherRoomName = (Game.map.describeExits(roomName) as ExitsInformation)[dir];
    if (!otherRoomName) {
      return false;
    }
    ...
    while (startPos) {
      ...
      while (!region.center && range < 25) {
        for (const coords of [
          ...
          if (this.costMatrix.get(x, y) !== 200 + regions.length) {
            continue;
          }
  ...
  getUntouchedExit(region: RegionInfo, exits: ExitInfo[]): number | null {
    for (const exit of exits) {
      if (exit.touched) {
        continue;
      }
  ...
  addConnectingPathsForRegion(region: RegionInfo, paths: Record<number, Record<number, number>>, roomName: string, costMatrix: CostMatrix) {
    ...
    for (const exitId of region.exits) {
      ...
      for (const exitId2 of region.exits) {
        ...
        if (paths[exitId2] && paths[exitId2][exitId]) {
          continue;
        }
  ...
  getPortals(roomName: string) {
    ...
    for (const portal of room.structuresByType[STRUCTURE_PORTAL] || []) {
      ...
      if (this.isPortalBlocked(room, portal.pos)) {
        continue;
      }
    ...
    return _.map(portals, (portal) => {
      const pos = _.minBy(portal.positions, pos => pos.getRangeTo(
  ...
  isPortalBlocked(room: Room, pos: RoomPosition): boolean {
    ...
    handleMapArea(pos.x, pos.y, (x, y) => {
      ...
      if (this.costMatrix.get(x, y) >= 100) {
        return;
      }
  ...
  estimateTravelTime(startPos: RoomPosition, endPos: RoomPosition): number | null {
  ...
  findPath(startPos: RoomPosition, endPos: RoomPosition, options?: { maxPathLength?: number, allowDanger?: boolean, maxCpu?: number }): {
    ...
    while (openList.length > 0) {
      ...
      if (current.portal) {
        ...
        if (closedList[`${nextRoom}/${exitPos}`]) {
          continue;
        }
      ...
      if (hivemind.segmentMemory.isReady()) {
        ...
        if (roomIntel.isOwned()) {
          if (!options.allowDanger && !hivemind.relations.isAlly(roomIntel.getOwner())) {
            continue;
          }
        }
      }

      const costMultiplier = this.calculateCostMultiplier(nextRoom!, options.allowDanger ?? false);
      for (const exit of this.getAvailableExitsCorrespondingTo(nextRoom!, correspondingExit, current.portal)) {
        // Check if in closed list.
        if (closedList[`${nextRoom!}/${exit.center}`]) {
          continue;
        }
        if (openListLookup[`${nextRoom!}/${exit.center}`]) {
          continue;
        }
      ...
      for (const portal of roomMemory.portals || []) {
        // Check if in closed list.
        if (closedList[`${nextRoom!}/${portal.pos}`]) {
          continue;
        }
        if (openListLookup[`${nextRoom!}/${portal.pos}`]) {
          continue;
        }
        ...
        } as unknown as NavMeshPathfindingEntry;
  ...
  getAvailableExits(startPos: RoomPosition, roomName: string): Array<{
    ...
    for (const region of roomMemory.regions) {
      ...
      const result = PathFinder.search(
        startPos,
        deserializePosition(region.center, roomName) as RoomPosition,
        {
          roomCallback: () => costMatrix,
          maxRooms: 1,
        },
  ...
  getAvailableExitsCorrespondingTo(roomName: string, exitId: number | null, isPortal: boolean): Array<{
    ...
    return _.filter(roomMemory.exits, (exit: {
      id: number
      center: number
    }) => (exit.id !== exitId && region.exits.includes(exit.id) as boolean));
  }
  ...
  popBestCandidate(openList: NavMeshPathfindingEntry[]): NavMeshPathfindingEntry {
    ...
    if (minId && (minId < openList.length - 1)) {
  ...
  pluckRoomPath(current: NavMeshPathfindingEntry): RoomPosition[] {
    ...
    return path.reverse() as RoomPosition[];
  }
  ...
  getRoomDistance(room1: string, room2: string): number {
    ...
    if (route.incomplete) {
      return roomLinearDistance;
    }

    return Math.min((route.path?.length || 0) - 1, roomLinearDistance);
  }
}
