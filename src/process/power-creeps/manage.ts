...
export default class ManagePowerCreepsProcess extends Process {
  ...
  run() {
    ...
    const creepToUpgrade = minBy(
      this.getUpgradeablePowerCreeps(),
      (creep) => creep.level,
    );
    if (
      typeof creepToUpgrade !== "undefined" &&
      typeof creepToUpgrade !== "number"
    ) {
  ...
  getUpgradeablePowerCreeps(): PowerCreep[] {
    return filter(Game.powerCreeps, (creepToUpgrade: PowerCreep) => {
      ...
      const skipForNewCreep =
        smallPCLevel && creepToUpgrade.level >= smallPCLevel;
  ...
  upgradePowerCreep(creep: PowerCreep) {
    for (const powerOption of hivemind.settings.get("powerPriorities")) {
      ...
      if (
        powerOption === PWR_OPERATE_FACTORY && // Special handling for OPERATE_FACTORY.
        currentLevel >= this.getDesiredFactoryLevel(creep)
      )
        continue;
  ...
  getDesiredFactoryLevel(creep: PowerCreep): number {
    ...
    if (currentLevel > 0) {
      if (
        (levelDistribution[currentLevel] || 0) >=
        (levelDistribution[currentLevel + 1] || 0) + 2
      ) {
        return Math.min(
          currentLevel + 1,
          POWER_INFO[PWR_OPERATE_FACTORY].level.length,
        );
      }
