...
export type EnemyStrength =
  | typeof ENEMY_STRENGTH_NONE
...
export default class RoomDefense {
  ...
  getLowestWallStrength(): number {
    return cache.inHeap("weakestWallStrength:" + this.room.name, 5, () => {
      ...
      const rampartPositions: RoomPosition[] =
        this.room.roomPlanner.getLocations("rampart");
      let minHits: number;

      for (const pos of rampartPositions) {
        if (this.room.roomPlanner.isPlannedLocation(pos, "rampart.ramp"))
          continue;
  ...
  getEnemyStrength(): EnemyStrength {
    return cache.inHeap("getEnemyStrength:" + this.roomName, 5, () => {
      ...
      for (const userName in this.room.enemyCreeps) {
        ...
        for (const creep of creeps) {
          for (const part of creep.body) {
            ...
            if (
              (
                [ATTACK, RANGED_ATTACK, CLAIM, WORK] as BodyPartConstant[]
              ).includes(part.type)
            ) {
      ...
      const towerStrength =
        (TOWER_POWER_ATTACK *
          (this.room.myStructuresByType[STRUCTURE_TOWER] || []).length) /
        2;

      // Active defense is calculated as having 2 creeps
      // with 50% attack and move parts.
      const defensiveCreepStrength =
        2 *
        ATTACK_POWER *
        Math.min(
          MAX_CREEP_SIZE / 2,
          Math.floor(
            this.room.energyCapacityAvailable /
              (BODYPART_COST[ATTACK] + BODYPART_COST[MOVE]),
          ),
        );
      ...
      if (
        invaderOnly ||
        (healStrength < towerStrength && attackStrength < highDamageThreshold)
      )
        return ENEMY_STRENGTH_WEAK;
      if (
        healStrength < towerStrength + defensiveCreepStrength &&
        attackStrength > normalDamageThreshold
      )
        return ENEMY_STRENGTH_NORMAL;
      if (
        healStrength <
          towerStrength + defensiveCreepStrength * defenseBoostPower &&
        attackStrength > highDamageThreshold
      )
        return ENEMY_STRENGTH_STRONG;
  ...
  openRampartsToFriendlies() {
    ...
    _.each(this.room.enemyCreeps, (creeps, username) => {
      ...
      for (const creep of creeps) {
        ...
        if (
          !this.isWhitelisted(username) ||
          (!this.isUnarmedCreep(creep) && !hivemind.relations.isAlly(username))
        ) {
        ...
        if (
          numberInRoom >= hivemind.settings.get("maxVisitorsPerUser") &&
          !this.isInRoom(creep)
        ) {
  ...
  calculatePlayerTrade(
    ...
    hivemind
      .log("trade", this.roomName)
      .notify("Trade with", username, ":", amount, resourceType);
  }
