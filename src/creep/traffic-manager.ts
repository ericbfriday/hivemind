...
export class TrafficManager {
  ...
  manageTraffic() {
    ...
    _.each([..._.values<Creep>(Game.creeps), ...powerCreeps], (creep) => {
      ...
      if (creep._hasMoveIntent) {
        return;
      }

      const blockedCreep = creep._blockingCreepMovement;
      if (blockedCreep instanceof Creep && blockedCreep.fatigue) {
        return;
      }
  ...
  getAlternateCreepPosition(creep: Creep | PowerCreep): RoomPosition | null {
    if (!creep._requestedMoveArea && !creep._alternatePositions) {
      return null;
    }
    ...
    handleMapArea(creep.pos.x, creep.pos.y, (x, y) => {
      ...
      if (this.dangerMatrix.get(x, y) > 0) {
        return null;
      }

      const pos = new RoomPosition(x, y, creep.room.name);
      if (!this.isAvailableAlternatePosition(creep, pos)) {
        return null;
      }
    ...
    if (!alternatePosition && blockingCreeps.length > 0) {
      ...
      for (const blockingCreep of blockingCreeps) {
        ...
        if (blockingCreep._hasMoveIntent) {
          continue;
        }
        if (blockingCreep._blockingCreepMovement) {
          continue;
        }
        if (blockingCreep instanceof Creep && blockingCreep.fatigue) {
          continue;
        }
  ...
  isAvailableAlternatePosition(creep: Creep | PowerCreep, position: RoomPosition): boolean {
    if (creep._requestedMoveArea && position.getRangeTo(creep._requestedMoveArea.pos) <= creep._requestedMoveArea.range) {
      return true;
    }

    if (creep._alternatePositions) {
      for (const alternatePosition of creep._alternatePositions) {
        if (position.isEqualTo(alternatePosition)) {
          return true;
        }
      }
    }
