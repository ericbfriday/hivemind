...
export default class NavMesh {
      ...
      while (openList.length > 0) {
        ...
        handleMapArea(currentPos % 50, Math.floor(currentPos / 50), (x, y) => {
          ...
          if (matrixValue > 10) {
            if (
              matrixValue < 200 &&
              this.exitLookup[matrixValue - 100] &&
              !this.exitLookup[matrixValue - 100].touched
            ) {
  ...
  getUntouchedExit(region: RegionInfo, exits: ExitInfo[]): number {
    for (const exit of exits) {
      ...
      const pos = exit.vertical
        ? exit.offset + 50 * exit.center
        : exit.center + 50 * exit.offset;
  ...
  getPortals(roomName: string) {
    ...
    return map(portals, (portal) => {
      const pos = minBy(portal.positions, (pos) =>
        pos.getRangeTo(
  ...
  estimateTravelTime(startPos: RoomPosition, endPos: RoomPosition): number {
    return cache.inHeap(
      "travelTime:" + encodePosition(startPos) + ":" + encodePosition(endPos),
      1000,
      () => {
  ...
  findPath(
    ...
    this.addExitsToOpenList(
      this.getAvailableExits(startPos, startRoom),
      openList,
      openListLookup,
      startRoom,
      endRoom,
    ...
    while (openList.length > 0) {
      ...
      const nextRoom = current.portal
        ? current.targetRoom
        : this.getAdjacentRoom(current.roomName, current.exitId);
      const correspondingExit = current.portal
        ? null
        : this.getCorrespondingExitId(current.exitId);
      ...
      if (
        ...
        current.pathLength >= options.maxPathLength
      ) {
        continue;
      }
      ...
      if (hivemind.segmentMemory.isReady()) {
        ...
        if (roomIntel.isOwned()) {
          if (
            !options.allowDanger &&
            !hivemind.relations.isAlly(roomIntel.getOwner())
          )
            continue;
        }
      }

      const costMultiplier = this.calculateCostMultiplier(
        nextRoom,
        options.allowDanger,
      );
      for (const exit of this.getAvailableExitsCorrespondingTo(
        nextRoom,
        correspondingExit,
        current.portal,
      )) {
        ...
        if (!current.portal) {
          ...
          const noPath1 =
            !roomMemory.paths[exit.id] ||
            !roomMemory.paths[exit.id][correspondingExit];
          const noPath2 =
            !roomMemory.paths[correspondingExit] ||
            !roomMemory.paths[correspondingExit][exit.id];
          if (noPath1 && noPath2) continue;
        }

        const segmentLength = current.portal
          ? 25
          : (roomMemory.paths[exit.id] &&
            roomMemory.paths[exit.id][correspondingExit]) ||
          roomMemory.paths[correspondingExit][exit.id];
        const item = {
          ...
          heuristic:
            (Game.map.getRoomLinearDistance(nextRoom, endRoom) - 1) * 50,
          portal: false,
        };
        ...
        if (nextRoom === endRoom) {
          ...
          item.pathLength =
            current.pathLength +
            costMultiplier *
            (current.portal ? 25 : roomMemory.paths[correspondingExit][0]);
          item.totalSteps =
            current.totalSteps +
            (current.portal ? 25 : roomMemory.paths[correspondingExit][0]);
        }
  ...
  getAvailableExitsCorrespondingTo(
    roomName: string,
    exitId: number | null,
    isPortal: boolean,
  ): Array<{
    ...
    ) as typeof roomMemory.exits;
  }

  addExitsToOpenList(
    exits: Array<{ id: number; center: number }>,
    openList: NavMeshPathfindingEntry[],
    openListLookup: Record<string, boolean>,
    roomName: string,
    endRoom: string,
  ) {
    const roomMemory = this.memory.rooms[roomName];
    for (const exit of exits) {
      const segmentLength = roomMemory.paths[exit.id]
        ? roomMemory.paths[exit.id][0]
        : 50;
  ...
  calculateCostMultiplier(roomName: string, allowDanger: boolean): number {
    ...
    if (
      Memory.rooms[roomName]?.enemies &&
      !Memory.rooms[roomName]?.enemies?.safe &&
      !allowDanger
    ) {
  ...
  getAdjacentRoom(roomName: string, exitId: number): string {
    ...
    switch (dir) {
      case 0:
        ...
        if (parts[3] === "N") {
          return (
            parts[1] + parts[2] + parts[3] + (Number.parseInt(parts[4], 10) + 1)
          );
        }
      ...
      case 1:
        ...
        if (parts[1] === "E") {
          return (
            parts[1] + (Number.parseInt(parts[2], 10) + 1) + parts[3] + parts[4]
          );
        }
  ...
  cleanMemory() {
    for (const roomName in this.memory.rooms) {
      if (Game.time - this.memory.rooms[roomName].gen < CREEP_LIFE_TIME * 50)
        continue;
