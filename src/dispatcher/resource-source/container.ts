...
export default class ContainerSource extends StructureSource<ContainerSourceTask> {
  ...
  private emptyEnergyFromContainers(
    options: ContainerSourceTask[],
    ...
    for (const container of containers) {
      ...
      for (const source of container.room.sources) {
        ...
        option.priority++;
        if (
          container.store.getUsedCapacity() >= creep.store.getFreeCapacity() && // This container is filling up, prioritize emptying it when we aren't
      ...
      for (const bay of container.room.bays) {
        ...
        if (
          !container.room.roomPlanner.isPlannedLocation(
            container.pos,
            "harvester",
          )
        )
      ...
      option.priority -=
        this.room.getCreepsWithOrder("container", container.id).length * 3;

      options.push(option);
    }
  }

  private emptyResourcesFromContainers(
    options: ContainerSourceTask[],
    context: ResourceSourceContext,
  ) {
    ...
    for (const container of containers) {
      ...
      for (const resourceType of getResourcesIn(container.store)) {
        ...
        option.priority -=
          room.getCreepsWithOrder("container", container.id).length * 2;
  ...
  private getAssignedResourceType(
