...
function isTargettedOrder(
  order: TransporterOrder,
): order is StructureDestinationTask | StructureSourceTask {
  return "target" in order;
}

function isResourceDestinationOrder(
  room: Room,
  order: TransporterOrder,
): order is ResourceDestinationTask {
...
function isStructureDestinationOrder(
  order: ResourceDestinationTask,
): order is StructureDestinationTask {
  return "target" in order;
}

function isBayDestinationOrder(
  order: ResourceDestinationTask,
): order is BayDestinationTask {
  return order.type === "bay";
}

function isResourceSourceOrder(
  room: Room,
  order: TransporterOrder,
): order is ResourceSourceTask {
...
function isStructureSourceOrder(
  order: ResourceSourceTask,
): order is StructureSourceTask {
  return "target" in order;
}
...
export default class TransporterRole extends Role {
  ...
  run(creep: TransporterCreep) {
    ...
    if (
      creep.memory.singleRoom &&
      creep.memory.order &&
      isTargettedOrder(creep.memory.order)
    ) {
      const target = Game.getObjectById<RoomObject & _HasId>(
        creep.memory.order.target,
      );
      if (
        target &&
        target.pos &&
        target.pos.roomName !== creep.memory.singleRoom
      ) {
  ...
  unblockBayIfNeeded(creep: Creep): boolean {
    ...
    for (const bay of creep.room.bays) {
      ...
      if (
        creep.memory.order &&
        isResourceDestinationOrder(creep.room, creep.memory.order) &&
        isBayDestinationOrder(creep.memory.order) &&
        creep.memory.order.name === bay.name
      )
        continue;
      ...
      if (
        creep.memory.order &&
        isResourceSourceOrder(creep.room, creep.memory.order) &&
        isStructureSourceOrder(creep.memory.order)
      ) {
        const order = creep.memory.order;
        const target = Game.getObjectById(order.target);
        if (
          order.type === "overfullExtension" &&
          bay.pos.getRangeTo(target.pos) <= 1
        )
          continue;
      }
  ...
  calculateDeliveryTarget(): void {
    ...
    creep.room.visual.text(
      "target: " + creep.memory.order.type + "@" + creep.memory.order.priority,
      creep.pos,
    );
  }
   ...
   */
  ensureSourceTaskValidity(
    order: TransporterOrder,
    calculateSourceCallback: () => void,
  ): order is ResourceSourceTask {
    const creep = this.creep;
  ...
  calculateSource() {
      ...
      if (this.destinationHasHigherPriority(bestDestination, bestSource)) {
        ...
        const source = creep.room.sourceDispatcher.getTask(
          ...
          (task: ResourceSourceTask) => {
            ...
            if (
              isStructureSourceOrder(task) &&
              isStructureDestinationOrder(bestDestination)
            )
              return task.target !== bestDestination.target;
        ...
        if (source) {
          creep.room.visual.text(
            "for: " + bestDestination.type + "@" + bestDestination.priority,
            creep.pos.x,
            creep.pos.y + 0.8,
          );
          bestSource = source;
    ...
    creep.room.visual.text(
      "source: " + bestSource.type + "@" + bestSource.priority,
      creep.pos,
    );
  ...
  getSource(): ResourceSourceTask {
    ...
    const resourceType =
      creep.room.terminal || creep.room.storage ? null : RESOURCE_ENERGY;
   ...
   */
  destinationHasHigherPriority(
    destination: ResourceDestinationTask,
    source: ResourceSourceTask,
  ): boolean {
  ...
  calculateEnergySource() {
    ...
    creep.room.visual.text(
      "source: " + best.type + "@" + best.priority,
      creep.pos,
    );
