...
declare global {
  ...
  export interface DepositTargetRoom {
    spawnRooms?: Array<{
      room: string
      distance: number
    }>
...
export default DepositMiningProcess;
export class DepositMiningProcess extends Process {
  mesh: NavMesh;
  ...
  shouldRun(): boolean {
    ...
    if (!hivemind.settings.get('enableDepositMining')) {
      return false;
    }
  ...
  run() {
    ...
    _.each(memory.rooms, (info, roomName) => {
      ...
      if (Memory.rooms[roomName] && Memory.rooms[roomName].enemies && (Memory.rooms[roomName].enemies.parts[WORK] || 0) > 0) {
        return;
      }
  ...
  getPotentialSpawns(targetRoom: string): Array<{ room: string, distance: number }> {
    ...
    for (const room of Game.myRooms) {
      ...
      if (room.isFullOnMinerals()) {
        continue;
      }
      if (room.getEffectiveAvailableEnergy() < hivemind.settings.get('minEnergyForDepositMining')) {
        continue;
      }
      if (room.controller.level < hivemind.settings.get('minRclForDepositMining')) {
        continue;
      }
      if (Game.map.getRoomLinearDistance(targetRoom, room.name) > hivemind.settings.get('maxRangeForDepositMining')) {
        continue;
      }
      ...
      if (roomRoute.incomplete || roomRoute.path.length > 3 * hivemind.settings.get('maxRangeForDepositMining')) {
        continue;
      }
