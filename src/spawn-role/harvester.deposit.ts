...
export default class DepositHarvesterSpawnRole extends SpawnRole {
  ...
  addOptionForDeposit(
    ...
    strategyInfo: DepositTargetRoom,
    depositRoomName: string,
    spawnRoomInfo: { room: string; distance: number },
    options: DepositHarvesterSpawnOption[],
  ) {
    ...
    const activeDepositHarvesters = _.filter(
      ...
      (creep: DepositHarvesterCreep) => {
        ...
        const travelTime =
          this.getTravelTime(creep.memory.origin, creep.memory.targetPos) ||
          spawnRoomInfo.distance * 50;
  ...
  getCreepMemory(
    room: Room,
    option: DepositHarvesterSpawnOption,
  ): DepositHarvesterCreepMemory {
    return {
