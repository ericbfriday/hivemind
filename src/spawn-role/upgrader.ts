...
export default class UpgraderSpawnRole extends SpawnRole {
    ...
    if (maxUpgraders === 0) {
      ...
      if (this.isCloseToDowngrade(room)) {
        hivemind
          ...
          .info(
            "trying to spawn upgrader because controller is close to downgrading",
            room.controller.ticksToDowngrade,
            "/",
            CONTROLLER_DOWNGRADE[room.controller.level],
          );
        return 1;
      }
      ...
      if (
        room.controller.ticksToDowngrade <
          CONTROLLER_DOWNGRADE[room.controller.level] * 0.5 &&
  ...
  isCloseToDowngrade(room: Room): boolean {
    return (
      room.controller.ticksToDowngrade <
      CONTROLLER_DOWNGRADE[room.controller.level] * 0.1
    );
  }
  ...
  getBaseUpgraderAmount(room: Room): number {
    ...
    const isFunneling =
      ...
      !funnelManager.isFunnelingTo(room.name);
  ...
  getCreepBody(room: Room, option: UpgraderSpawnOption): BodyPartConstant[] {
    const hasEasyEnergyAccess =
      room.memory.controllerContainer || room.memory.controllerLink;
    ...
    return new BodyBuilder()
      ...
      .setMovementMode(
        hasEasyEnergyAccess ? MOVEMENT_MODE_SLOW : MOVEMENT_MODE_ROAD,
      )
