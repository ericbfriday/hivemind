...
Creep.prototype.isDangerous = function (this: Creep) {
    if (hivemind.relations.isAlly(this.owner.username)) {
        return false;
    }
    ...
    if (mightStompConstructionSites(this, isInMyRoom)) {
        return true;
    }
...
Creep.prototype.getEffectiveDamage = function (this: Creep, potentialDamage) {
    ...
    for (const part of this.body) {
        if (damageTaken >= potentialDamage) {
            break;
        }
        if (part.hits === 0) {
            continue;
