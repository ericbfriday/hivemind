...
export default class CombatManager {
  ...
  public performFleeTowards(
    ...
    const scoredPositions = this.scoreKitingPositions(
      creep,
      enemyCreeps,
      positions,
    );
  ...
  public needsToFlee(creep: Creep): boolean {
    ...
    return some(
      ...
      (c) =>
        ...
        !this.couldWinFightAgainst(creep, c),
    );
  }
  ...
  public hasEnemyCreepsInFightingRange(
    creep: Creep,
    enemyCreeps?: Creep[],
  ): boolean {
    return any(
  ...
  public couldWinFightAgainst(creep: Creep, otherCreep: Creep): boolean {
    ...
    if (
      creep.getActiveBodyparts(RANGED_ATTACK) * RANGED_ATTACK_POWER >
        otherCreep.getActiveBodyparts(HEAL) * HEAL_POWER &&
      otherCreep.getActiveBodyparts(RANGED_ATTACK) === 0
    )
      return true;
    ...
    if (
      ...
      otherCreep.getActiveBodyparts(HEAL) * HEAL_POWER +
        otherCreep.getActiveBodyparts(RANGED_ATTACK) * RANGED_ATTACK_POWER
    ) {
      return true;
    }
    ...
    if (
      creep.getActiveBodyparts(ATTACK) >
        otherCreep.getActiveBodyparts(ATTACK) &&
  ...
  private healNearbyTargets(creep: Creep) {
    ...
    const target = max(targets, (c) =>
      Math.min(
        ...
        healParts *
          (c.pos.getRangeTo(creep) <= 1 ? HEAL_POWER : RANGED_HEAL_POWER),
      ),
    );
  ...
  private attackNearbyTargets(creep: Creep) {
    ...
    if (creep.getActiveBodyparts(RANGED_ATTACK) > 0) {
      if (
        availableOwnedTargets.length >= 2 &&
        this.determineMassAttackDamage(creep, availableOwnedTargets) >
          RANGED_ATTACK_POWER
      ) {
  ...
  private determineMassAttackDamage(
    creep: Creep,
    targets: AttackTarget[],
  ): number {
    let total = 0;
  ...
  private getNearbyTargets(creep: Creep): AttackTarget[] {
    ...
    const isMyRoom =
      ...
      (Memory.strategy?.remoteHarvesting?.rooms || []).includes(
        creep.room.name,
      );
  ...
  private addEnemyRangeScore(
    ...
    for (const enemy of enemyCreeps) {
      ...
      const isEnemyMeleeCreep =
        enemy.getActiveBodyparts(ATTACK) > 0 &&
        enemy.getActiveBodyparts(RANGED_ATTACK) === 0;
      const mightMoveTowardsUs =
        enemy.getActiveBodyparts(MOVE) > 0 && enemy.fatigue === 0;
      ...
      for (const position of positions) {
        ...
        if (distance > maxRange) {
          position.score -=
            (willFight ? (isRangedCreep ? 10 : 30) : -50) *
            (distance - maxRange);
          continue;
        }

        const enemyRange =
          (isEnemyMeleeCreep ? 1 : 3) + (mightMoveTowardsUs ? 1 : 0);
        if (!willFight) {
