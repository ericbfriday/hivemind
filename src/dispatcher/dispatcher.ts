...
type ValidatorCallback<TaskType extends Task, ContextType> = (
  task: TaskType,
  context: ContextType,
) => boolean;
...
export default class Dispatcher<TaskType extends Task, ContextType> {
  ...
  getTask(
    context: ContextType,
    validator?: ValidatorCallback<TaskType, ContextType>,
  ): TaskType {
    ...
    for (const provider of this.getProvidersByPriority(context)) {
      ...
      for (const option of provider.getTasks(context)) {
        ...
        options.push(option);
        if (highestPriority < option.priority)
