...
function getExitCoordsByDirection(roomName: string): ExitCoords {
  ...
  for (let i = 1; i < 49; i++) {
    if (terrain.get(0, i) !== TERRAIN_MASK_WALL)
      exitCoords.W.push(new RoomPosition(0, i, roomName));
    if (terrain.get(49, i) !== TERRAIN_MASK_WALL)
      exitCoords.E.push(new RoomPosition(49, i, roomName));
    if (terrain.get(i, 0) !== TERRAIN_MASK_WALL)
      exitCoords.N.push(new RoomPosition(i, 0, roomName));
    if (terrain.get(i, 49) !== TERRAIN_MASK_WALL)
      exitCoords.S.push(new RoomPosition(i, 49, roomName));
  }
  ...
  for (const dir of _.keys(exitCoords)) {
    ...
    for (const pos of exitCoords[dir]) {
      ...
      if (previousPos && pos.getRangeTo(previousPos) > 1) {
        ...
        const middlePos = new RoomPosition(
          Math.ceil((previousPos.x + startPos.x) / 2),
          Math.ceil((previousPos.y + startPos.y) / 2),
          roomName,
        );
