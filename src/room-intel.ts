...
export interface GathererCreep extends Creep { }
...
export class RoomIntel {
  ...
  gatherIntel() {
    ...
    if (intel.lastScan && !hivemind.hasIntervalPassed(lastScanThreshold, intel.lastScan)) {
      return;
    }
  ...
  gatherPowerIntel(powerBanks: StructurePowerBank[]) {
    ...
    if (!powerBank || powerBank.hits === 0 || powerBank.power === 0) {
      return;
    }
  ...
  gatherDepositIntel() {
    ...
    for (const deposit of deposits) {
      if (!deposit || deposit.lastCooldown > maxCooldown) {
        return;
      }
  ...
  gatherAbandonedResourcesIntel(room: Room, structures: Record<string, Structure[]>) {
    // Find origin room.
    if (!this.roomStatus.hasRoom(this.roomName)) {
      return;
    }
    ...
    if (!roomMemory.abandonedResources) {
      roomMemory.abandonedResources = {};
    }
    delete roomMemory.abandonedResources[this.roomName];

    if (this.memory.owner) {
      return;
    }
    ...
    if (Game.shard.name === 'shardSeason') {
      ...
      let assignedSpace = _.sumBy(_.map(assignedGatherers, creep => creep.store.getFreeCapacity()));
  ...
  generateObstacleList(roomName: string, structures: Record<string, Structure[]>, constructionSites: Record<string, ConstructionSite[]>) {
    ...
    markBuildings(
      ...
      (structure) => {
        ...
        if (!_.some(result.roads, location)) {
          result.roads.push(location);
        }
      },
      (structure) => {
        const location = serializeCoords(structure.pos.x, structure.pos.y);
        if (!_.some(result.obstacles, location)) {
          result.obstacles.push(location);
        }
      },
      (x, y) => {
        const location = serializeCoords(x, y);
        if (!_.some(result.endangered, location)) {
  ...
  isClaimed(): boolean {
    ...
    if (this.memory.reservation?.username && this.memory.reservation.username !== getUsername()) {
      return true;
    }
  ...
  isOwned(): boolean {
    ...
    if (this.memory.owner !== getUsername()) {
      return true;
    }
  ...
  getControllerPosition(): RoomPosition {
    if (!this.memory.structures || !this.memory.structures[STRUCTURE_CONTROLLER]) {
      return null;
    }
  ...
  addAdjacentRoomToCheck(roomName: string, openList: Record<string, AdjacentRoomEntry>, base: { range: number, dir: string }) {
    if (!this.isPotentiallyUnsafeRoom(roomName)) {
      return;
    }
  ...
  isPotentiallyUnsafeRoom(roomName: string): boolean {
    if (this.otherUnsafeRooms.includes(roomName)) {
      return true;
    }
    if (this.otherSafeRooms.includes(roomName)) {
      return false;
    }

    if (Game.rooms[roomName] && Game.rooms[roomName].isMine()) {
      // This is one of our own rooms, and as such is possibly safe.
      if ((Game.rooms[roomName].controller.level >= Math.min(5, this.getRcl() - 1)) && !Game.rooms[roomName].isEvacuating() && !Game.rooms[roomName].isStripmine()) {
        return false;
      }
...
globalThis.getRoomIntel = getRoomIntel;

export default RoomIntel;
