...
export default class RoomDefenseProcess extends Process {
  ...
  run() {
    ...
    this.room.visual.text(
      "Wall status:" + (this.room.defense.isWallIntact() ? "intact" : "broken"),
      5,
      4,
    );
    this.room.visual.text(
      "Enemy strength: " + this.room.defense.getEnemyStrength(),
      5,
      5,
    );
  }
  ...
  performTowerHeal(
    ...
    const damagedTargets = cache.inObject(
      tower.room,
    ...
    const bestTarget = minBy(
      damagedTargets,
      (creep) =>
        creep.hits /
        creep.hitsMax /
        (1 + tower.getPowerAtRange(tower.pos.getRangeTo(creep.pos))),
    );
  ...
  performTowerRampartRepair(
    ...
    const availableRamparts = cache.inObject(
      tower.room,
      "endangeredRamparts",
      1,
      () => {
        ...
        for (const creep of hostileCreeps) {
          ...
          const rampartsInRange = (
            tower.room.structuresByType[STRUCTURE_RAMPART] || []
          ).filter(
            (rampart) => rampart.pos.getRangeTo(creep.pos) <= creepRange,
          );
    ...
    const prioritizedRampart = minBy(
      [...availableRamparts],
      (rampart) =>
        rampart.hits /
        (1 + tower.getPowerAtRange(tower.pos.getRangeTo(rampart.pos))),
    );
  ...
  manageSafeMode() {
    ...
    if (result === OK) {
      Game.notify(
        "🛡 Activated safe mode in room " +
        this.room.name +
        ". " +
        this.room.controller.safeModeAvailable +
        " remaining.",
      );
