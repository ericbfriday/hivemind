...
declare global {
    ...
    export interface SegmentManagerMemory {
        lastFullSave: number
        startSegment: number
        endSegment: number
    }
...
export class SegmentedMemory {
    ...
    manage() {
        ...
        for (let i = 0; i < 100; i++) {
            if (typeof RawMemory.segments[i] === 'string') {
                this.savedSegments.push(i);
            }
        }
    ...
    set<T>(key: string, value: T) {
        if (!this.isReady()) {
            throw new Error('Segmented Memory is not ready yet.');
        }
...
export default SegmentedMemory;
