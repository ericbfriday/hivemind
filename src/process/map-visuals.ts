 ...
 */
export default MapVisualsProcess;
export class MapVisualsProcess extends Process {
   ...
   */
  run() {
    if (hivemind.settings.get('disableMapVisuals') || !Game.map.visual) {
      return;
    }
    ...
    if (!drawn) {
      Game.map.visual.import(visuals);
    }
  }
  ...
  drawRoomStatus(roomName: string) {
    ...
    if (expansionScore < this.getExpansionScoreCutoff()) {
      return;
    }
  ...
  getExpansionScoreCutoff(): number {
    return cache.inHeap('expansionScoreCutoff', 5000, () => _.maxBy(_.map(this.roomStatus.getPotentialExpansionTargets(), (roomName: string) => {
   ...
   */
  drawNavMesh() {
    if (!hivemind.settings.get('visualizeNavMesh')) {
      return;
    }
    if (!Memory.nav) {
      return;
    ...
    _.each(Memory.nav.rooms, (navInfo, roomName) => {
      ...
      for (const region of navInfo.regions) {
        for (const exit of navInfo.exits) {
          if (!region.exits.includes(exit.id)) {
            continue;
          }
