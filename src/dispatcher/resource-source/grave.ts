...
export default class GraveSource extends TaskProvider<
  GraveSourceTask,
...
> {
  ...
  private addGraveResourceOptions(
    options: GraveSourceTask[],
    ...
    for (const target of targets) {
      ...
      for (const resourceType of getResourcesIn(target.store)) {
        ...
        if (resourceType === RESOURCE_POWER) {
          ...
          if (
            creep.room.storage &&
            creep.room.getFreeStorage() < amount &&
        ...
        if (target.store.getUsedCapacity() < creep.store.getCapacity() * 2) {
          ...
          option.priority -=
            this.room.getCreepsWithOrder("grave", target.id).length * 2;
        }
