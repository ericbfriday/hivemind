...
export default class StorageSource extends StructureSource<StorageSourceTask> {
  ...
  addStorageEnergySourceOptions(
    options: StorageSourceTask[],
    ...
    if (
      ...
      storageSource.store[RESOURCE_ENERGY] > 5000 ||
      !this.room.storage ||
      storageSource.id !== this.room.storage.id
    ) {
      options.push({
        priority: 2,
        weight: 0,
  ...
  addClearingStorageResourceOptions(
    options: StorageSourceTask[],
    context: ResourceSourceContext,
  ) {
    ...
    for (const resourceType of getResourcesIn(storage.store)) {
      ...
      if (
        this.room.getBestStorageTarget(
          context.creep.store.getFreeCapacity(),
          resourceType,
        )?.id === storage.id
      )
        continue;
    ...
    options.push({
      priority:
        storage.store[maxResourceType] > context.creep.store.getCapacity() / 2
          ? 2
          : 1,
  ...
  addClearingTerminalResourceOptions(
    options: StorageSourceTask[],
    context: ResourceSourceContext,
  ) {
    const storage = this.room.storage;
    ...
    if ((this.terminalNeedsClearing() && storageHasSpace) || noSpaceForEnergy) {
      ...
      for (const resourceType of getResourcesIn(terminal.store)) {
        ...
        if (
          ...
          resourceType === RESOURCE_ENERGY &&
          storage &&
          storage.store[RESOURCE_ENERGY] > terminal.store[RESOURCE_ENERGY] * 5
        )
          continue;
  ...
  terminalNeedsClearing() {
    ...
    return (
      terminal &&
      (terminal.store.getUsedCapacity() > terminal.store.getCapacity() * 0.8 ||
        this.room.isClearingTerminal()) &&
      !this.room.isClearingStorage()
    );
  }
