...
declare global {
    ...
    export interface ScoutSpawnOption extends SpawnOption {
        shard?: string
        portalTarget?: string
    }
}
...
export class ScoutSpawnRole extends SpawnRole {
    ...
    getSpawnOptions(room: Room): ScoutSpawnOption[] {
        return this.cacheEmptySpawnOptionsFor(room, 50, () => {
            ...
            // of the room.
            if (room.memory.recentScout && Game.time - (room.memory.recentScout || -scoutSpawnThrottle) < scoutSpawnThrottle) {
                return options;
            }

            const roomScouts = _.filter(Game.creepsByRole.scout, creep => creep.memory.origin === room.name);
            if (_.size(roomScouts) >= hivemind.settings.get('maxScoutsPerRoom') || !room.needsScout()) {
                return options;
            }
    ...
    // Check if a portal requires a scout and has this room as origin.
        ...
        _.each(memory.scouting, (isActive, shardName) => {
            _.each(memory.portals[shardName], (info, portalPos) => {
                if (info.scouted && Game.time - info.scouted < 2000) {
                    return;
                }

                // Only spawn scout if we're responsible for the portal room.
                const pos = decodePosition(portalPos);
                if (this.roomStatus.getOrigin(pos.roomName) !== room.name) {
                    return;
                }
