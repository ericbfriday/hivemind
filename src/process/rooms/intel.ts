...
export default class RoomIntelProcess extends Process {
  ...
  findHostiles() {
    ...
    this.room.memory.enemies = cache.inHeap(
      "enemies:" + this.room.name,
      cacheDuration,
      () => {
        const parts = {};
        let lastSeen = this.room.memory.enemies
          ? this.room.memory.enemies.lastSeen
          : 0;
        let expires = null;
        let safe = true;
        let healCapacity = 0;
        ...
        for (const structure of this.room.structuresByType[
          ...
          for (const effect of structure.effects || []) {
            if (
              effect.effect === EFFECT_COLLAPSE_TIMER &&
              (!expires || expires < Game.time + effect.ticksRemaining)
            ) {
