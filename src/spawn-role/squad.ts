...
export interface SquadSpawnOption extends SpawnOption {
  unitType: SquadUnitType
  squad: string
}

export class SquadSpawnRole extends SpawnRole {
  /**
  ...
  needsSpawning(room: Room, squad: Squad): SquadUnitType | null {
    ...
    for (const unitType in squad.memory.composition) {
      if (!availableUnitTypes.includes(unitType as SquadUnitType)) {
        continue;
      }
    ...
    if (_.size(neededUnits) === 0) {
      squad.memory.fullySpawned = true;
    }
  ...
  getCreepBody(room: Room, option: SquadSpawnOption): BodyPartConstant[] {
    ...
    if (bodyCallback) {
      return bodyCallback.call(this, room, option);
    }
