...
function updateNewMatrices(
  ...
  handleMapArea(pos.x, pos.y, (x, y) => {
    ...
    if (terrain.get(x, y) === TERRAIN_MASK_WALL && !Game.rooms[roomName].roomPlanner.isPlannedLocation(newPos, 'road')) {
      return;
    }
    ...
    if (_.size(structures[STRUCTURE_KEEPER_LAIR]) > 0) {
      if (!(Memory.strategy?.remoteHarvesting?.rooms || []).includes(roomName)) {
        ...
        _.each(roomIntel.getSourcePositions(), (sourceInfo) => {
          ...
          _.each(structures[STRUCTURE_KEEPER_LAIR], (structure) => {
            if (structure.pos.getRangeTo(sourceInfo.x, sourceInfo.y) > 7) {
              return;
            }
      ...
      if (!Memory.strategy?.skMining?.[roomName]) {
        ...
        for (const mineralInfo of roomIntel.getMineralPositions()) {
          ...
          // Add area around keeper lairs as obstacles.
          _.each(structures[STRUCTURE_KEEPER_LAIR], (structure) => {
            if (structure.pos.getRangeTo(mineralInfo.x, mineralInfo.y) > 7) {
              return;
            }
...
function markSourceKeeperExits(roomName: string, dir: TOP | LEFT | BOTTOM | RIGHT, sourceKeeperCallback: (x: number, y: number) => void) {
  ...
  if (!otherRoomIntel || !otherRoomIntel.hasCostMatrixData()) {
    return;
  }
  if (_.size(otherRoomIntel.getStructures(STRUCTURE_KEEPER_LAIR)) === 0) {
    return;
  }
...
globalThis.getCostMatrix = getCostMatrix;
globalThis.getDangerMatrix = getDangerMatrix;
