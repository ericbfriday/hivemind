...
import type { RemoteHarvesterCreep, RemoteHarvesterCreepMemory } from '@/role/harvester.remote';
...
export class RemoteMiningSpawnRole extends SpawnRole {
  ...
  registerPotentiallyActiveSource(room: Room, position: RoomPosition) {
    ...
    const roomList: Record<string, boolean> = cache.inHeap(`activeRemoteRooms:${room.name}`, 1, () => {
      return {};
    });
