  ...
  handleRequests() {
    for (const request of simpleAllies.allySegmentData?.requests?.funnel ??
      ...
      Memory.requests.trade[RESOURCE_ENERGY][request.roomName] = {
        ...
        lastSeen: Game.time,
        priority:
          request.goalType === FunnelGoal.RCL7
            ? 3
            : request.goalType === FunnelGoal.RCL8
              ? 2
  ...
  makeDefenseRequests() {
    for (const roomName in Memory.requests.defense || {}) {
      ...
      if (Game.time - request.lastSeen < 10) {
        ...
        if (
          Game.rooms[roomName].getEffectiveAvailableEnergy() < 20_000 &&
          Game.rooms[roomName].terminal?.isOperational()
        ) {
          simpleAllies.requestResource({
            roomName,
            ...
            amount: Math.max(
              5000,
              20_000 - Game.rooms[roomName].getEffectiveAvailableEnergy(),
            ),
            terminal: true,
            timeout: Game.time + 20,
          });
