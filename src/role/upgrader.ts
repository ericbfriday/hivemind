...
export default class UpgraderRole extends Role {
    ...
    if (
      ...
      (creep.store[RESOURCE_ENERGY] === creep.store.getCapacity() ||
        (creep.store[RESOURCE_ENERGY] > 0 &&
          creep.room.memory.controllerContainer))
    ) {
  ...
  performUpgrade(creep: UpgraderCreep, allowRefilling: boolean) {
    ...
    if (allowRefilling && creep.store.getUsedCapacity() < workParts * 2) {
      ...
      if (creep.room.memory.controllerLink) {
        const controllerLink = Game.getObjectById<StructureLink>(
          creep.room.memory.controllerLink,
        );
        if (
          controllerLink &&
          controllerLink.store[RESOURCE_ENERGY] > 50 &&
          creep.pos.getRangeTo(controllerLink) <= 1 &&
          creep.withdraw(controllerLink, RESOURCE_ENERGY) === OK
        ) {
          withdrawn = true;
        }
      }
      ...
      if (!withdrawn && creep.room.memory.controllerContainer) {
        const controllerContainer = Game.getObjectById<StructureContainer>(
          creep.room.memory.controllerContainer,
        );
        if (
          controllerContainer &&
          controllerContainer.store.energy > 50 &&
          creep.pos.getRangeTo(controllerContainer) <= 1 &&
          creep.withdraw(controllerContainer, RESOURCE_ENERGY) === OK
        ) {
          withdrawn = true;
        }
      }
  ...
  performGetUpgraderEnergy(creep: UpgraderCreep) {
    ...
    if (creep.room.memory.controllerLink) {
      const target = Game.getObjectById<StructureLink>(
        creep.room.memory.controllerLink,
      );
    ...
    if (creep.room.memory.controllerContainer) {
      const target = Game.getObjectById<StructureContainer>(
        creep.room.memory.controllerContainer,
      );
    ...
    const droppedResources = creep.room.controller.pos.findInRange(
      FIND_DROPPED_RESOURCES,
      creep.room.controller.level < 4 ? 10 : 3,
      {
        filter: (resource) =>
          resource.resourceType === RESOURCE_ENERGY &&
          resource.amount >= creep.store.getCapacity(),
      },
    );
    ...
    const otherContainers = _.filter(
      ...
      (structure) =>
        ...
        creep.room.controller.pos.getRangeTo(structure.pos) <=
          (creep.room.controller.level < 4 ? 10 : 3),
    );
