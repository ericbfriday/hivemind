...
export type PositionCache = Record<string, Record<number, RoomPosition>>;
...
export class RoomPlan {
    ...
    unserialize(input: SerializedPlan) {
        this.positionsByType = _.mapValues(input, function (posList: string): Record<number, RoomPosition> {
            ...
            return cache;
        }); // had to remove `this` as third param for the function, might need to call a `.bind(this)` or similar.
    }
