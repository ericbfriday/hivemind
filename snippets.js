...
const mem = []; _.each(Memory.operations, (o, name) => {
  if (o.type !== 'mining')
    return; mem.push({ name, income: o.stats.energy / o.stats.cpu })
...
p.generateMineralStatus(); Memory.hivemind.canExpand = true; const r = []; _.each(Game.rooms, (room) => {
  if (!room.isMine())
    return; const i = p.calculateExpansionScore(room.name); i.roomName = room.name; r.push(i)
}); Memory.hivemind.canExpand = false; console.log(JSON.stringify(r))
