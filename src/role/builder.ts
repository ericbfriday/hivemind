...
export default class BuilderRole extends Role {
  ...
  run(creep: BuilderCreep) {
    ...
    if (
      (creep.memory.repairing || creep.memory.upgrading) &&
    ...
    if (creep.memory.repairing) {
      if (
        ...
        const isFunnelingElsewhere =
          creep.room.terminal &&
          funnelManager.isFunneling() &&
          !funnelManager.isFunnelingTo(creep.room.name) &&
          creep.room.getEffectiveAvailableEnergy() < 100_000;
        const isStripmine =
          creep.room.controller.level >= 6 && creep.room.isStripmine();
        if (
    ...
    const deliveringCreeps = creep.room.getCreepsWithOrder(
      "workerCreep",
      creep.id,
    );
    ...
    if (
      !creep.room.storage ||
      creep.room.getEffectiveAvailableEnergy() > 2500
    ) {
      ...
      this.transporterRole.performGetEnergy(
        creep as unknown as TransporterCreep,
      );
    }
  }
  ...
  performUpgrade(creep: BuilderCreep) {
    ...
    creep.room.memory.noBuilderNeeded = Game.time;
    const roomHasTooLittleEnergy =
      creep.room.storage && creep.room.getEffectiveAvailableEnergy() < 25_000;
    const shouldNotUpgrade =
      creep.room.controller.level === 8 && !balancer.maySpendEnergyOnGpl();
    const funnelManager = container.get("FunnelManager");
    const isFunnelingElsewhere =
      creep.room.terminal &&
      funnelManager.isFunneling() &&
  ...
  performRepair(creep: BuilderCreep): boolean {
    if (
      !creep.memory.order ||
      !creep.memory.order.target ||
      !Game.getObjectById(creep.memory.order.target)
    ) {
    ...
    if (
      ...
      ).includes(Game.getObjectById(creep.memory.order.target).structureType)
    ) {
      this.calculateBuilderTarget(creep);
    ...
    hivemind
      .log("creeps", creep.pos.roomName)
      .notify("Unknown order type detected on", creep.name);
    this.calculateBuilderTarget(creep);
    return true;
  }
  ...
  calculateBuilderTarget(creep: BuilderCreep) {
    ...
    const best: any = utilities.getBestOption(
      this.getAvailableBuilderTargets(creep),
    );
  ...
  addRepairOptions(creep: BuilderCreep, options: OrderOption[]) {
    ...
    for (const target of targets) {
      ...
      if (
        ...
        this.modifyRepairDefensesOption(
          creep,
          option,
          target as StructureWall | StructureRampart,
        );
      } else {
      ...
      if (
        ...
        target.hits / (option.maxHealth || target.hitsMax) > 0.9
      ) {
        continue;
      }
      ...
      if (creep.room.defense.getEnemyStrength() === ENEMY_STRENGTH_NONE) {
        option.priority -= creep.room.getCreepsWithOrder(
          "repair",
          target.id,
        ).length;
      }
  ...
  getStructureMaxHits(
    ...
    if (
      ...
      if (
        structure.structureType === STRUCTURE_RAMPART &&
        structure.room.roomPlanner &&
        structure.room.roomPlanner.isPlannedLocation(
          structure.pos,
          "rampart.ramp",
        )
      ) {
  ...
  modifyRepairDefensesOption(
    ...
    if (target.structureType === STRUCTURE_RAMPART) {
      ...
      if (creep.room.defense.getEnemyStrength() >= ENEMY_STRENGTH_NORMAL) {
        ...
        creep.room.getEffectiveAvailableEnergy() < 5000 &&
        target.hits >= 10_000
      ) {
        // Don't strengthen ramparts too much if room is struggling for energy.
        option.priority = -1;
      } else if (
        target.hits < hivemind.settings.get("minWallIntegrity") &&
  ...
  addBuildOptions(creep: BuilderCreep, options) {
    ...
    for (const target of targets) {
      ...
      if (
        (
          [STRUCTURE_LAB, STRUCTURE_NUKER, STRUCTURE_FACTORY] as string[]
        ).includes(target.structureType)
      ) {
  ...
  repairNearby(creep: BuilderCreep) {
    if (
      creep.store[RESOURCE_ENERGY] < creep.store.getCapacity() * 0.7 &&
      creep.store[RESOURCE_ENERGY] > creep.store.getCapacity() * 0.3
    )
      return;
