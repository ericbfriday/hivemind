...
export class ReportProcess extends Process {
    ...
    // Check if it's time for sending a report.
        if (Date.now() < this.memory.nextReportTime) {
            return;
        }
    ...
    generateMiningOperationsReport() {
        if (_.size(Game.operationsByType.mining) === 0) {
            return;
        }
    ...
    generateRoomOperationsReport() {
        if (_.size(Game.operationsByType.room) === 0) {
            return;
        }
    ...
    formatSignificantEntries<T>(list: T[], formatter: (entry: T, index: number) => string): string[] {
        ...
        for (let index = 0; index < list.length; index++) {
            ...
            if (index > 2 && index < list.length - 3 && index !== Math.floor(list.length / 2)) {
                continue;
            }
