...
export interface DropSourceTask extends ResourceSourceTask {
  type: 'drop'
  target: Id<Resource>
}

export default DropSource;
export class DropSource extends TaskProvider<DropSourceTask, ResourceSourceContext> {
  ...
  private addDroppedResourceOptions(options: DropSourceTask[], context: ResourceSourceContext) {
    ...
    for (const resource of resources) {
      if (context.resourceType && context.resourceType !== resource.resourceType) {
        return;
      }
      ...
      if (resource.resourceType === RESOURCE_POWER) {
        ...
        if (storagePosition && resource.pos.x === storagePosition.x && resource.pos.y === storagePosition.y) {
          ...
          if (creep.room.energyAvailable < creep.room.energyCapacityAvailable && creep.room.getCurrentResourceAmount(RESOURCE_ENERGY) > 5000 && creep.memory.role === 'transporter') {
            option.priority -= 2;
          }
  ...
  isValid(task: DropSourceTask, context: ResourceSourceContext): boolean {
    ...
    if (!this.isSafePosition(context.creep, resource.pos)) {
      return false;
    }
