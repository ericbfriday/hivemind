...
declare global {
  ...
  export interface RelayHaulerCreepMemory extends CreepMemory {
    role: 'hauler.relay'
    delivering?: boolean
    source?: string
  }

  export interface RelayHaulerCreepHeapMemory extends CreepHeapMemory {
...
export class RelayHaulerRole extends Role {
  actionTaken: boolean;
  ...
  performPickup(creep: RelayHaulerCreep) {
    ...
    if (container) {
      ...
      const hasActiveHarvester = _.some(creep.room.creepsByRole['harvester.remote'], (harvester: RemoteHarvesterCreep) => {
        if (harvester.memory.source !== creep.memory.source) {
          return false;
        }
        if (harvester.pos.roomName !== container.pos.roomName) {
          return false;
        }
        if (harvester.pos.getRangeTo(container.pos) > 3) {
          return false;
        }
      ...
      creep.whenInRange(1, target, () => {
        if (target instanceof Resource) {
          ...
          for (const resourceType of getResourcesIn(target.store)) {
            if (resourceType !== RESOURCE_ENERGY && !this.hasSourceRoomStorage(creep)) {
              continue;
            }
  ...
  getNearbyResourceTarget(creep: RelayHaulerCreep) {
    if (creep.heapMemory.pickupTarget) {
      ...
      if (container.length > 0) {
        creep.heapMemory.pickupTarget = container[0].id;
      }
      return container[0];
    }
  ...
  isValidPickupTarget(creep: RelayHaulerCreep, target: Resource | Tombstone | Ruin | StructureContainer | ScoreContainer) {
    ...
    if (_.find(sourceKeepers, sourceKeeper => sourceKeeper.pos.getRangeTo(target) <= 5)) {
      return false;
    }
...
export default RelayHaulerRole;
