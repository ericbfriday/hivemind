...
export class HarvesterSpawnRole extends SpawnRole {
  /**
  ...
  addInitialHarvester(source: Source, options: HarvesterSpawnOption[]) {
    ...
    const minSpawnDistance = _.minBy(_.map(spawns, spawn => spawn.pos.getRangeTo(source.pos)));
  ...
  addAdditionalHarvesters(source: Source, options: HarvesterSpawnOption[]) {
    ...
    if (source.room.controller.level > 3) {
      return;
    }

    // Don't spawn more harvesters than we have space for.
    if (source.harvesters.length >= source.getNumHarvestSpots()) {
      return;
    }
    if (!this.isSourceSafe(source)) {
      return;
  ...
  isSmallHarvesterNeeded(room: Room): boolean {
    ...
    if (_.size(room.creepsByRole.harvester) > 0) {
      return false;
    }
...
export default HarvesterSpawnRole;
