...
export default class ReclaimSpawnRole extends SpawnRole {
  ...
  addIntershardSpawnOptionsFor(
    request: { name: string; safe: boolean; rcl: number; portalRoom?: string },
    shardName: string,
    options: ReclaimSpawnOption[],
  ) {
    ...
    const remoteBuilderCount = _.filter(
      ...
      (creep: RemoteBuilderCreep) =>
        ...
        creep.memory.interShardPortal,
    ).length;
    if (remoteBuilderCount > 5) return;

    const portalLocation = this.findClosestPortalToRemoteRoom(
      request.portalRoom,
      shardName,
    );
  ...
  findClosestPortalToRemoteRoom(roomName: string, shardName: string) {
    return cache.inHeap(
      "portalRoomPos:" + this.room.name + ":" + roomName + ":" + shardName,
      2 * CREEP_LIFE_TIME,
      () => {
        let bestPortal;

        console.log(
          "Checking if we can spawn a reclaimer to " +
            shardName +
            "/" +
            roomName,
        );
  ...
  getCreepMemory(
    room: Room,
    option: ReclaimSpawnOption,
  ): RemoteBuilderCreepMemory {
