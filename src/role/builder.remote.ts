...
declare global {
    ...
    export interface RemoteBuilderCreepMemory extends CreepMemory {
        role: 'builder.remote'
        targetRoom?: string
        interShardPortal?: string
    }

    export interface RemoteBuilderCreepHeapMemory extends CreepHeapMemory {
        repairMinHits?: number
    }
}

export default RemoteBuilderRole;
export class RemoteBuilderRole extends Role {
    ...
    run(creep: RemoteBuilderCreep) {
        ...
        if (creep.memory.interShardPortal) {
            ...
            if (creep.interRoomTravel(targetPos, true)) {
                return;
            }
        ...
        if (creep.memory.targetRoom) {
            if (creep.interRoomTravel(new RoomPosition(25, 25, creep.memory.targetRoom))) {
                return;
            }
    ...
    performRemoteBuild() {
        ...
        if (this.supplyTowers()) {
            return;
        }

        let target = Game.getObjectById<ConstructionSite>(creep.memory.buildTarget);
        if ((!target || (target.structureType !== STRUCTURE_SPAWN && target.structureType !== STRUCTURE_TOWER)) && this.saveExpiringRamparts(10_000)) {
            return;
        }
        ...
        if (!creep.memory.buildTarget) {
            ...
            if (!creep.memory.buildTarget && !creep.memory.repairTarget) {
                if (this.creep.room.needsReclaiming() && this.saveExpiringRamparts(hivemind.settings.get('minWallIntegrity'))) {
                    return;
                }
