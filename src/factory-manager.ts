...
export class FactoryManager {
  ...
  hasAllComponents(product: FactoryProductConstant): boolean {
    ...
    for (resourceType in recipe.components) {
      if (this.room.factory.store.getUsedCapacity(resourceType) < recipe.components[resourceType]) {
        return false;
      }
    }
  ...
  getMissingComponents(): Partial<Record<FactoryComponentConstant, number>> | null {
    ...
    for (resourceType in requestedResources) {
      if ((this.room.factory.store[resourceType] || 0) > requestedResources[resourceType]) {
        continue;
      }
  ...
  isRecipeAvailable(resourceType: FactoryProductConstant, recipe: Recipe): boolean {
    if (recipe.level && recipe.level !== this.getFactoryLevel()) {
      return false;
    }
  ...
  mayCreateCommodities(product: FactoryProductConstant, recipe: Recipe): boolean {
    ...
    if (this.isMadeOnlyFromBasicResources(recipe)) {
      for (const resourceType in recipe.components) {
        ...
        if (resourceAmount < recipe.components[resourceType] * createdAmount * 5) {
          return false;
        }
      }
  ...
  hasRequiredResources(recipe: Recipe): boolean {
    for (const resourceType in recipe.components) {
      if (this.room.getCurrentResourceAmount(resourceType) + this.room.factory.store.getUsedCapacity(resourceType as ResourceConstant) < recipe.components[resourceType]) {
        return false;
      }
    }
