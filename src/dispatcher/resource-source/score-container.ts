...
export interface ScoreContainerSourceTask extends ResourceSourceTask {
    type: 'score-container'
    target: Id<ScoreContainer>
}

export default ScoreContainerSource;
export class ScoreContainerSource extends TaskProvider<ScoreContainerSourceTask, ResourceSourceContext> {
    ...
    getTasks(context: ResourceSourceContext) {
        if (context.resourceType && context.resourceType !== RESOURCE_SCORE) {
            return [];
        }
    ...
    isValid(task: ScoreContainerSourceTask, context: ResourceSourceContext): boolean {
        ...
        if (scoreContainer.store.getUsedCapacity(task.resourceType) === 0) {
            return false;
        }
        if (context.creep.store.getFreeCapacity(task.resourceType) === 0) {
            return false;
        }
        if (!this.isSafePosition(context.creep, scoreContainer.pos)) {
            return false;
        }
