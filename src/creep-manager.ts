...
export default class CreepManager {
  ...
  runCreepLogic(creep: Creep) {
    ...
    if (totalTime >= 5) {
      hivemind
        .log("creeps", creep.room.name)
        .error(creep.name, "took", totalTime.toPrecision(3), "CPU this tick!");
    }
  ...
  report() {
    if (this.performance.total.throttled) {
      const total =
        this.performance.total.throttled + this.performance.total.run;
      hivemind
        .log("creeps")
        .debug(
          this.performance.total.throttled,
          "of",
          total,
          "creeps have been throttled due to bucket this tick.",
        );
    }
