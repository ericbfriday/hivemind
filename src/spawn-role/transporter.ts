...
export interface TransporterSpawnOption extends SpawnOption {
    force: boolean
    size: number
}

export class TransporterSpawnRole extends SpawnRole {
    /**
    ...
    // On low level rooms, do not use (too many) transporters.
        ...
        if (room.memory.controllerLink) {
            maxTransporters -= 1 + _.sumBy(room.sources, (source: Source) => source.getNearbyLink() ? 1 : 0);
        }
    ...
    // @todo Take into account boosts.
        const upgraderWorkParts = _.sumBy(Object.values(room.creepsByRole.upgrader), creep => creep.getActiveBodyparts(WORK));
