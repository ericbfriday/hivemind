...
const expansionScoreCache: Record<
  string,
  [number, number] | [number, number, Record<string, number>]
> = {};
...
export default class ScoutProcess extends Process {
  ...
  run() {
    ...
    const maxCpuUsage = hivemind.settings.get(
      "maxRoomPrioritizationCpuPerTick",
    );
    ...
    let allDone = true;
    let checkedCount = 0;
    if (!Memory.strategy.roomListProgress)
      Memory.strategy.roomListProgress = [];
  ...
  calculateRoomPriorities(roomName: string) {
    ...
    if (range === 0 && roomIntel.isClaimable()) {
      ...
      this.roomStatus.setExpansionScore(
        roomName,
        expansionInfo.score,
        expansionInfo.reasons,
      );
    }
    ...
    if (range > 0 && range <= (Memory.hivemind.maxScoutDistance || 7)) {
      ...
      if ((roomIntel.memory.lastScan || 0) > 0) {
        this.roomStatus.setHarvestPriority(
          roomName,
          this.calculateHarvestScore(roomName),
        );
    ...
    if (
      range <= 6 &&
      (/^[EW]\d*0[NS]\d+$/.test(roomName) ||
        /^[EW]\d+[NS]\d*0$/.test(roomName)) &&
      timeSinceLastScan > hivemind.settings.get("highwayScoutInterval") &&
      observer
    ) {
    ...
    if (
      this.roomStatus.getScoutPriority(roomName) > 0 &&
      observer &&
      range <= (Memory.hivemind.maxScoutDistance || 7) &&
      ...
      if (!observer.hasScouted) {
        ...
        hivemind
          .log("intel", observer.pos.roomName)
          .info("Observing", roomName);
      } else {
  ...
  calculateHarvestScore(roomName: string) {
    ...
    if (
      range === 0 ||
      range > hivemind.settings.get("maxRemoteMineRoomDistance")
    )
      return 0;
  ...
  calculateExpansionScore(roomName: string) {
    ...
    result.addScore(
      roomIntel.getSourcePositions().length * 2 - 2,
      "numSources",
    );
    ...
    for (const mineralType of roomIntel.getMineralTypes()) {
      ...
      result.addScore(
        1 / ((this.mineralCount[mineralType] || 0) + mineralGain),
        "numMinerals",
      );
    }
    ...
    for (const adjacentRoom in adjacentRoomInfluence) {
      ...
      result.addScore(
        multiplier * this.getHarvestRoomScore(adjacentRoom),
        "harvest" + adjacentRoom,
      );
    }

    if (hasHighwayExit) {
      result.addScore(
        hivemind.settings.get("expansionScoreBonusHighwayExit"),
        "highwayExit",
      );
    }
    ...
    for (const otherRoom of values(Game.rooms)) {
      ...
      const newSafeExits = Math.abs(
        sum(Object.keys(adjustedSafety.directions)) - sum(Object.keys(normalSafety.directions)),
      );
      ...
      if (values(exits).includes(otherRoom.name)) {
        ...
        result.addScore(
          -this.getHarvestRoomScore(roomName),
          "blockHarvest" + otherRoom.name,
        );
      }
    }
    ...
    result.addScore(sum(Object.keys(safety.directions)) * 0.25, "safeExits");
    ...
    const unsafeRatio = (4 - sum(Object.keys(safety.directions))) / size(exits);
    result.addScore(
    ...
    result.addScore(
      0.25 - roomIntel.countTiles("swamp") * 0.0001,
      "swampTiles",
    );
    ...
    if (distancesToRoom.length > 0) {
      const distanceToNextRoom = minBy(distancesToRoom);
  ...
  getExpansionScoreFromCache(roomName: string, result: ExpansionScore) {
    ...
    if (
      hivemind.hasIntervalPassed(
        ...
        expansionScoreCache[roomName][1],
      )
    )
      return false;
  ...
  getHarvestRoomScore(roomName: string, forOwnedRoom = false) {
    ...
    if (!forOwnedRoom) {
      ...
      if (roomIntel.isOwned() && roomIntel.memory.owner !== "Invader")
        return -0.5;

      // If another player has reserved the adjacent room, we can't profit all that well.
      if (
        roomIntel.isClaimed() &&
        roomIntel.memory.reservation.username !== "Invader"
      )
        sourceFactor = 0.1;
    }
  ...
  generateScoutTargets() {
    ...
    for (const roomName of this.roomStatus.getAllKnownRooms()) {
      if (scoutTargets[roomName]) {
        ...
        this.roomStatus.setDistanceToOrigin(
          roomName,
          scoutTargets[roomName].range,
        );
      } else {
    ...
    for (const roomName in scoutTargets) {
      if (!this.roomStatus.hasRoom(roomName)) {
        ...
        this.roomStatus.addRoom(
          roomName,
          scoutTargets[roomName].origin,
          scoutTargets[roomName].range,
        );
      }
    }
  ...
  addAdjacentRooms(
    roomName: string,
    openList: Record<string, ScoutTarget>,
  ...
  getClosestObserver(roomName: string): StructureObserver {
    ...
    for (const observer of this.observers) {
      const roomDist = Game.map.getRoomLinearDistance(
        observer.room.name,
        roomName,
      );
      ...
      if (
        !bestObserver ||
        roomDist <
        Game.map.getRoomLinearDistance(bestObserver.room.name, roomName)
      ) {
  ...
  generateMineralStatus() {
    ...
    for (const room of Game.myRooms) {
      ...
      for (const mineralType of roomIntel.getMineralTypes()) {
        this.mineralCount[mineralType] =
          (this.mineralCount[mineralType] || 0) + 1;
      }
    }
  }
}
