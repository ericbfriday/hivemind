...
export interface RoomDefenseMemory {
  lastActivity?: number
...
export class RoomDefense {
  ...
  drawDebug() {
    ...
    if (!visual || hivemind.settings.get('disableRoomVisuals')) {
      return;
    }
  ...
  getWallStrengthMultiplier(): number {
    if (this.hasSafeModeCooldown()) {
      return 2;
    }
    if (this.isAnyRoomSafeModed()) {
      return 1.5;
    }
  ...
  getEnemyStrength(): EnemyStrength {
    return cache.inHeap(`getEnemyStrength:${this.roomName}`, 5, () => {
      ...
      if (attackStrength === 0) {
        return ENEMY_STRENGTH_NONE;
      }
      if (invaderOnly || (healStrength < towerStrength && attackStrength < highDamageThreshold)) {
        return ENEMY_STRENGTH_WEAK;
      }
      if (healStrength < towerStrength + defensiveCreepStrength && attackStrength > normalDamageThreshold) {
        return ENEMY_STRENGTH_NORMAL;
      }
      if (healStrength < towerStrength + defensiveCreepStrength * defenseBoostPower && attackStrength > highDamageThreshold) {
  ...
  openRampartsToFriendlies() {
    ...
    if (!this.memory.creepStatus) {
      this.memory.creepStatus = {};
    }
    ...
    _.each(ramparts, (rampart) => {
      ...
      if (rampart.isPublic !== newState) {
        rampart.setPublic(newState);
      }
    });
  }
  ...
  isThief(creep: Creep): boolean {
    ...
    if (!this.memory.creepStatus[creep.id].isThief) {
      return false;
    }
