 ...
 */
export default InterShardProcess;
export class InterShardProcess extends Process {
  ...
  distributeCPU() {
    ...
    const totalCpu = _.sumBy(Game.cpu.shardLimits);
    const newLimits = {};
  ...
  addShardData(shardName: string, shardMemory?: ShardMemory) {
    ...
    if (this._shardData[shardName]) {
      return;
    }

    if (!shardMemory) {
      shardMemory = interShard.getRemoteMemory(shardName);
    }
  ...
  isAdjacentShardFuntional(shardName: string) {
    ...
    _.each(this._shardData, (data, compareShard) => {
      if (compareShard === 'total') {
        return null;
      }

      const compareMemory = compareShard === Game.shard.name ? interShard.getLocalMemory() : interShard.getRemoteMemory(compareShard);
      if (!compareMemory.info) {
        return null;
      }
      if (!compareMemory.portals || !compareMemory.portals[shardName]) {
        return null;
      }
      if ((compareMemory.info.maxRoomLevel || 0) < minRoomLevelToIntershardScout) {
        return null;
      }
  ...
  manageScouting() {
    ...
    if (this.memory.info.maxRoomLevel < minRoomLevelToIntershardScout) {
      return;
    }
  ...
  manageExpanding() {
    ...
    if (this.memory.info.interShardExpansion) {
      if (this.memory.info.interShardExpansion.start && Game.time - this.memory.info.interShardExpansion.start < 50 * CREEP_LIFE_TIME) {
        return;
      }
  ...
  startIntershardExpansion() {
    ...
    if (!this.memory.info.rooms.bestExpansion) {
      return;
    }
  ...
  manageExpansionSupport() {
    ...
    _.each(this.memory.portals, (portals, shardName) => {
      ...
      if (!remoteMemory.info || !remoteMemory.info.interShardExpansion) {
        return;
      }
      ...
      if (remoteMemory.info.maxRoomLevel === 0) {
        squad.addUnit('singleClaim');
      }
  ...
  findClosestPortalToSpawnTo(roomName: string, targetShard: string) {
    ...
    _.each(this.memory.portals, (portals, shardName) => {
      ...
      _.each(portals, (portalInfo, portalPosition) => {
        ...
        if (bestPortal && bestPortal.range <= bestSourceRoom.range) {
          return;
        }
  ...
  findClosestSpawn(targetRoom: string): { name: string, range: number } | null {
    ...
    for (const room of Game.myRooms) {
      ...
      if (room.getEffectiveAvailableEnergy() < 30_000) {
        continue;
      }
      if (Game.map.getRoomLinearDistance(room.name, targetRoom) > 7) {
        continue;
      }
    ...
    return cache.inHeap(`portalRoomName:${roomName}`, 2 * CREEP_LIFE_TIME, () => {
      ...
      _.each(this.memory.portals, (portals, shardName) => {
        ...
        _.each(portals, (portalInfo, portalLocation) => {
          ...
          if (Game.map.getRoomLinearDistance(portalPosition.roomName, roomName) > 10) {
            return;
          }
          ...
          if (bestPortal && bestPortal.range <= path.path.length) {
            return;
          }
