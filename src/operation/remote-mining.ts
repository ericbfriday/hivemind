...
export type RemoteMiningPath = Record<string, {
    accessible: boolean
    path?: RoomPosition[]
    sourceRoom?: string
    travelTime?: number
...
declare global {
    export interface RemoteMiningOperationMemory extends OperationMemory {
        type: 'mining'
 ...
 */
export class RemoteMiningOperation extends Operation {
    ...
    getMiningLocationsByRoom() {
        ...
        return cache.inHeap(`sourceRooms:${this.name}`, 1000, () => {
            ...
            Object.entries(paths).forEach((
                ...
                const { sourceRoom } = info;
                if (!info.accessible) {
                    return;
                }
                if (!sourceRoom || !result[sourceRoom]) {
                    result[sourceRoom] = [];
                }

                result[sourceRoom].push(sourceLocation);
            });
    ...
    getSourceRoom(sourceLocation: string): string {
        ...
        for (const roomName in locations) {
            if (locations[roomName].includes(sourceLocation)) {
                return roomName;
            }
        }
    ...
    getPaths() {
        return cache.inObject(this, 'getPaths', 0, () => {
            ...
            for (const sourcePos of positions) {
                ...
                if (!this.memory.status[sourceLocation]) {
                    this.memory.status[sourceLocation] = {};
                }
    ...
    isUnderAttack(): boolean {
        ...
        if (roomMemory?.enemies && !roomMemory.enemies.safe && roomMemory.enemies.expires > Game.time) {
            return true;
        }
    ...
    getTotalEnemyData(): EnemyData {
        ...
        for (const roomName of this.getRoomsOnPath()) {
            ...
            if (!roomMemory?.enemies) {
                continue;
            }
            if (roomMemory.enemies.hasInvaderCore) {
                totalEnemyData.hasInvaderCore = true;
            }
            if (roomMemory.enemies.safe) {
                continue;
            }
    ...
    estimateRequiredWorkPartsForMaintenance(sourceLocation: string) {
        return cache.inHeap(`neededWorkParts:${sourceLocation}`, 500, () => {
            ...
            for (const position of path.path || []) {
                if (Game.rooms[position.roomName]?.isMine()) {
                    continue;
                }
    ...
    needsRepairs(sourceLocation: string) {
        return cache.inHeap(`needsRepairs:${sourceLocation}`, 50, () => {
            ...
            if (this.getNeededWorkForPosition(containerPositon, STRUCTURE_CONTAINER) > CONTAINER_HITS / 2) {
                return true;
            }
            ...
            for (const position of path.path || []) {
                ...
                if (this.getNeededWorkForPosition(position, STRUCTURE_ROAD) > ROAD_HITS / 3) {
                    return true;
                }
            }
    ...
    getNeededWorkForPosition(position: RoomPosition, structureType: BuildableStructureConstant) {
        ...
        if (site) {
            return site.hitsMax * REPAIR_POWER;
        }
    ...
    getRoomsOnPath(sourceLocation?: string): string[] {
        return cache.inHeap(`rmPath:${this.name}:${sourceLocation ?? 'all'}`, 1000, () => {
            ...
            const result: string[] = [];
            const checkedRooms = {};
            for (const location of Object.keys(paths)) {
                if (sourceLocation && location !== sourceLocation) {
                    continue;
                }
    ...
    canReserveFrom(roomName: string): boolean {
        ...
        const roomIntel = getRoomIntel(this.roomName);
        if (roomIntel.getControllerReservePositionCount() > 1) {
            return Game.rooms[roomName].energyCapacityAvailable >= BODYPART_COST[CLAIM] + BODYPART_COST[MOVE];
        }
    ...
    hasReservation(): boolean {
        ...
        if (room) {
            return room.controller?.reservation && room.controller.reservation.username === getUsername() && room.controller.reservation.ticksToEnd >= CONTROLLER_RESERVE_MAX * 0.1;
        }
    ...
    getContainer(sourceLocation: string): StructureContainer | null {
        if (!this.hasContainer(sourceLocation)) {
            return null;
        }
        if (!this.memory.status[sourceLocation]) {
            return null;
    ...
    hasContainer(sourceLocation: string): boolean {
        return cache.inObject(this, `hasContainer:${sourceLocation}`, 0, () => {
            ...
            const paths = this.getPaths();
            if (!paths[sourceLocation] || !paths[sourceLocation].accessible) {
                return false;
            }
    ...
    getEnergyForPickup(sourceLocation: string): number {
        ...
        if (!Game.rooms[position.roomName]) {
            return energyCache[sourceLocation] || total;
        }
    ...
    shouldSpawnHaulers(sourceLocation: string): boolean {
        ...
        if (this.needsDismantler(sourceLocation)) {
            return false;
        }
    ...
    getHaulerSize(sourceLocation: string): number {
        ...
        if (!paths[sourceLocation]) {
            return 0;
        }
        if (!paths[sourceLocation].accessible) {
            return 0;
        }
    ...
    hasActiveHarvesters(sourceLocation?: string): boolean {
        if (sourceLocation) {
            return _.some(Game.creepsByRole['harvester.remote'], (creep: RemoteHarvesterCreep) => creep.memory.source === sourceLocation);
        }

        for (const pos of this.getSourcePositions()) {
            if (this.hasActiveHarvesters(encodePosition(pos))) {
                return true;
            }
        }
    ...
    needsDismantler(sourceLocation?: string): boolean {
        ...
        if (sourceLocation) {
            return this.getDismantlePositions(sourceLocation).length > 0;
        }

        for (const pos of this.getSourcePositions()) {
            if (this.needsDismantler(encodePosition(pos))) {
                return true;
            }
        }
    ...
    getDismantlePositions(sourceLocation: string): RoomPosition[] {
        ...
        const cached = cache.inHeap(`blockedTiles:${sourceLocation}`, 100, () => {
            ...
            for (let i = path.length - 1; i > 0; i--) {
                ...
                if (cannotDismantlePositions[encodePosition(pos)]) {
                    continue;
                }
    ...
    needsBuilder(sourceLocation: string): boolean {
        ...
        const container = this.getContainer(sourceLocation);
        if (container && container.hits < container.hitsMax / 3) {
            return true;
        }
        ...
        return cache.inHeap(`needsBuilder:${sourceLocation}`, 100, () => {
            ...
            for (let i = path.length - 1; i >= 0; i--) {
                ...
                if (!road || road.hits < road.hitsMax / 5) {
                    return true;
                }
            }
    ...
    drawReport(targetPos: string) {
        if (hivemind.settings.get('disableRoomVisuals')) {
            /** empty I guess. */
            console.log('Not implemented "drawReport" in remote-mining.ts');
        }
...
export default RemoteMiningOperation;
