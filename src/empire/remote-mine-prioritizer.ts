...
export interface SourceRoomAvailability {
  current: number
  max: number
};

export default RemoteMinePrioritizer;
export class RemoteMinePrioritizer {
  ...
  getRoomsToMine(maxAmount: number): { rooms: string[], maxRooms: number } {
    ...
    for (const info of sortedHarvestRooms) {
      if (sourceRooms[info.origin].current >= sourceRooms[info.origin].max) {
        continue;
      }
      ...
      if (availableHarvestRoomCount < maxAmount) {
        ...
        if (roomFilter && !roomFilter(info.roomName)) {
          continue;
        }
  ...
  getRemoteMiningSourceRooms(): Record<string, SourceRoomAvailability> {
    ...
    for (const room of Game.myRooms) {
      ...
      if (powerCreep) {
        ...
        if (operateSpawnLevel > 0) {
          spawnCapacity /= POWER_INFO[PWR_OPERATE_SPAWN].effect[operateSpawnLevel - 1];
        }
      }
