...
export default class RoomPlanGenerator {
  ...
  isFinished(): boolean {
    return (
      ...
      this.variationIndex > this.variationGenerator.getVariationAmount()
    );
  }

  initVariation() {
    this.currentVariation =
      this.variationGenerator.getVariationList()[this.variationIndex++];
    const variationInfo = this.variationGenerator.getVariationInfo(
      this.currentVariation,
    );
  ...
  provideDistanceMatrixes() {
    ...
    [this.wallMatrix, this.exitMatrix] = new RoomPlanMatrixGenerator().generate(
      this.roomName,
    );
  }
