...
export default class LabSource extends StructureSource<LabSourceTask> {
   ...
   */
  addLabResourceOptions(
    options: LabSourceTask[],
    context: ResourceSourceContext,
  ) {
    ...
    for (const labID of labs) {
      ...
      if (context.resourceType && context.resourceType !== lab.mineralType)
        continue;
      ...
      if (lab && mineralAmount > 0) {
        if (
          room.boostManager.isLabUsedForBoosting(lab.id) &&
          lab.mineralType === room.boostManager.getRequiredBoostType(lab.id)
        )
          continue;
        ...
        if (
          currentReaction && // If we're doing a different reaction now, clean out faster!
        ...
        option.priority -=
          this.room.getCreepsWithOrder(this.getType(), lab.id).length * 2;
    ...
    this.addClearSourceLabOption(
      options,
      Game.getObjectById<StructureLab>(room.memory.labs.source1),
      context,
      currentReaction[0],
    );
    this.addClearSourceLabOption(
      options,
      Game.getObjectById<StructureLab>(room.memory.labs.source2),
      context,
      currentReaction[1],
    );
  }

  addClearSourceLabOption(
    options: LabSourceTask[],
    lab: StructureLab,
    context: ResourceSourceContext,
  ...
  addLabEvacuationOptions(
    ...
    for (const lab of labs) {
      ...
      if (lab.store[RESOURCE_ENERGY] > 0) {
        options.push({
          priority:
            3 - this.room.getCreepsWithOrder(this.getType(), lab.id).length * 2,
          weight: 0,
          type: this.getType(),
          target: lab.id,
