...
export default class CleanupProcess extends Process {
  ...
  removeConstructionSiteIfExpired(site: ConstructionSite) {
    ...
    if (this.memory.constructionSites[site.id]) {
      const hasNotProgressed =
        this.memory.constructionSites[site.id].progress === site.progress;
      lastProgressTime = hasNotProgressed
        ? this.memory.constructionSites[site.id].time
        : Game.time;
    }
  ...
  cleanupConstructionSiteMemory() {
    for (const id in this.memory.constructionSites) {
      if (!Game.getObjectById(id as Id<ConstructionSite>))
        delete this.memory.constructionSites[id];
    }
  }
}
