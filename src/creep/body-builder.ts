...
export default class BodyBuilder {
  ...
  private calculatePartCounts(): PartCounts {
    ...
    while (currentSize < this.maxSize) {
      ...
      if (
        this.partLimits[nextPart] &&
        partCounts[nextPart] >= this.partLimits[nextPart]
      )
        break;
      ...
      const partCost =
        BODYPART_COST[nextPart] + neededMoves * BODYPART_COST[MOVE];
  ...
  private getNextMovePartIncrement(
  ...
  private generateSortedParts(
    partCounts: Partial<Record<BodyPartConstant, number>>,
  ): BodyPartConstant[] {
