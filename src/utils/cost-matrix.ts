...
function getCostMatrix(
  ...
  const cacheDuration = hivemind.segmentMemory.isReady()
    ? roomHasBlockingConstructionSites(roomName)
      ? 20
      : 500
    : 1;

  let cacheKey = "costMatrix:" + roomName;
  let matrix = hivemind.segmentMemory.isReady()
    ? cache.inHeap(cacheKey, cacheDuration, () => {
  ...
  if (matrix && options.singleRoom && hivemind.segmentMemory.isReady()) {
    ...
    matrix = cache.inHeap(cacheKey, cacheDuration, () =>
...
function roomHasBlockingConstructionSites(roomName: string): boolean {
  return cache.inHeap(
    ...
    () =>
      (
        Game.rooms[roomName]?.find(FIND_MY_CONSTRUCTION_SITES, {
 ...
 */
function generateSingleRoomCostMatrix(
  matrix: CostMatrix,
  roomName: string,
): CostMatrix {
 ...
 *   The modified cost matrix.
 */
function generateQuadCostMatrix(
  matrix: CostMatrix,
  roomName: string,
): CostMatrix {
  const newMatrix = matrix.clone();
 ...
 *   The modified cost matrix.
 */
function generateCombatCostMatrix(
  matrix: CostMatrix,
  roomName: string,
  ...
  for (const username in Game.rooms[roomName].enemyCreeps) {
    ...
    for (const creep of Game.rooms[roomName].enemyCreeps[username]) {
      ...
      if (Game.rooms[roomName].roomPlanner.isPlannedLocation(creep.pos, "safe"))
        continue;
  ...
  while (openList.length > 0) {
    ...
    if (Game.rooms[roomName].roomPlanner.isPlannedLocation(pos, "safe"))
      continue;
  ...
  handleMapArea(
    ...
    (x, y) => {
      ...
      if (
        ...
        !Game.rooms[roomName].roomPlanner.isPlannedLocation(newPos, "road")
      )
        return;
      if (Game.rooms[roomName].roomPlanner.isPlannedLocation(newPos, "rampart"))
        return;
      if (Game.rooms[roomName].roomPlanner.isPlannedLocation(newPos, "safe"))
        return;
      dangerMatrix.set(x, y, 2);
      if (Game.rooms[roomName].roomPlanner.isPlannedLocation(newPos, "wall"))
        return;
...
function handleAdjacentTiles(
  pos: RoomPosition,
  openList: RoomPosition[],
...
function getDangerMatrix(roomName: string): CostMatrix {
  ...
  if (Game.rooms[roomName].defense.getEnemyStrength() === ENEMY_STRENGTH_NONE)
    return new PathFinder.CostMatrix();

  getCostMatrix(roomName);

  return (
    cache.fromHeap("dangerMatrix:" + roomName, true) ||
    new PathFinder.CostMatrix()
  );
}
...
function markSourceKeeperExits(
  roomName: string,
...
export { getCostMatrix, getDangerMatrix, markBuildings };
