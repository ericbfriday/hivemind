...
export interface DropDestinationTask extends ResourceDestinationTask {
  type: 'drop'
}

export default DropDestination;
export class DropDestination extends TaskProvider<DropDestinationTask, ResourceDestinationContext> {
  ...
  addDropResourceTasks(context: ResourceDestinationContext, options: DropDestinationTask[]) {
    ...
    for (const resourceType of getResourcesIn(creep.store)) {
      ...
      if (storageTarget && !wouldBlockTerminal) {
        continue;
      }
  ...
  isValid(task: DropDestinationTask, context: ResourceDestinationContext) {
    if (!context.creep.store[task.resourceType]) {
      return false;
    }
