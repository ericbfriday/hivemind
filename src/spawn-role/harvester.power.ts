...
export default class PowerHarvesterSpawnRole extends SpawnRole {
  ...
  addOptionsForTarget(
    ...
    const activePowerHarvesters = filter(
      ...
      (creep: Creep) => {
        ...
        const travelTime =
          ...
          ) || info.spawnRooms[sourceRoom.name].distance * 50;
        if (
          (creep.ticksToLive || CREEP_LIFE_TIME) <
          CREEP_SPAWN_TIME * MAX_CREEP_SIZE + travelTime
        )
          return false;
    ...
    if (
      currentDps < expectedDps &&
      dpsRatio <= hpsRatio &&
      timeToKill > 0 &&
      activePowerHarvesters.length < info.freeTiles
    ) {
      options.push({
  ...
  getCreepBody(
    room: Room,
    option: PowerHarvesterSpawnOption,
  ): BodyPartConstant[] {
