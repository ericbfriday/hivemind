...
type MilitaryTargetOption =
  ...
  | CreepTargetOption
  | HostileCreepTargetOption
  | HostileStructureTargetOption;
...
export default class BrawlerRole extends Role {
  ...
  run(creep: BrawlerCreep) {
    ...
    if (creep.memory.order) {
      ...
      const target = Game.getObjectById<Creep | AnyStructure>(
      ...
      if (
        ...
        !target.my &&
        this.attackMilitaryTarget(creep, target)
      )
        return;
    }
  ...
  initBrawlerState(creep: BrawlerCreep) {
    ...
    if (creep.memory.pathTarget) {
      ...
      const path = pathManager.getPathFor(
        decodePosition(creep.memory.pathTarget),
      );
      if (path) {
  ...
  calculateMilitaryTarget(creep: BrawlerCreep) {
    const best = utilities.getBestOption(
      this.getAvailableMilitaryTargets(creep),
    );
  ...
  getAvailableMilitaryTargets(creep: BrawlerCreep) {
    ...
    if (
      options.length === 0 &&
      creep.getActiveBodyparts(CLAIM) > 0 &&
      creep.memory.squadName.startsWith("expand")
    ) {
   ...
   *   An array of target options for this creep.
   */
  addMilitaryAttackOptions(
    creep: BrawlerCreep,
    options: MilitaryTargetOption[],
  ) {
    ...
    if (
      ...
      const structures = creep.room.controller.pos.findInRange(
        FIND_STRUCTURES,
        1,
        {
          filter: (structure) =>
    ...
    if (size(damaged) === 0) {
      damaged = creep.room.find(FIND_HOSTILE_CREEPS, {
        filter: (friendly) =>
          ...
          hivemind.relations.isAlly(friendly.owner.username),
      });
    }
  ...
  performMilitaryMove(creep: BrawlerCreep) {
    ...
    if (creep.memory.target) {
      ...
      if (!enemiesNearby && creep.interRoomTravel(targetPosition, allowDanger))
        return;
      ...
      if (enemiesNearby) {
        ...
        container
          .get("CombatManager")
          .performKitingMovement(
            creep,
            container.get("CombatManager").getMostValuableTarget(creep),
          );
        return;
      }
    }
    ...
    if (creep.memory.squadName) {
      ...
      if (targetPos) {
        creep.whenInRange(
          this.isPositionBlocked(targetPos) ? 1 : 0,
          targetPos,
          () => {
      ...
      const result = PathFinder.search(
        ...
        { pos: target.pos, range: 2 },
        {
          roomCallback: (roomName) =>
            getCostMatrix(roomName, { ignoreMilitary: true }),
          flee: true,
          maxRooms: 1,
        },
      );
  ...
  performTrainMove(creep: BrawlerCreep) {
    ...
    if (!creep.isTrainFullySpawned()) {
      ...
      const direction = (1 +
        Math.floor(Math.random() * 8)) as DirectionConstant;
  ...
  performSquadMove(creep: BrawlerCreep) {
    ...
    if (creep.ticksToLive < CREEP_LIFE_TIME * 0.66) {
      const spawn = creep.pos.findClosestByRange<StructureSpawn>(
        FIND_STRUCTURES,
        {
  ...
  performMilitaryAttack(creep: BrawlerCreep) {
    if (creep.memory.order) {
      ...
      if (
        ...
        return (creep.getActiveBodyparts(ATTACK) || 0) > 0;
    }
    ...
    for (const hostile of hostiles) {
      ...
      if (hostile.owner && hivemind.relations.isAlly(hostile.owner.username))
        continue;
    ...
    const structures = creep.pos.findInRange(FIND_HOSTILE_STRUCTURES, 1, {
      filter: (structure) =>
        ...
        (!structure.owner ||
          !hivemind.relations.isAlly(structure.owner.username)),
    });
    ...
    for (const structure of structures) {
      if (
        structure.hits &&
        (!lowestStructure || structure.hits < lowestStructure.hits)
      ) {
    ...
    if (target instanceof StructureController) {
      ...
      if (creep.memory.squadName) {
        ...
        if (targetPos && targetPos.getRangeTo(target) === 0) {
          if (
            target.reservation &&
            target.reservation.username !== getUsername()
          ) {
      ...
      !("owner" in target) ||
      !hivemind.relations.isAlly(target.owner.username)
    ) {
