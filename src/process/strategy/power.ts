...
export default class PowerMiningProcess extends Process {
  ...
  getPotentialSpawnRoomsForHarvesting(
    roomName: string,
    ...
    for (const room of Game.myRooms) {
      ...
      if (
        ...
        hivemind.settings.get("minEnergyForPowerHarvesting")
      )
        continue;
      if (room.controller.level < hivemind.settings.get("minRclForPowerMining"))
        continue;
      if (
        Game.map.getRoomLinearDistance(roomName, room.name) >
        hivemind.settings.get("maxRangeForPowerMining")
      )
        continue;
      ...
      if (
        ...
        roomRoute.path.length >
          2 * hivemind.settings.get("maxRangeForPowerMining")
      )
        continue;
