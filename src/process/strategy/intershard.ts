...
export default class InterShardProcess extends Process {
  ...
  updateShardInfo() {
    ...
    for (const roomName of this.roomStatus.getPotentialExpansionTargets()) {
      ...
      if (
        ...
        roomStats.bestExpansion.score < expansionScore
      ) {
        roomStats.bestExpansion = {
          name: roomName,
  ...
  distributeCPU() {
    ...
    const totalCpu = sum(Object.values(Game.cpu.shardLimits));
    const newLimits = {};
    each(this._shardData, (data, shardName) => {
      if (shardName === "total") return;

      newLimits[shardName] = Math.round(
        (totalCpu * data.neededCpu) / this._shardData.total.neededCpu,
      );
    });
  ...
  addShardData(shardName: string, shardMemory?: ShardMemory) {
    ...
    if (shardMemory.info) {
      ...
      this._shardData[shardName].neededCpu =
        3 + this._shardData[shardName].rooms;

      if (
        shardMemory.info.interShardExpansion &&
        this.isAdjacentShardFuntional(shardName)
      ) {
  ...
  isAdjacentShardFuntional(shardName: string) {
    ...
    each(this._shardData, (data, compareShard) => {
      ...
      const compareMemory =
        compareShard === Game.shard.name
          ? interShard.getLocalMemory()
          : interShard.getRemoteMemory(compareShard);
      if (!compareMemory.info) return null;
      if (!compareMemory.portals || !compareMemory.portals[shardName])
        return null;
      if (
        (compareMemory.info.maxRoomLevel || 0) < minRoomLevelToIntershardScout
      )
        return null;
  ...
  redistributeExtraCPU(newLimits: Record<string, number>) {
    ...
    each(newLimits, (limit, shardName) => {
      ...
      newLimits[shardName] = Math.round(
        limit + (extraCpu * limit) / otherShardsTotal,
      );
    });
  }
