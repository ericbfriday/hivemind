...
export interface LabSourceTask extends StructureSourceTask {
  type: 'lab'
  target: Id<StructureLab | StructureStorage | StructureTerminal>
}

export default LabSource;
export class LabSource extends StructureSource<LabSourceTask> {
  ...
  addLabResourceOptions(options: LabSourceTask[], context: ResourceSourceContext) {
    ...
    for (const labID of labs) {
      ...
      if (lab && mineralAmount > 0) {
        if (room.boostManager.isLabUsedForBoosting(lab.id) && lab.mineralType === room.boostManager.getRequiredBoostType(lab.id)) {
          continue;
        }
  ...
  addClearSourceLabOption(options: LabSourceTask[], lab: StructureLab, context: ResourceSourceContext, resourceType: ResourceConstant) {
    ...
    if (lab.store.getUsedCapacity(lab.mineralType) === 0) {
      return;
    }
  ...
  addLabEvacuationOptions(options: LabSourceTask[], context: ResourceSourceContext) {
    ...
    for (const lab of labs) {
      if (this.room.boostManager.isLabUsedForBoosting(lab.id)) {
        continue;
      }
