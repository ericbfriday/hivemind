...
export default class RoomIntel {
  ...
  gatherIntel() {
    ...
    if (
      intel.lastScan &&
      !hivemind.hasIntervalPassed(lastScanThreshold, intel.lastScan)
    )
      return;
    hivemind
      .log("intel", room.name)
      .debug(
        "Gathering intel after",
        intel.lastScan ? Game.time - intel.lastScan : "infinite",
        "ticks.",
      );
  ...
  gatherControllerIntel(room: Room) {
    ...
    if (!room.controller) {
      const invaderCores = room.structuresByType[
  ...
  gatherPowerIntel(powerBanks: StructurePowerBank[]) {
    ...
    hivemind
      .log("intel", this.roomName)
      .info("Power bank containing", powerBank.power, "power found!");
    ...
    if (Memory.strategy) {
      ...
      if (
        !Memory.strategy.power.rooms[this.roomName] ||
        !Memory.strategy.power.rooms[this.roomName].isActive
      ) {
  ...
  gatherDepositIntel() {
    ...
    for (const deposit of deposits) {
      ...
      hivemind
        .log("intel", this.roomName)
        .info("Deposit containing", deposit.depositType, "found!");
      ...
      if (Memory.strategy) {
        ...
        if (
          !Memory.strategy.deposits.rooms[this.roomName] ||
          !Memory.strategy.deposits.rooms[this.roomName].isActive
        ) {
  ...
  gatherPortalIntel(portals: StructurePortal[]) {
    ...
    for (const portal of portals || []) {
      ...
      if (!targetRooms.includes(portal.destination.roomName))
        targetRooms.push(portal.destination.roomName);
    }
  ...
  gatherExitIntel(roomName: string) {
    ...
    for (const dir in this.memory.exits) {
      if (!this.isAvailableExitDirection(roomName, this.memory.exits[dir]))
        delete this.memory.exits[dir];
    }
  }

  isAvailableExitDirection(roomName: string, otherRoomName: string): boolean {
    return (
      Game.map.getRoomStatus(otherRoomName).status ===
  ...
  gatherInvaderIntel(structures: Record<string, Structure[]>) {
    ...
    this.memory.invaderInfo = {
      ...
      activates: core.ticksToDeploy
        ? Game.time + core.ticksToDeploy
        : undefined,
      collapses: null,
    };
  ...
  gatherPathfindingInfo(structures, constructionSites) {
    const obstaclePositions = this.generateObstacleList(
      this.roomName,
      structures,
      constructionSites,
    );
  ...
  isClaimed(): boolean {
    ...
    if (
      ...
      this.memory.reservation.username !== getUsername()
    )
      return true;
  ...
  getCostMatrix(): CostMatrix {
    ...
    if (this.memory.costPositions) {
      obstaclePositions = {
        obstacles: unpackCoordListAsPosList(
          this.memory.costPositions[0],
          this.roomName,
        ),
        roads: unpackCoordListAsPosList(
          this.memory.costPositions[1],
          this.roomName,
        ),
      };
    }
  ...
  getStructures(
    structureType: StructureConstant,
  ): Record<string, { x: number; y: number; hits: number; hitsMax: number }> {
    if (!this.memory.structures || !this.memory.structures[structureType])
      return {};
   ...
   */
  addAdjacentRoomToCheck(
    roomName: string,
    openList: Record<string, AdjacentRoomEntry>,
    base: { range: number; dir: string },
  ) {
    ...
    if (Game.rooms[roomName] && Game.rooms[roomName].isMine()) {
      ...
      if (
        Game.rooms[roomName].controller.level >=
          Math.min(5, this.getRcl() - 1) &&
        !Game.rooms[roomName].isEvacuating() &&
        !Game.rooms[roomName].isStripmine()
      )
        return false;
   ...
   */
  handleAdjacentRoom(
    roomData: AdjacentRoomEntry,
    openList: Record<string, AdjacentRoomEntry>,
    closedList: Record<string, AdjacentRoomEntry>,
  ) {
    ...
    for (const roomName of _.values<string>(roomIntel.getExits())) {
      ...
      this.addAdjacentRoomToCheck(roomName, openList, {
        dir: roomData.origin,
...
function getRoomIntel(roomName: string): RoomIntel {
  if (!hivemind.segmentMemory.isReady())
    throw new Error(
      "Memory is not ready to generate room intel for room " + roomName + ".",
    );
...
export { getRoomIntel, getRoomsWithIntel };
