 ...
 */
Creep.prototype.whenInRange = function (
  this: Creep | PowerCreep,
  range,
  target,
  callback,
  ...
  if (
    visual &&
    !settings.get("disableRoomVisuals") &&
 ...
 */
Creep.prototype.setCachedPath = function (
  this: Creep | PowerCreep,
  path,
  reverse,
  distance,
...
Creep.prototype.getCachedPath = function (this: Creep | PowerCreep) {
  ...
  if (
    ...
    (typeof this.memory.cachedPath.path[0] === "string" &&
      !decodePosition(this.memory.cachedPath.path[0]).isEqualTo(
        this.heapMemory._decodedCachedPath[0],
      ))
  ) {
    this.heapMemory._decodedCachedPath = deserializePositionPath(
...
Creep.prototype.followCachedPath = function (this: Creep | PowerCreep) {
  ...
  if (
    ...
    hivemind
      .log("creeps", this.room.name)
      .error(this.name, "Trying to follow non-existing path");
    return;
  }
  ...
  if (this.memory.cachedPath.forceGoTo) {
    ...
    if (this.pos.getRangeTo(pos) > 0) {
      ...
      const path = this.calculatePath(pos, {
        avoidNearbyCreeps: this.heapMemory._stuckDetection[1] > 2,
      });
      ...
      if (
        settings.get("visualizeCreepMovement") &&
        !settings.get("disableRoomVisuals")
      ) {
  ...
  } else if (!this.memory.cachedPath.position && this.getOntoCachedPath())
    return;
...
Creep.prototype.getOntoCachedPath = function (this: Creep | PowerCreep) {
  ...
  if (this.pos.x === target.x && this.pos.y === target.y) {
    ...
    for (const [i, element] of path.entries()) {
      if (
        this.pos.x === element.x &&
        this.pos.y === element.y &&
        this.pos.roomName === element.roomName
      ) {
...
function getToPathRoom(creep: Creep | PowerCreep): void {
  if (creep.pos.roomName === creep.getCachedPath()[0].roomName) {
    ...
    if (path[0].roomName === creep.pos.roomName) {
      ...
      if (otherCreep)
        container.get("TrafficManager").setBlockingCreep(creep, otherCreep);

      const powerCreep = path[0].lookFor(LOOK_POWER_CREEPS)[0];
      if (powerCreep)
        container.get("TrafficManager").setBlockingCreep(creep, powerCreep);
    } else {
      creep.moveTo(path[0]);
    }
  } else {
...
function updateStuckDetection(creep: Creep | PowerCreep) {
  if (
    !creep.heapMemory._stuckDetection ||
    new RoomPosition(
      creep.heapMemory._stuckDetection[0].x,
      creep.heapMemory._stuckDetection[0].y,
      creep.heapMemory._stuckDetection[0].roomName,
    ).getRangeTo(creep.pos) > 0
  ) {
  ...
  creep.room.visual.text(
    creep.heapMemory._stuckDetection[1].toString(),
    creep.pos,
  );
}
...
Creep.prototype.manageBlockingCreeps = function (this: Creep | PowerCreep) {
  const path = this.getCachedPath();
  if (
    typeof this.memory.cachedPath.position === "undefined" ||
    this.memory.cachedPath.position === null
  ) {
...
Creep.prototype.incrementCachedPathPosition = function (
  ...
  if (next.roomName !== this.pos.roomName) {
    ...
    if (
      afterNext &&
      afterNext.roomName === this.pos.roomName &&
      afterNext.getRangeTo(this.pos) <= 1
    ) {
...
Creep.prototype.moveAroundObstacles = function (this: Creep | PowerCreep) {
  ...
  if (!("fatigue" in this) || this.fatigue === 0) {
    ...
    this.memory.cachedPath.lastPositions[Game.time % REMEMBER_POSITION_COUNT] =
      encodePosition(this.pos);
  }
...
Creep.prototype.canMoveOnto = function (this: Creep | PowerCreep, position) {
  ...
  if (
    ...
    powerCreeps[0].id !== this.id &&
    !isMovingCreep(powerCreeps[0])
  )
    return false;
...
function drawCreepMovement(creep: Creep | PowerCreep) {
  ...
  const target = creep.memory.go?.target
    ? decodePosition(creep.memory.go.target)
    : null;

  const color = getVisualizationColor(creep);
  const pathPosition =
    creep.memory.cachedPath?.position || creep.memory.cachedPath?.forceGoTo;
...
Creep.prototype.goTo = function (
  ...
  if (
    ...
    if (!this.calculateGoToPath(target, options)) {
      hivemind
        .log("creeps", this.room.name)
        .error("No path from", this.pos, "to", target, "found!");
      return false;
    }
  }
 ...
 */
Creep.prototype.calculateGoToPath = function (
  this: Creep | PowerCreep,
  target,
  options,
) {
...
Creep.prototype.calculatePath = function (
  this: Creep | PowerCreep,
  target: RoomPosition,
  options?: GoToOptions,
): RoomPosition[] {
 ...
 *   True if movement is possible and ongoing.
 */
Creep.prototype.moveToRoom = function (
  this: Creep | PowerCreep,
  roomName,
  allowDanger,
) {
 ...
 *   could be found.
 */
Creep.prototype.calculateRoomPath = function (
  this: Creep | PowerCreep,
  roomName,
  allowDanger,
) {
...
Creep.prototype.interRoomTravel = function (
  this: Creep | PowerCreep,
  targetPos,
  allowDanger = false,
): boolean {
...
Creep.prototype.moveUsingNavMesh = function (
  this: Creep | PowerCreep,
  targetPos,
  options: GoToOptions,
) {
  ...
  const nextPos = decodePosition(
    this.heapMemory._nmp.path[this.heapMemory._nmpi],
  );
  ...
  if (
    ...
    Game.map.getRoomLinearDistance(nextPos.roomName, this.pos.roomName) > 3
  ) {
    const previousPosition = decodePosition(
      this.heapMemory._nmp.path[this.heapMemory._nmpi - 1],
    );
    if (
      Game.map.getRoomLinearDistance(
        previousPosition.roomName,
        this.pos.roomName,
      ) <= 3
    ) {
  ...
  if (
    this.pos.roomName !== nextPos.roomName ||
    this.pos.getRangeTo(nextPos) > 1
  ) {
  ...
  if (Game.rooms[nextPos.roomName]) {
    ...
    if (portal) {
      ...
      handleMapArea(this.pos.x, this.pos.y, (x, y) => {
        ...
        if (
          portalHere &&
          portalHere.destination instanceof RoomPosition &&
          portal.destination instanceof RoomPosition &&
          portalHere.destination.roomName === portal.destination.roomName &&
          this.move(this.pos.getDirectionTo(x, y)) === OK
        ) {
          this.heapMemory._nmpi++;
          portalUsed = true;
          return false;
...
PowerCreep.prototype.incrementCachedPathPosition =
  Creep.prototype.incrementCachedPathPosition;
...
PowerCreep.prototype.manageBlockingCreeps =
  Creep.prototype.manageBlockingCreeps;
...
PowerCreep.prototype.getNavMeshMoveTarget =
  Creep.prototype.getNavMeshMoveTarget;
