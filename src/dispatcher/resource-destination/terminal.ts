...
export interface TerminalDestinationTask extends StructureDestinationTask {
  type: 'terminal'
  target: Id<StructureTerminal>
}

export class TerminalDestination extends StructureDestination<TerminalDestinationTask> {
  ...
  getTasks(context: ResourceDestinationContext) {
    ...
    if (this.room.isClearingTerminal()) {
      return [];
    }
  ...
  addMinimumEnergyDestination(options: TerminalDestinationTask[], context: ResourceDestinationContext) {
    ...
    if (this.terminalNeedsClearing()) {
      return;
    }

    const terminal = this.room.terminal;
    if (terminal.store[RESOURCE_ENERGY] >= 5_000) {
      return;
    }
  ...
  addTransferResourceDestination(options: TerminalDestinationTask[], context: ResourceDestinationContext) {
    if (!this.room.memory.fillTerminal) {
      return;
    }
    ...
    if (terminal.store.getFreeCapacity(resourceType) <= missingAmount) {
      return;
    }
  ...
  addResourcesForSaleDestination(options: TerminalDestinationTask[], context: ResourceDestinationContext) {
    ...
    for (const order of roomSellOrders) {
      ...
      if (terminal.store[resourceType] >= order.remainingAmount) {
        continue;
      }
      if (this.room.getCurrentResourceAmount(resourceType) < order.remainingAmount) {
        continue;
      }
      if (terminal.store.getFreeCapacity(resourceType) < order.remainingAmount - terminal.store[resourceType]) {
        continue;
      }
...
export default TerminalDestination;
