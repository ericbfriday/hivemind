  ...
  getPath(
    ...
    const options: PathFinderOpts = {
      ...
      roomCallback: (roomName) => {
        ...
        if (
          !(allowDanger || addOptions.allowDanger) &&
          hivemind.segmentMemory.isReady() &&
          getRoomIntel(roomName).isOwned() &&
          (!addOptions.whiteListRooms ||
            !addOptions.whiteListRooms.includes(roomName))
        ) {
          return false;
        }
        ...
        const options = {
          singleRoom:
            addOptions.singleRoom && addOptions.singleRoom === roomName,
        ...
        if (
          addOptions.avoidNearbyCreeps &&
          roomName === startPosition.roomName
        ) {
          ...
          handleMapArea(
            ...
            startPosition.y,
            (x, y) => {
              const creeps = new RoomPosition(x, y, roomName).lookFor(
                LOOK_CREEPS,
              );
              if (creeps.length > 0) {
