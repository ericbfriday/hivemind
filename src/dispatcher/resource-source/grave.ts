...
export interface GraveSourceTask extends ResourceSourceTask {
    type: 'grave'
    target: Id<Tombstone | Ruin>
}

export default GraveSource;
export class GraveSource extends TaskProvider<GraveSourceTask, ResourceSourceContext> {
    ...
    isValid(task: GraveSourceTask, context: ResourceSourceContext): boolean {
        ...
        if (tombstone.store.getUsedCapacity(task.resourceType) === 0) {
            return false;
        }
        if (context.creep.store.getFreeCapacity(task.resourceType) === 0) {
            return false;
        }
        if (!this.isSafePosition(context.creep, tombstone.pos)) {
            return false;
        }
