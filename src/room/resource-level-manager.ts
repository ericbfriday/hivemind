...
export default class ResourceLevelManager {
  ...
  private getCommodityCutoffs(
    ...
    if (
      !this.isCommodityNeededAtFactoryLevel(
        room.factory.getEffectiveLevel(),
        resourceType,
      )
    ) {
      return [1, 0, 0];
  ...
  private isCommodityNeededAtFactoryLevel(
    factoryLevel: number,
    resourceType: ResourceConstant,
  ): boolean {
  ...
  private getBoostCutoffs(
    ...
    for (const bodyPart in BOOSTS) {
      ...
      if (
        bodyPart === WORK &&
        BOOSTS[bodyPart][resourceType].repair &&
        room.defense.getEnemyStrength() > ENEMY_STRENGTH_NORMAL
      ) {
        return [15_000, 7500, 2500];
      }
      ...
      if (
        bodyPart === WORK &&
        BOOSTS[bodyPart][resourceType].upgradeController &&
  ...
  private isReactionResource(
    ...
    if (
      reaction &&
      (resourceType === reaction[0] || resourceType === reaction[1])
    ) {
      return true;
    }
