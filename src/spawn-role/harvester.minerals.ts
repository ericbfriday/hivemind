...
export interface MineralHarvesterSpawnOption extends SpawnOption {
    source: Id<Mineral>
}

export class MineralHarvesterSpawnRole extends SpawnRole {
    /**
    ...
    getSpawnOptions(room: Room): MineralHarvesterSpawnOption[] {
        return this.cacheEmptySpawnOptionsFor(room, 50, () => {
            ...
            if (room.getEffectiveAvailableEnergy() < 5000) {
                return [];
            }
            ...
            const minerals = room.find(FIND_MINERALS, {
                filter: (mineral) => {
                    const extractors = mineral.mineralAmount > 0
                        ? _.filter(room.myStructuresByType[STRUCTURE_EXTRACTOR], structure => structure.isOperational() && mineral.pos.isEqualTo(structure.pos))
                        : [];
            ...
            for (const mineral of minerals) {
                ...
                if (_.size(mineralHarvesters) >= maxHarvesters) {
                    continue;
                }

                const minAmount = 0;
                if (mineral.mineralAmount <= minAmount) {
                    continue;
                }
...
export default MineralHarvesterSpawnRole;
