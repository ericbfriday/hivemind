import type PlacementManager from 'room/planner/placement-manager';
import type RoomPlan from 'room/planner/room-plan';
import { getRoomIntel } from '@/room-intel';
import _ from 'lodash';

export interface ScoredTowerPosition {
    pos: RoomPosition
    score: number
}

export default placeTowersStep;
export class placeTowersStep {
    constructor(
    ...
    run(): StepResult {
        ...
        while (this.roomPlan.canPlaceMore('tower')) {
            ...
            while (newTowers.length < this.roomPlan.remainingStructureCount('tower')) {
                ...
                if (!info || typeof info === 'number' || info.score < 0) {
                    break;
                }
            ...
            for (const pos of newTowers) {
                ...
                if (!this.placementManager.isPositionAccessible(pos, true)) {
                    continue;
                }
    ...
    findTowerPositions(): ScoredTowerPosition[] {
        ...
        const allDirectionsSafe = _.sumBy(Object.entries(safety.directions).filter(Boolean)) === 4;
        if (allDirectionsSafe) {
            return positions;
        }
        ...
        for (let x = 1; x < 49; x++) {
            for (let y = 1; y < 49; y++) {
                ...
                if (this.safetyMatrix.get(x, y) !== 1) {
                    continue;
                }
