  ...
  transportResources(routes: TransportRouteOption[]) {
    ...
    while (best) {
      ...
      if (
        ...
        room.terminal.store.getFreeCapacity(room.memory.fillTerminal) < 5000
      ) {
      ...
      if (best.source === best.target) {
        ...
        terminal.store[best.resourceType] &&
        terminal.store[best.resourceType] >= tradeVolume * 0.9
      ) {
        ...
        (!room?.storage[best.resourceType] ||
          terminal.store.getFreeCapacity() <
            terminal.store.getCapacity() * 0.05) &&
        terminal.store[best.resourceType]
      ) {
        ...
        if (
          !room.memory.fillTerminal &&
          room.terminal.store.getFreeCapacity(best.resourceType) -
            room.terminal.store.getUsedCapacity(best.resourceType) >
            tradeVolume
        ) {
          hivemind
            .log("trade")
