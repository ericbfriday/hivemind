...
export default RoomPlanScorer;
export class RoomPlanScorer {
  ...
  getDefensibilityScore(plan: RoomPlan, exitMatrix?: CostMatrix): number {
    ...
    for (const rampartPosition of ramparts) {
      ...
      if (exitDistance < 6) {
        total += (6 - exitDistance) * (6 - exitDistance);
      }
    }
