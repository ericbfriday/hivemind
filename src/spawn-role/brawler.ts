...
export class BrawlerSpawnRole extends SpawnRole {
  navMesh: NavMesh;
  ...
  getRemoteDefenseSpawnOptions(room: Room, options: BrawlerSpawnOption[]) {
    ...
    for (const pos of harvestPositions) {
      ...
      if (!operation.isUnderAttack() && !operation.hasInvaderCore()) {
        continue;
      }

      // Only do costly defense if we have enough energy.
      if (operation.isUnderAttack() && room.getEffectiveAvailableEnergy() < 5_000) {
        return;
      }

      // Don't defend operations where we still need a dismantler.
      if (operation.isUnderAttack() && operation.needsDismantler()) {
        continue;
      }

      // Only spawn defenders for actively used sources.
      if (!this.isActivelyUsedSource(room, pos)) {
        continue;
      }
      ...
      if (room.memory.recentBrawler && Game.time - (room.memory.recentBrawler[targetPos] || -10_000) < defenseTimeDiff) {
        continue;
      }

      const brawlers = _.filter(Game.creepsByRole.brawler || [], (creep: BrawlerCreep) => creep.memory.operation === `mine:${pos.roomName}`);
      if (_.size(brawlers) > 0) {
        continue;
      }
  ...
  getPowerHarvestDefenseSpawnOptions(room: Room, options: BrawlerSpawnOption[]) {
    ...
    if (room.getEffectiveAvailableEnergy() < 10_000) {
      return;
    }
    ...
    _.each(Memory.strategy.power.rooms, (info, roomName) => {
      ...
      if (!roomMemory || !roomMemory.enemies) {
        return;
      }
      ...
      const brawlers = _.filter(Game.creepsByRole.brawler || [], (creep: Creep) => creep.memory.target && decodePosition(creep.memory.target).roomName === roomName);
      if (_.size(brawlers) > 0) {
        return;
      }
      ...
      if (enemies.parts[ATTACK]) {
        enemies.parts[ATTACK] *= 0.1;
      }
  ...
  getTrainPartSpawnOptions(room: Room, options: BrawlerSpawnOption[]) {
    const trainStarters: Creep[] = _.filter(Object.values(room.creepsByRole.brawler) || [], (creep: Creep) => ((creep.memory.train && _.size(creep.memory.train.partsToSpawn) > 0)));
  ...
  getReclaimSpawnOptions(room: Room, options: BrawlerSpawnOption[]) {
    ...
    for (const targetRoom of Game.myRooms) {
      ...
      if (!this.canReclaimRoom(targetRoom, room)) {
        continue;
      }
  ...
  canReclaimRoom(targetRoom: Room, room: Room): boolean {
    if (!targetRoom.needsReclaiming()) {
      return false;
    }
    if (!targetRoom.isSafeForReclaiming()) {
      return false;
    }
    if (!targetRoom.roomPlanner) {
      return false;
    }
...
export default BrawlerSpawnRole;
