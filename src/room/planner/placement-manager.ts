...
export default class PlacementManager {
  ...
  discouragePosition(x: number, y: number) {
    if (
      this.buildingMatrix.get(x, y) >= this.DISCOURAGED_POSITION ||
      this.buildingMatrix.get(x, y) === this.ROAD_POSITION
    )
      return;
  ...
  isBuildableTile(
    ...
    if (
      matrixValue === this.DISCOURAGED_POSITION &&
      this.wallDistanceMatrix.get(x, y) <= 2
    )
      return true;
  ...
  placeAll(structureType: StructureConstant, addRoad: boolean) {
    while (this.roomPlan.canPlaceMore(structureType)) {
      ...
      this.planLocation(
        new RoomPosition(nextPos.x, nextPos.y, this.roomPlan.roomName),
        structureType,
      );
  ...
  isPositionAccessible(pos: RoomPosition, noTunnels?: boolean) {
    ...
    if (noTunnels) {
      for (const position of result.path) {
        if (
          this.terrain.get(position.x, position.y) === TERRAIN_MASK_WALL &&
          !this.roomPlan.hasPosition("road", position)
        )
          return false;
      }
    }
   ...
   *   Value to set in the pathfinding costmatrix at this position (Default 255).
   */
  planTemporaryLocation(
    pos: RoomPosition,
    locationType: string,
    pathFindingCost?: number,
  ) {
    if (!this.costMatrixBackup[encodePosition(pos)]) {
      this.costMatrixBackup[encodePosition(pos)] = this.buildingMatrix.get(
        pos.x,
        pos.y,
      );
    }
  ...
  discardTemporaryLocations(locationType: string) {
    for (const position of this.roomPlan.getPositions(
      locationType + "_placeholder",
    )) {
