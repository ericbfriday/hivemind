...
export class ScoutProcess extends Process {
  ...
  run() {
    ...
    for (const roomName of this.roomStatus.getAllKnownRooms()) {
      ...
      if (Memory.strategy.roomListProgress.includes(roomName)) {
        continue;
      }
  ...
  calculateHarvestScore(roomName: string) {
    const range = this.roomStatus.getDistanceToOrigin(roomName);
    if (range === 0 || range > hivemind.settings.get('maxRemoteMineRoomDistance')) {
      return 0;
    }
  ...
  calculateExpansionScore(roomName: string) {
    ...
    for (const otherRoom of _.values<Room>(Game.rooms)) {
      ...
      const newSafeExits = Math.abs(_.sumBy(Object.entries(adjustedSafety.directions)) - _.sumBy(Object.entries(normalSafety.directions)));
    ...
    result.addScore(_.sumBy(Object.values(safety.directions)) * 0.25, 'safeExits');
    ...
    const unsafeRatio = (4 - _.sumBy(Object.keys(safety.directions))) / _.size(exits);
  ...
  getExpansionScoreFromCache(roomName: string, result: ExpansionScore) {
    if (!expansionScoreCache[roomName]) {
      return false;
    }
    if (hivemind.hasIntervalPassed(hivemind.settings.get('expansionScoreCacheDuration'), expansionScoreCache[roomName][1])) {
      return false;
    }
  ...
  getHarvestRoomScore(roomName: string, forOwnedRoom = false) {
    ...
    if (!forOwnedRoom) {
      ...
      if (roomIntel.isClaimed() && roomIntel.memory.reservation.username !== 'Invader') {
        sourceFactor = 0.1;
      }
    }
  ...
  addAdjacentRooms(roomName: string, openList: Record<string, ScoutTarget>, closedList: Record<string, boolean>) {
    ...
    for (const portal of roomIntel.getRoomPortals()) {
      if (openList[portal] || closedList[portal]) {
        continue;
      }
