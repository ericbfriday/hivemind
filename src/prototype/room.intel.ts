...
function getCachedStructureReference<
  T extends (typeof structuresToReference)[number],
>(room: Room, structureType: T): Structure<T> {
...
Object.defineProperty(Room.prototype, "factoryManager", {
  get(this: Room) {
    return cache.inObject(this, "factoryManager", 1, () =>
      this.factory.isOperational() ? new FactoryManager(this.name) : null,
    );
  },
  enumerable: false,
  configurable: true,
...
Room.prototype.updateControllerContainer = function (this: Room) {
  ...
  if (this.roomPlanner) {
    const containerPositions: RoomPosition[] = this.roomPlanner.getLocations(
...
Room.prototype.needsScout = function (this: Room) {
  ...
  return roomStatus
    .getPotentialScoutTargets()
    .some((roomName) => roomStatus.getOrigin(roomName) === this.name);
};
