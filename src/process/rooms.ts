...
export default class RoomsProcess extends Process {
  ...
  run() {
    each(Game.rooms, (room, roomName) => {
      ...
      if (
        Game.operations["mine:" + room.name] &&
        settings.get("visualizeRemoteMines")
      ) {
  ...
  manageExpansionRoomPlan() {
    ...
    this.runRoomPlannerAndManager(
      Memory.strategy.expand.currentTarget.roomName,
    );
  }
    ...
    if (Game.rooms[roomName]) {
      ...
      hivemind.runSubProcess("rooms_manager", () => {
        hivemind.runProcess(room.name + "_manager", RoomManagerProcess, {
          interval: prioritizeRoomManager
            ? 0
            : room.needsReclaiming()
              ? 20
              : 100,
          room,
