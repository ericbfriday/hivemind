...
Object.defineProperty(Room.prototype, "sourceDispatcher", {
  get(this: Room) {
    return cache.inObject(
      ...
      "sourceDispatcher",
      1,
      () => new ResourceSourceDispatcher(this),
    );
  },
  enumerable: false,
  configurable: true,
...
Object.defineProperty(Room.prototype, "destinationDispatcher", {
  get(this: Room) {
    return cache.inObject(
      ...
      1,
      () => new ResourceDestinationDispatcher(this),
    );
  },
  enumerable: false,
  configurable: true,
 ...
 */
Room.prototype.getCurrentResourceAmount = function (
  this: Room,
  ...
  return cache.inObject(room, "allResources", 1, () => {
    ...
    for (const resourceType of getResourcesIn(room.storage?.store)) {
      resources[resourceType] =
        (resources[resourceType] || 0) + room.storage.store[resourceType];
    }

    for (const resourceType of getResourcesIn(room.terminal?.store)) {
      resources[resourceType] =
        (resources[resourceType] || 0) + room.terminal.store[resourceType];
    }
...
Room.prototype.getStoredEnergy = function (this: Room) {
  return cache.inObject(this, "storedEnergy", 1, () => {
    ...
    const storagePosition = new RoomPosition(
      storageLocation.x,
      storageLocation.y,
      this.name,
    );
    ...
    for (const position of harvestPositions || []) {
      ...
      for (const structure of position.lookFor(LOOK_STRUCTURES)) {
        ...
        total += (structure as StructureContainer).store.getUsedCapacity(
          RESOURCE_ENERGY,
        );
      }
    }
    ...
    const container =
      ...
      Game.getObjectById<StructureContainer>(this.memory.controllerContainer);
...
Room.prototype.getEffectiveAvailableEnergy = function (this: Room) {
  ...
  if (!this.factory || !this.factory.isOperational() || this.isEvacuating())
    return availableEnergy;

  // @todo Get resource unpacking factor from API or config.
  return (
    availableEnergy +
    Math.max(0, this.getCurrentResourceAmount(RESOURCE_BATTERY) - 5000) * 5
  );
};
...
Room.prototype.isFullOnEnergy = function (this: Room) {
  return (
    this.getCurrentResourceAmount(RESOURCE_ENERGY) > this.getStorageLimit() / 2
  );
};
...
Room.prototype.isFullOnPower = function (this: Room) {
  return (
    this.getCurrentResourceAmount(RESOURCE_POWER) > this.getStorageLimit() / 6
  );
};
 ...
 */
Room.prototype.isFullOn = function (
  this: Room,
  resourceType: ResourceConstant,
) {
 ...
 */
Room.prototype.prepareForTrading = function (
  this: Room,
  resourceType: ResourceConstant,
  amount?: number,
) {
  if (!amount)
    amount = Math.min(10_000, this.getCurrentResourceAmount(resourceType));
...
function getRemoteHarvestSourceEvaluation(
  operation: RemoteMiningOperation,
  location: string,
): SourceEvaluation {
  const filteredPaths = Object.values(operation.getPaths()).filter((path) => path.path);
  ...
  return {
    ...
    averageDistance:
      _.sumBy(filteredPaths, (path) => path.path.length) / _.size(filteredPaths),
  };
}
...
Room.prototype.getResourceState = function (this: Room) {
  ...
  return cache.inObject(this, "resourceState", 1, () => {
    const roomData: RoomResourceState = {
      ...
      canTrade: false,
      addResource(
        this: RoomResourceState,
        resourceType: ResourceConstant,
        amount: number,
      ) {
        this.totalResources[resourceType] =
          (this.totalResources[resourceType] || 0) + amount;
      },
      isEvacuating: false,
      mineralTypes: [],
    };
    ...
    for (const resourceType of getResourcesIn(roomData.totalResources)) {
      roomData.state[resourceType] =
        resourceLevelManager.determineResourceLevel(
          this,
 ...
 */
Room.prototype.getBestStorageTarget = function (
  this: Room,
  amount,
  resourceType,
) {
...
function determineBestStorageTarget(
  room: Room,
  amount: number,
  resourceType: ResourceConstant,
) {
  ...
  if (
    room.isEvacuating() &&
    terminalFree > room.terminal.store.getCapacity() * 0.2
  ) {
  ...
  if (
    ...
    terminalFree > amount + (resourceType === RESOURCE_ENERGY ? 0 : 5000)
  ) {
  ...
  if (
    ...
    room.terminal &&
    room.terminal.store[RESOURCE_ENERGY] < 7000 &&
    terminalFree > 0
  ) {
  ...
  if (
    storageFree >= amount &&
    terminalFree >= amount &&
    (room.storage.store[resourceType] || 0) / storageFree <
    (room.terminal.store[resourceType] || 0) / terminalFree
  ) {
    return room.storage;
  }
 ...
 *   The room's storage or terminal.
 */
Room.prototype.getBestStorageSource = function (
  this: Room,
  resourceType: ResourceConstant,
) {
  if (this.storage && this.terminal) {
    ...
    if (
      ...
      this.storage.store.getCapacity() <
      this.terminal.store[resourceType] / this.terminal.store.getCapacity() &&
      this.memory.fillTerminal !== resourceType
    ) {
    ...
    this.terminal?.store[resourceType] &&
    (!this.memory.fillTerminal || this.memory.fillTerminal !== resourceType)
  ) {
    return this.terminal;
  }
 ...
 *   The room's storage or terminal.
 */
Room.prototype.getBestCircumstancialStorageSource = function (
  this: Room,
  resourceType: ResourceConstant,
) {
  ...
  const secondaryFull =
    secondarySource.store.getUsedCapacity() >
    secondarySource.store.getCapacity() * 0.8;

  if (
    primarySource.store[resourceType] &&
    (!secondaryFull || !secondarySource.store[resourceType])
  ) {
    return primarySource;
  }

  if (
    secondarySource.store[resourceType] &&
    (resourceType === RESOURCE_ENERGY || secondaryFull)
  ) {
