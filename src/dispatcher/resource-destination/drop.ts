...
export default class DropDestination extends TaskProvider<
  DropDestinationTask,
...
> {
  ...
  addDropResourceTasks(
    context: ResourceDestinationContext,
    options: DropDestinationTask[],
  ) {
  ...
  execute(task: DropDestinationTask, context: ResourceDestinationContext) {
    ...
    if (
      ...
      creep.pos.getRangeTo(creep.room.getStorageLocation()) > 0
    ) {
